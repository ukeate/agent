...
describe('reasoningStore测试', () => {
  ...
  it('executeReasoning成功执行', async () => {
    ...
    expect(mockReasoningService.executeReasoning).toHaveBeenCalledWith(mockReasoningRequest);
  ...
  it('streamReasoning正确处理流式数据', async () => {
    ...
    expect(mockReasoningService.streamReasoning).toHaveBeenCalledWith(mockReasoningRequest);
  ...
  it('流式推理完成时正确处理', async () => {
    ...
    // 模拟完成事件
    act(() => {
      const completeCallback = mockEventSource.addEventListener.mock.calls
  ...
  it('getReasoningChain正确获取特定推理链', async () => {
    ...
    expect(mockReasoningService.getReasoningChain).toHaveBeenCalledWith('chain-123');
    expect(chainResult).toEqual(mockReasoningChain);
  });
  ...
  it('deleteReasoningChain正确删除推理链', async () => {
    ...
    expect(mockReasoningService.deleteReasoningChain).toHaveBeenCalledWith('chain-123');
  });
  ...
  it('recoverChain正确执行恢复', async () => {
    ...
    let recoveryResult;
    await act(async () => {
      recoveryResult = await result.current.recoverChain('chain-123', 'backtrack');
    });
    
    expect(mockReasoningService.recoverChain).toHaveBeenCalledWith('chain-123', 'backtrack');
