...
describe('ReasoningChainVisualization组件测试', () => {
  it('无数据时显示等待状态', () => {
    ...
    expect(
      screen.getByText('请在左侧"推理输入"标签页配置推理参数并开始推理')
    ).toBeInTheDocument()
  })
  ...
  it('步骤视图正确渲染所有步骤', () => {
    ...
    // 检查推理过程
    expect(
      screen.getByText('首先需要收集和分析现有的市场数据')
    ).toBeInTheDocument()
    expect(
      screen.getByText('通过统计分析识别重要的市场信号')
    ).toBeInTheDocument()
  ...
  it('结论卡片正确显示', () => {
    ...
    expect(
      screen.getByText('市场呈现上升趋势，建议适当增加投资')
    ).toBeInTheDocument()
  })
  ...
  it('时间线视图显示推理进行中状态', async () => {
    const user = userEvent.setup()
    render(<ReasoningChainVisualization chain={mockChain} isExecuting={true} />)
