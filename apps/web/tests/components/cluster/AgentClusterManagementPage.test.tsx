...
describe('AgentClusterManagementPage', () => {
      ...
      switch (path) {
        case '/status':
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ success: true, data: mockClusterStats })
          });
        case '/agents':
          return Promise.resolve({
            ok: true,
        ...
        case '/scaling/recommendations':
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ success: true, data: mockScalingRecommendations })
          });
        case '/metrics/query':
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ success: true, data: mockMetricsData })
          });
        default:
          return Promise.resolve({
            ok: true,
  ...
  it('opens agent details dialog', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('端点: http://localhost:8081')).toBeInTheDocument();
    });
  ...
  it('refreshes data when refresh button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith('/api/v1/cluster/status', expect.any(Object));
      expect(mockFetch).toHaveBeenCalledWith('/api/v1/cluster/agents', expect.any(Object));
      expect(mockFetch).toHaveBeenCalledWith('/api/v1/cluster/scaling/recommendations', expect.any(Object));
    });
  });
