...
describe('CoTReasoningChain', () => {
  ...
  it('renders reasoning chain with basic information', () => {
    ...
    expect(
      screen.getByText('分析性推理 - 逐步分解复杂问题')
    ).toBeInTheDocument()
  ...
  it('expands step details when clicked', async () => {
    ...
    const step1 =
      screen.getByText('步骤 1').closest('[data-testid]') ||
  ...
  it('shows metadata when expanded', async () => {
    ...
    const step1Header = screen
      .getByText('用户查询分析：需要评估贷款申请的风险')
      .closest('header')
    if (step1Header) {
  ...
  it('shows dependencies correctly', async () => {
    ...
    // 展开第三步（有依赖关系）
    const step3Header = screen
      .getByText('建议批准贷款申请，风险等级为低')
      .closest('header')
    if (step3Header) {
  ...
  it('handles different reasoning modes', () => {
    ...
    expect(
      screen.getByText('演绎推理 - 从一般原理推导具体结论')
    ).toBeInTheDocument()
  })
