...
describe('DemoScenarioSelector', () => {
  ...
  it('renders scenario selector with title and description', () => {
    ...
    expect(screen.getByText(/选择不同的业务场景来体验解释性AI/)).toBeInTheDocument();
  });
  ...
  it('displays learning objectives when expanded', async () => {
    ...
    if (infoButton) {
      ...
      await waitFor(() => {
        expect(screen.getByText('理解金融风险评估的多维度分析')).toBeInTheDocument();
        expect(screen.getByText('掌握置信度计算在信贷决策中的应用')).toBeInTheDocument();
      });
    }
  });
  ...
  it('displays usage instructions', () => {
    ...
    expect(screen.getByText('每个场景都包含完整的数据处理和决策解释流程')).toBeInTheDocument();
    expect(screen.getByText('建议按照从初级到高级的顺序体验不同复杂度的场景')).toBeInTheDocument();
  });
  ...
  it('handles scenario expansion and collapse', async () => {
    ...
    const infoButtons = screen.getAllByRole('button');
    const firstInfoButton = infoButtons.find(button => 
      button.querySelector('svg')?.getAttribute('data-lucide') === 'info'
    );
