...
describe('ConfidenceAnalysis', () => {
  ...
  it('shows confidence interpretation in overview', () => {
    ...
    expect(
      screen.getByText('决策具有高可信度，建议采用并保持监控。')
    ).toBeInTheDocument()
  })
  ...
  it('handles high uncertainty correctly', () => {
    ...
    expect(
      screen.getByText('决策可信度较低，建议重新评估或收集更多高质量数据。')
    ).toBeInTheDocument()
  })
  ...
  it('provides calibration recommendations', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('收集更多历史决策数据以改进校准')
      ).toBeInTheDocument()
      expect(
        screen.getByText('定期评估预测准确性与置信度的匹配程度')
      ).toBeInTheDocument()
      expect(
        screen.getByText('考虑不同场景下的校准表现差异')
      ).toBeInTheDocument()
  ...
  it('applies correct confidence level styling', () => {
    ...
    const { rerender } = render(
      <ConfidenceAnalysis metrics={veryHighMetrics} />
    )
