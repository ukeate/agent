...
test.describe('完整应用功能测试', () => {
  ...
  test('首页渲染验证', async () => {
    ...
    await page.screenshot({
      path: 'test-results/homepage-rendered.png',
      fullPage: true,
    })
  ...
  test('懒加载功能验证', async () => {
    ...
    // 验证页面最终加载成功
    await expect(
      page.locator('h1, h2, [data-testid="page-content"]')
    ).toBeVisible()
  ...
  test('JavaScript错误检查', async () => {
    ...
    // 不阻止测试，但记录错误
    console.log(
      `✅ JavaScript错误检查完成 (JS错误: ${jsErrors.length}, 页面错误: ${pageErrors.length})`
    )
  })
  ...
  test('左侧菜单收缩展开功能', async () => {
    ...
    const toggleButton = page
      .locator(
        '.ant-layout-sider-trigger, button[aria-label*="菜单"], button[aria-label*="menu"]'
      )
      .first()
  ...
  test('页面路由完整性验证', async () => {
    ...
    for (const route of criticalRoutes) {
      ...
      const hasContent = await page
        .locator('h1, h2, [data-testid="page-content"], main')
        .count()
