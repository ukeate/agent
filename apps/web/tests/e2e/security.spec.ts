...
test.describe('Security Management System', () => {
  ...
  test.describe('Security Dashboard', () => {
    test('should display security dashboard with statistics', async ({ page }) => {
      // 等待dashboard加载
    ...
    test('should handle alert resolution', async ({ page }) => {
      // 等待告警加载
      await page.waitForSelector('[data-testid="security-alert"]', { timeout: 10000 });
      
      // 找到活跃告警的解决按钮
      const resolveButton = page.locator('button:has-text("标记已解决")').first();
  ...
  test.describe('API Key Management', () => {
    ...
    test('should open create API key dialog', async ({ page }) => {
      ...
      await expect(page.locator('input[placeholder*="生产环境密钥"]')).toBeVisible();
    ...
    test('should create new API key', async ({ page }) => {
      ...
      // 验证密钥创建成功
      await expect(page.locator('text=新API密钥已创建')).toBeVisible({ timeout: 5000 });
  ...
  test.describe('Security Alerts', () => {
    ...
    test('should display alerts list', async ({ page }) => {
      ...
      await expect(page.locator('input[placeholder="搜索告警..."]')).toBeVisible();
    });
    ...
    test('should update alert status', async ({ page }) => {
      const investigateButton = page.locator('button:has-text("开始调查")').first();
      
      if (await investigateButton.isVisible()) {
        await investigateButton.click();
        
        // 验证状态更新
        await expect(page.locator('text=investigating')).toBeVisible({ timeout: 5000 });
      }
    });

    test('should mark alert as false positive', async ({ page }) => {
      const falsePositiveButton = page.locator('button:has-text("标记误报")').first();
      
      if (await falsePositiveButton.isVisible()) {
        await falsePositiveButton.click();
        
        // 验证状态更新
        await expect(page.locator('text=false_positive')).toBeVisible({ timeout: 5000 });
      }
    });
  });
  ...
  test.describe('Security Integration', () => {
    ...
    test('should handle API errors gracefully', async ({ page }) => {
      ...
      // 验证错误提示
      await expect(page.locator('text=/.*加载.*失败.*/')).toBeVisible({ timeout: 5000 });
    });
    ...
    test('should handle network timeout', async ({ page }) => {
      ...
      // 验证超时处理
      await expect(page.locator('text=/.*加载.*/')).toBeVisible({ timeout: 5000 });
    });
  });

  test.describe('Performance', () => {
    test('should load security dashboard within 2 seconds', async ({ page }) => {
    ...
    test('should handle large datasets efficiently', async ({ page }) => {
      ...
      // 验证大数据集渲染
      await expect(page.locator('[data-testid="alert-card"]').first()).toBeVisible({ timeout: 5000 });
    });
  });
});