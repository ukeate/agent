...
test.describe('Multi-Agent Collaboration', () => {
  test('multi-agent page loads correctly', async ({ page }) => {
    ...
    await expect(
      page.locator('text=新建对话').or(page.locator('text=清空对话'))
    ...
    await expect(page.locator('text=创建').first()).toBeVisible({
      timeout: 10000,
    })
  })
  ...
  test('page loads on mobile viewport', async ({ page }) => {
    ...
    await expect(page.locator('text=多个AI专家协作讨论')).toBeVisible({
      timeout: 10000,
    })
  })
