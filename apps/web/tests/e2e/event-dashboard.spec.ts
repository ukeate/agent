...
test.describe('事件监控仪表板', () => {
  ...
  test('实时监控开关功能', async ({ page }) => {
    ...
    await expect(
      page.locator('text=系统正在通过WebSocket接收实时事件')
    ).toBeVisible()
  ...
  test('集群状态显示', async ({ page }) => {
    const clusterCard = page.locator(
      '.ant-card:has(.ant-card-head-title:has-text("集群状态"))'
    )
    ...
    if ((await clusterCard.count()) > 0) {
      ...
      await expect(
        clusterCard.locator('span:has-text("状态")').first()
      ).toBeVisible()
  ...
  test('系统健康状态显示', async ({ page }) => {
    const healthCard = page.locator(
      '.ant-card:has(.ant-card-head-title:has-text("系统健康状态"))'
    )
  ...
  test('时间线显示测试', async ({ page }) => {
    const timelineCard = page.locator(
      '.ant-card:has(.ant-card-head-title:has-text("最近事件时间线"))'
    )
