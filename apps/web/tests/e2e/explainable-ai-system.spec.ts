...
test.describe('可解释AI系统E2E测试', () => {
      ...
      if (url.includes('/health')) {
          ...
          body: JSON.stringify({
            ...
            visualization_data: {
              factor_importance: {
                chart_type: 'bar',
                data: [
                  { name: 'credit_score', value: 0.56, weight: 0.8, impact: 0.7 },
                  { name: 'annual_income', value: 0.42, weight: 0.7, impact: 0.6 }
                ]
              },
        ...
        route.fulfill({
          ...
          body: JSON.stringify({
            ...
            decision_description: '贷款审批决策',
            decision_outcome: 'approved',
            summary_explanation: '基于综合评估，建议批准此贷款申请。主要依据包括良好的信用记录、稳定的收入来源和合理的债务比例。',
            detailed_explanation: '申请人信用评分为750分，年收入8万元，就业期间5年，债务收入比为25%。根据风险评估模型，该申请属于低风险类别，符合银行贷款审批标准。',
            components: [
              {
                id: 'demo-comp-001',
                factor_name: 'credit_score',
            ...
            visualization_data: {
              factor_importance: {
                chart_type: 'bar',
                data: [
                  { name: 'credit_score', value: 0.28, weight: 0.35, impact: 0.8 },
                  { name: 'annual_income', value: 0.175, weight: 0.25, impact: 0.7 },
                  { name: 'employment_duration', value: 0.12, weight: 0.2, impact: 0.6 },
  ...
  test('可解释AI页面基本功能测试', async ({ page }) => {
    ...
    // 验证主要功能区域存在
    await expect(page.locator('[data-testid="explanation-controls"]')).toBeVisible();
    await expect(page.locator('[data-testid="explanation-result"]')).toBeVisible();

    // 验证解释类型选择器
    const explanationTypeSelect = page.locator('[data-testid="explanation-type-select"]');
    await expect(explanationTypeSelect).toBeVisible();
    
    // 验证解释级别选择器
    const explanationLevelSelect = page.locator('[data-testid="explanation-level-select"]');
    await expect(explanationLevelSelect).toBeVisible();

    // 验证生成解释按钮
    const generateButton = page.locator('[data-testid="generate-explanation-btn"]');
  ...
  test('演示场景功能测试', async ({ page }) => {
    ...
    // 验证基本信息显示
    await expect(page.locator('[data-testid="decision-id"]')).toContainText('loan_demo_001');
    await expect(page.locator('[data-testid="decision-outcome"]')).toContainText('approved');
  ...
  test('反事实分析功能测试', async ({ page }) => {
    ...
    // 切换到反事实分析标签
    const counterfactualsTab = page.locator('[data-testid="counterfactuals-tab"]');
    await counterfactualsTab.click();

    // 验证反事实场景显示
    const counterfactualCard = page.locator('[data-testid="counterfactual-card"]').first();
  ...
  test('可视化图表功能测试', async ({ page }) => {
    ...
    // 验证置信度分解图表
    const confidenceChart = page.locator('[data-testid="confidence-breakdown-chart"]');
    await expect(confidenceChart).toBeVisible();

    // 验证图表控制按钮
    const chartToggleButtons = page.locator('[data-testid="chart-toggle-buttons"] button');
  ...
  test('自定义解释生成测试', async ({ page }) => {
    ...
    // 填写因子信息
    const factorNameInput = page.locator('[data-testid="factor-name-input"]').first();
    await factorNameInput.fill('test_factor');

    const factorValueInput = page.locator('[data-testid="factor-value-input"]').first();
    await factorValueInput.fill('test_value');

    const factorWeightInput = page.locator('[data-testid="factor-weight-input"]').first();
    await factorWeightInput.fill('0.8');

    const factorImpactInput = page.locator('[data-testid="factor-impact-input"]').first();
    ...
    // 验证解释结果
    await expect(page.locator('[data-testid="explanation-result"]')).toBeVisible();
    await expect(page.locator('[data-testid="decision-id"]')).toContainText('test_decision_001');
  });
  ...
  test('解释导出功能测试', async ({ page }) => {
    ...
    for (const format of exportFormats) {
      const exportButton = page.locator(`[data-testid="export-${format.toLowerCase()}-btn"]`);
    ...
    await expect(page.locator('[data-testid="export-status"]')).toContainText('正在导出', { timeout: 3000 });
  });
  ...
  test('数据持久化测试', async ({ page }) => {
    ...
    // 记录当前解释ID
    const decisionId = await page.locator('[data-testid="decision-id"]').textContent();
