...
test.describe('Multi-Agent Flow Control', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // 等待页面完全加载
    await expect(page.locator('h1:has-text("多智能体协作")')).toBeVisible({
      timeout: 15000,
    })
  })
  ...
  test('pause functionality stops token generation', async ({ page }) => {
    ...
    // 等待对话状态变为进行中
    await expect(page.locator('.bg-green-100:has-text("进行中")')).toBeVisible({
      timeout: 10000,
    })
    ...
    // 记录暂停前的消息数量
    const messagesBeforePause = await page
      .locator('.flex.flex-col.gap-4 > div')
      .count()
    ...
    const messagesAfterPause = await page
      .locator('.flex.flex-col.gap-4 > div')
      .count()
  ...
  test('resume functionality continues token generation', async ({ page }) => {
    ...
    await expect(
      page.locator('.bg-yellow-100:has-text("已暂停")')
    ).toBeVisible()

    // 记录恢复前的消息数量
    const messagesBeforeResume = await page
      .locator('.flex.flex-col.gap-4 > div')
      .count()
    ...
    const messagesAfterResume = await page
      .locator('.flex.flex-col.gap-4 > div')
      .count()
  ...
  test('terminate functionality completely stops conversation', async ({
    page,
  }) => {
    console.log('=== 测试中止功能 ===')
    ...
    // 验证UI返回到创建对话状态
    await expect(page.locator('h3:has-text("创建多智能体对话")')).toBeVisible({
      timeout: 5000,
    })
  ...
  test('smart auto-scroll when user is at bottom', async ({ page }) => {
    ...
    await page.fill(
      'textarea',
      '请进行详细的技术讨论，每个智能体都要详细回应多段内容，这是自动滚动测试'
    )
    await page.click('button:has-text("开始多智能体讨论")')
    ...
    const scrollHeightBefore = await chatContainer.evaluate(
      el => el.scrollHeight
    )
    console.log(
      `滚动到底部 - 位置: ${scrollBefore}, 总高度: ${scrollHeightBefore}`
    )
    ...
    const scrollHeightAfter = await chatContainer.evaluate(
      el => el.scrollHeight
    )
    const clientHeight = await chatContainer.evaluate(el => el.clientHeight)

    console.log(
      `新消息后 - 位置: ${scrollAfter}, 总高度: ${scrollHeightAfter}, 可见高度: ${clientHeight}`
    )
  ...
  test('smart auto-scroll does not scroll when user is not at bottom', async ({
    page,
  }) => {
    ...
    await page.fill(
      'textarea',
      '请进行详细的技术讨论，每个智能体都要详细回应多段内容，这是非底部滚动测试'
    )
    await page.click('button:has-text("开始多智能体讨论")')
