...
test.describe('完整应用功能验证', () => {
  test('应用完整功能测试 - 首页渲染正常', async ({ page }) => {
    ...
    const menuItems = await page
      .locator('.ant-menu-item, .ant-menu-submenu')
      .count()
    ...
    await page.screenshot({
      path: 'test-results/app-working-homepage.png',
      fullPage: true,
    })
  ...
  test('展开子菜单测试', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/qlearning-submenu-expanded.png',
    })
  ...
  test('应用无JavaScript错误', async ({ page }) => {
    ...
    console.log(
      `✅ 应用无严重JavaScript错误 (忽略了 ${jsErrors.length - criticalErrors.length} 个已知的图标错误)`
    )
  })
  ...
  test('所有18个功能分组验证', async ({ page }) => {
    ...
    // 截图记录完整菜单结构
    await page.screenshot({
      path: 'test-results/complete-menu-structure.png',
      fullPage: true,
    })
  })
  ...
  test('懒加载和路由正常工作', async ({ page }) => {
    ...
    for (const testPage of testPages) {
      ...
      const contentElements = await page
        .locator('h1, h2, .ant-card, .ant-form')
        .count()
