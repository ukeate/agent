...
test.describe('Chat Flow', () => {
  test('user can send and receive messages', async ({ page }) => {
    ...
    try {
      await expect(page.locator('text=智能体正在思考...')).toBeVisible({
        timeout: 1000,
      })
    } catch {
    ...
    await expect(
      page.locator('text=我收到了你的消息：Hello, AI!').first()
  ...
  test('input validation works correctly', async ({ page }) => {
    ...
    await expect(
      page.locator('text=消息长度不能超过2000个字符').first()
    ).toBeVisible()
  })
  ...
  test('responsive design works on mobile', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'AI Agent' })).toBeVisible()
    await expect(
      page.locator('textarea[placeholder="请输入你的问题..."]')
    ).toBeVisible()
  })
