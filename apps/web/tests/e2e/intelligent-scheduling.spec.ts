...
test.describe('智能调度监控 E2E 测试', () => {
        ...
        body: JSON.stringify({
          ...
          predictive_scheduling: {
            ...
            scaling_recommendations: {
              ...
              target_workers: 6,
              confidence: 0.87,
              reason: 'High queue depth and worker overload detected. CPU usage above 70% threshold.'
            },
            resource_forecast: {
              next_hour: {
                cpu_usage: 78.0,
  ...
  test('预测性调度建议显示测试', async ({ page }) => {
    ...
    // 验证建议原因
    await expect(page.locator('text=High queue depth and worker overload detected')).toBeVisible();
  });
  ...
  test('不同扩缩容建议显示测试', async ({ page }) => {
        ...
        body: JSON.stringify({
          ...
          predictive_scheduling: {
            ...
            scaling_recommendations: {
              ...
              target_workers: 2,
              confidence: 0.92,
              reason: 'Low resource utilization detected. System is over-provisioned.'
            },
            resource_forecast: {
              next_hour: {
                cpu_usage: 20.0,
    ...
    await expect(page.locator('text=Low resource utilization detected')).toBeVisible();
  });
  ...
  test('工作者负载进度条测试', async ({ page }) => {
    ...
    // worker-002: 35%
    await expect(workerProgressBars.nth(1)).toHaveAttribute('aria-valuenow', '35');
    // worker-003: 85% (应该显示为异常状态)
    await expect(workerProgressBars.nth(2)).toHaveAttribute('aria-valuenow', '85');
    // worker-004: 0%
    await expect(workerProgressBars.nth(3)).toHaveAttribute('aria-valuenow', '0');
  });
  ...
  test('SLA合规率颜色显示测试', async ({ page }) => {
    ...
    const slaStatistic = page.locator('.ant-statistic-content-value:has-text("89.4")');
    ...
    // 验证高合规率显示为绿色
    const highSlaStatistic = page.locator('.ant-statistic-content-value:has-text("97.5")');
