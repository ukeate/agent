...
test.describe('LangGraph 0.6.5 Features E2E Tests', () => {
  ...
  test.describe('Context API 新特性测试', () => {
    test('新Context API vs 旧config模式对比', async ({ request }) => {
      // 测试新Context API
      const newApiResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/context-api/demo`,
        {
          data: {
      ...
      expect(newApiResult.metadata.context_schema).toBe(
      ...
      // 验证消息内容包含用户信息
      const assistantMessage = newApiResult.result.messages.find(
      ...
      // 测试旧config模式
      const oldApiResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/context-api/demo`,
        {
          data: {
    ...
    test.describe.parallel('三种durability模式测试', () => {
      ;['exit', 'async', 'sync'].forEach(mode => {
        test(`${mode} durability模式`, async ({ request }) => {
          const response = await request.post(
            `${API_BASE_URL}/api/v1/langgraph/durability/demo`,
            {
              data: {
  ...
  test.describe('Node Caching测试', () => {
    test('缓存功能验证', async ({ request }) => {
      ...
      const cachedResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/caching/demo`,
        {
          data: {
            message: '计算密集型任务测试',
            enable_cache: true,
      ...
      expect(cacheStats.actual_computations).toBeLessThan(
        cacheStats.total_executions
      )

      // 验证性能提升
      const performanceImprovement = parseFloat(
        cachedResult.metadata.performance_improvement.replace('%', '')
      )
      expect(performanceImprovement).toBeGreaterThan(0)

      // 测试禁用缓存的对比
      const noCacheResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/caching/demo`,
        {
          data: {
            message: '计算密集型任务测试',
      ...
      expect(noCacheResult.metadata.cache_statistics.actual_computations).toBe(
        3
      )
    })
    ...
    test('缓存管理端点', async ({ request }) => {
      ...
      const statsResponse = await request.get(
        `${API_BASE_URL}/api/v1/langgraph/cache/stats`
      )
      ...
      const clearResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/cache/clear`
      )
  ...
  test.describe('Pre/Post Hooks测试', () => {
    test('Hooks功能验证', async ({ request }) => {
      ...
      // 测试启用所有hooks
      const hooksResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/hooks/demo`,
        {
          data: {
      ...
      expect(hooksResult.metadata.final_message_count).toBeGreaterThan(
        hooksResult.metadata.original_message_count
      )
      ...
      // 测试禁用hooks对比
      const noHooksResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/hooks/demo`,
        {
          data: {
            messages: testMessages,
      ...
      expect(noHooksResult.metadata.hooks_executed).toBeLessThan(
        hooksResult.metadata.hooks_executed
      )
    })
    ...
    test('Hooks状态管理', async ({ request }) => {
      ...
      const statusResponse = await request.get(
        `${API_BASE_URL}/api/v1/langgraph/hooks/status`
      )
  ...
  test.describe('完整工作流集成测试', () => {
    test('所有特性综合演示', async ({ request }) => {
      const response = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/complete-demo`
      )
      ...
      // 验证所有特性都被演示了
      expect(result.metadata.features_demonstrated).toContain(
  ...
  test.describe('错误处理和边界情况测试', () => {
    test('无效请求处理', async ({ request }) => {
      // 测试无效的durability模式
      const invalidDurabilityResponse = await request.post(
        `${API_BASE_URL}/api/v1/langgraph/durability/demo`,
        {
          data: {
