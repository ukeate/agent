...
test.describe('pgvector优化功能E2E测试', () => {
  test.beforeAll(async ({ request }) => {
    ...
    try {
      await request.delete(
        `${API_BASE_URL}/api/v1/pgvector/collections/${TEST_COLLECTION}`
      )
    } catch (e) {
      // 集合可能不存在，忽略错误
    }
  ...
  test('创建向量集合', async ({ request }) => {
    ...
    const response = await request.post(
      `${API_BASE_URL}/api/v1/pgvector/collections`,
      {
        data: createRequest,
      }
    )
  ...
  test('批量插入向量', async ({ request }) => {
    ...
    const response = await request.post(
      `${API_BASE_URL}/api/v1/pgvector/vectors`,
      {
        data: insertRequest,
      }
    )
  ...
  test('向量相似性搜索', async ({ request }) => {
    ...
    const response = await request.post(
      `${API_BASE_URL}/api/v1/pgvector/search/similarity`,
      {
        data: searchRequest,
      }
    )
  ...
  test('混合搜索功能', async ({ request }) => {
    ...
    const response = await request.post(
      `${API_BASE_URL}/api/v1/pgvector/search/hybrid`,
      {
        data: searchRequest,
      }
    )
  ...
  test('获取集合统计信息', async ({ request }) => {
    const response = await request.get(
      `${API_BASE_URL}/api/v1/pgvector/collections/${TEST_COLLECTION}/stats`
    )
    expect(response.ok()).toBeTruthy()
  ...
  test('向量量化配置', async ({ request }) => {
    ...
    const response = await request.post(
      `${API_BASE_URL}/api/v1/pgvector/quantization/configure`,
      {
  ...
  test('性能监控报告', async ({ request }) => {
    const response = await request.get(
      `${API_BASE_URL}/api/v1/pgvector/monitoring/performance-report?time_range_hours=1`
    )
    expect(response.ok()).toBeTruthy()
  ...
  test('集合优化', async ({ request }) => {
    const response = await request.post(
      `${API_BASE_URL}/api/v1/pgvector/collections/${TEST_COLLECTION}/optimize`
    )
    expect(response.ok()).toBeTruthy()
