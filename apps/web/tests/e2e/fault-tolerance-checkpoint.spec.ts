...
test.describe('容错和检查点功能 E2E 测试', () => {
  ...
  test('容错监控功能完整性测试', async ({ page }) => {
    ...
    await expect(
      page
        ...
        .filter({ hasText: '失败连接' })
        .locator('.ant-statistic-content')
    ).toContainText('1') // 失败连接数

    await expect(page.locator('text=总重连次数')).toBeVisible()
    await expect(
      page
        .locator('.ant-statistic')
        .filter({ hasText: '总重连次数' })
        .locator('.ant-statistic-content')
    ).toContainText('5') // 重连次数
    ...
    await expect(
      page.locator('.ant-tag:has-text("RECONNECTING")')
    ).toBeVisible()
    await expect(page.locator('.ant-tag:has-text("FAILED")')).toBeVisible()

    // 验证心跳状态
    await expect(
      page.locator('.ant-tag:has-text("正常")').first()
    ).toBeVisible()
    await expect(
      page.locator('.ant-tag:has-text("异常")').first()
    ).toBeVisible()
  ...
  test('容错监控重连功能测试', async ({ page }) => {
    // 模拟重连API
    await page.route(
      '**/api/v1/streaming/fault-tolerance/reconnect/session-002',
      async route => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
  ...
  test('检查点管理功能完整性测试', async ({ page }) => {
    ...
    await expect(
      page
        .locator('.ant-statistic')
        .filter({ hasText: '总检查点数' })
        .locator('.ant-statistic-content')
    ).toContainText('2') // 总检查点数
    await expect(page.locator('text=5.86 MB')).toBeVisible() // 存储空间
    await expect(
      page
        .locator('.ant-statistic')
        .filter({ hasText: '覆盖作业数' })
        .locator('.ant-statistic-content')
    ).toContainText('2') // 覆盖作业数

    // 验证检查点类型分布
    await expect(
      page.locator('.ant-tag:has-text("manual")').first()
    ).toBeVisible()
    await expect(
      page.locator('.ant-tag:has-text("auto")').first()
    ).toBeVisible()
    ...
    // 验证类型标签
    await expect(
      page.locator('.ant-tag', { hasText: /^MANUAL$/ })
    ).toBeVisible()
  ...
  test('检查点搜索功能测试', async ({ page }) => {
    ...
    const searchInput = page.locator(
      'input[placeholder="搜索检查点ID或作业ID"]'
    )
  ...
  test('检查点操作功能测试', async ({ page }) => {
    ...
    // 测试恢复操作
    const restoreButtons = page.locator(
      '.checkpoint-manager button:has-text("恢复")'
    )
    ...
    const deleteButtons = page.locator(
      '.checkpoint-manager button:has-text("删除")'
    )
