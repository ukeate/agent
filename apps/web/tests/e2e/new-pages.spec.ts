...
test.describe('新增页面功能测试', () => {
  ...
  test('异步智能体管理页面 - 基础功能', async ({ page }) => {
    ...
    // 检查统计卡片
    await expect(page.locator('.ant-statistic-title').first()).toContainText(
      '运行中的智能体'
    )
    ...
    // 填写表单并创建
    await modal
      .locator('input[placeholder="请输入智能体名称"]')
      .fill('测试智能体')
    await modal.locator('.ant-select-selector').click()
    await page
      .locator('.ant-select-dropdown')
      .getByText('知识检索', { exact: true })
      .click()
  ...
  test('事件监控仪表板页面 - 基础功能', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: '关闭实时监控' })
    ).toBeVisible()
  })
  ...
  test('性能监控页面 - 基础功能', async ({ page }) => {
    ...
    await expect(
      page.locator('.ant-card-head-title:text("历史性能数据")')
    ).toBeVisible()

    // 切换到性能分析标签页
    await page.getByRole('tab', { name: '性能分析' }).click()
    await expect(
      page.locator('.ant-card-head-title:text("性能评分")')
    ).toBeVisible()

    // 测试自动刷新切换
    await page.getByRole('button', { name: '关闭自动刷新' }).click()
    await expect(
      page.getByRole('button', { name: '开启自动刷新' })
    ).toBeVisible()
  })
