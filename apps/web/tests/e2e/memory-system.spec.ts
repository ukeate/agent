...
const testMemories = {
  ...
  semantic: {
    content:
      '测试语义记忆 - 记忆管理系统是AI智能体的核心能力之一，包括工作记忆、情景记忆和语义记忆三层架构',
    importance: 0.9,
...
test.describe('记忆管理系统E2E测试', () => {
  ...
  test('记忆层级架构页面基本功能', async ({ page }) => {
    ...
    // 验证三层记忆架构
    await expect(
      page.locator('.ant-card-head-title').filter({ hasText: '工作记忆' })
    ).toBeVisible()
    await expect(
      page.locator('.ant-card-head-title').filter({ hasText: '情景记忆' })
    ).toBeVisible()
    await expect(
      page.locator('.ant-card-head-title').filter({ hasText: '语义记忆' })
    ).toBeVisible()
    ...
    await expect(
      page.locator(
        'text=记忆通过重要性评估和访问频率，逐级提升到更持久的存储层'
      )
    ).toBeVisible()
  })
  ...
  test('记忆召回测试页面功能', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("向量搜索")').first()
    ).toHaveClass(/ant-btn-primary/)

    await page.click('text=时间搜索')
    await expect(
      page.locator('button:has-text("时间搜索")').first()
    ).toHaveClass(/ant-btn-primary/)

    await page.click('text=混合召回')
    await expect(
      page.locator('button:has-text("混合召回")').first()
    ).toHaveClass(/ant-btn-primary/)
  ...
  test('记忆召回页面交互功能', async ({ page }) => {
    ...
    for (const mode of modes) {
      ...
      await expect(
        page.locator(`button:has-text("${mode}")`).first()
      ).toHaveClass(/ant-btn-primary/)
    }
