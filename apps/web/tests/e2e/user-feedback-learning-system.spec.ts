...
test.describe('用户反馈学习系统', () => {
  ...
  test.describe('反馈收集功能', () => {
    test('应该能够提交评分反馈', async () => {
      ...
      // 验证评分被选中
      await expect(page.locator('[data-testid="rating-star-5"]')).toHaveClass(
        /selected/
      )
    ...
    test('应该能够提交点赞反馈', async () => {
      ...
      // 验证点踩按钮未激活
      await expect(
        page.locator('[data-testid="dislike-button"]')
    ...
    test('应该能够提交评论反馈', async () => {
      ...
      ).toContainText(`${commentText.length}/500`)
      ...
      await expect(
        page.locator('[data-testid="comment-success"]')
      ).toContainText('评论已提交')

      // 验证评论区域被禁用
      await expect(
        page.locator('[data-testid="comment-textarea"]')
      ).toBeDisabled()
    })
    ...
    test('应该支持批量反馈提交', async () => {
      ...
      // 验证所有反馈都被记录
      await expect(
        page.locator('[data-testid="feedback-summary"]')
      ).toContainText('评分: 4/5')
      await expect(
        page.locator('[data-testid="feedback-summary"]')
      ).toContainText('态度: 喜欢')
      await expect(
  ...
  test.describe('反馈分析界面', () => {
    test('应该显示反馈统计数据', async () => {
      ...
      // 验证统计数据
      await expect(
        page.locator('[data-testid="total-feedback-count"]')
      ).toContainText(/\d+/)
      await expect(
        page.locator('[data-testid="average-rating"]')
      ).toContainText(/\d+\.\d+/)
      await expect(
        page.locator('[data-testid="positive-feedback-ratio"]')
      ).toContainText(/\d+%/)
    })
    ...
    test('应该显示反馈质量监控', async () => {
      ...
      await expect(
        page.locator('[data-testid="spam-detection-rate"]')
      ).toBeVisible()
  ...
  test.describe('用户反馈档案', () => {
    test('应该显示用户反馈历史', async () => {
      ...
      await expect(
        page.locator('[data-testid="feedback-item"]').first()
      ).toBeVisible()
    })
    ...
    test('应该显示用户反馈偏好分析', async () => {
      ...
      await expect(
        page.locator('[data-testid="category-preferences"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="feedback-patterns"]')
      ).toBeVisible()
      await expect(
  ...
  test.describe('物品反馈分析', () => {
    test('应该显示物品反馈汇总', async () => {
      ...
      await expect(page.locator('[data-testid="item-rating"]')).toContainText(
        /\d+\.\d+/
      )
      await expect(
        page.locator('[data-testid="feedback-count"]')
      ).toContainText(/\d+/)
    })
  ...
  test.describe('实时反馈更新', () => {
    test('应该实时显示新的反馈数据', async () => {
      ...
      const initialCount = await page
        .locator('[data-testid="total-feedback-count"]')
        .textContent()
      ...
      await page.waitForFunction(
        initialCount => {
          const currentCount = document.querySelector(
            '[data-testid="total-feedback-count"]'
          )?.textContent
    ...
    test('应该显示实时反馈流', async () => {
      ...
      // 验证新反馈出现在流中
      await page.waitForSelector('[data-testid="new-feedback-item"]', {
        timeout: 5000,
      })
    })
  })
  ...
  test.describe('移动端适配', () => {
    test('应该在移动设备上正确显示反馈表单', async () => {
      ...
      // 验证紧凑布局
      await expect(page.locator('[data-testid="feedback-form"]')).toHaveClass(
        /mobile/
      )
    ...
    test('应该支持移动端手势操作', async () => {
      ...
      if (await ratingSlider.isVisible()) {
        await ratingSlider.dragTo(
          page.locator('[data-testid="rating-value-4"]')
        )
        await expect(
          page.locator('[data-testid="current-rating"]')
        ).toContainText('4')
      }
    })
  })
