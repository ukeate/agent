...
test.describe('基础RAG系统测试', () => {
  ...
  test('RAG页面加载和基本界面展示', async ({ page }) => {
    ...
    // 验证页面标题和导航
    await expect(
      page.getByRole('heading', { name: /RAG.*搜索/i })
    ).toBeVisible()
  ...
  test('RAG查询面板功能测试', async ({ page }) => {
    ...
    const queryInput = page
      .locator(
        'textarea[placeholder*="请输入"], input[placeholder*="搜索"], textarea[placeholder*="查询"]'
      )
      .first()
  ...
  test('RAG搜索结果展示', async ({ page }) => {
    ...
    // 验证搜索完成提示（可选）
    const completionStatus = page
      .locator('text=搜索完成')
      .or(page.locator('text=找到.*结果'))
    try {
    ...
    await expect(
      page
        .locator('[data-testid="rag-results"], .rag-results, .search-results')
        .first()
    ).toBeVisible()
  ...
  test('RAG索引状态监控', async ({ page }) => {
    ...
    if (
      await page
        .locator('[data-testid="index-status"], .index-status')
        .first()
        .isVisible()
    ) {
      ...
      // 验证向量数量统计
      await expect(
        statusPanel.locator('text=468').or(statusPanel.locator('text=向量'))
      ).toBeVisible()
      await expect(
        statusPanel.locator('text=1746').or(statusPanel.locator('text=代码'))
      ).toBeVisible()

      // 验证健康状态
      await expect(
        statusPanel.locator('text=green').or(statusPanel.locator('text=正常'))
      ).toBeVisible()
    }

    // 移动端/平板：查看状态抽屉
    const statusButton = page
      .locator('button:has-text("索引状态")')
      .or(page.locator('button:has-text("查看索引状态")'))
      .first()
    if (await statusButton.isVisible()) {
      ...
      // 验证抽屉内容
      await expect(
        page.locator('.ant-drawer').locator('text=索引状态')
      ).toBeVisible()
  ...
  test('RAG历史记录和清空功能', async ({ page }) => {
    ...
    if (await historyButton.isVisible()) {
      ...
      // 验证历史记录页面
      await expect(
        page.locator('text=第一次搜索').or(page.locator('text=第二次搜索'))
      ).toBeVisible()
    }
  })
  ...
  test('RAG响应式布局测试', async ({ page }) => {
    ...
    const queryPanel = page
      .locator('[data-testid="query-panel"], .rag-query-panel')
      .first()
    const resultsPanel = page
      .locator('[data-testid="results-panel"], .rag-results')
      .first()
    const statusPanel = page
      .locator('[data-testid="status-panel"], .rag-index-status')
      .first()
    ...
    const mobileMenuButton = page
      .locator('button[data-testid="mobile-menu"], button:has(.anticon-menu)')
      .first()
    if (await mobileMenuButton.isVisible()) {
      ...
      // 验证移动端菜单项
      await expect(
        page.locator('.ant-drawer').locator('text=搜索面板')
      ).toBeVisible()
      await expect(
        page.locator('.ant-drawer').locator('text=索引状态')
      ).toBeVisible()
  ...
  test('RAG搜索参数配置', async ({ page }) => {
    ...
    const advancedButton = page
      .locator('button:has-text("高级")')
      .or(page.locator('text=高级搜索'))
      .or(page.locator('button:has-text("选项")'))
      .first()
    if (await advancedButton.isVisible()) {
      ...
      const limitInput = page
        .locator('input[placeholder*="结果数量"], input[type="number"]')
        .first()
      ...
      const typeFilter = page
        .locator('select[placeholder*="文件类型"], .ant-select')
        .first()
  ...
  test('RAG错误处理测试', async ({ page }) => {
    ...
    // 验证错误提示
    await expect(
      page.locator('text=向量数据库连接失败').or(page.locator('text=系统错误'))
  ...
  test('RAG页面导航和路由', async ({ page }) => {
    ...
    const ragLink = page
      .locator('a[href="/rag"]')
      .or(page.locator('text=RAG'))
      .or(page.locator('text=检索'))
      .first()
    ...
    const upgradeButton = page
      .locator('button:has-text("升级到 Agentic RAG")')
      .or(page.locator('a:has-text("智能升级")'))
      .first()
  ...
  test('RAG键盘快捷键', async ({ page }) => {
    ...
    // 验证搜索被触发
    await expect(
      page.locator('text=开始搜索').or(page.locator('text=正在搜索'))
    ).toBeVisible()
