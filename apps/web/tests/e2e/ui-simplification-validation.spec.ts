...
test.describe('UI简化版应用验证', () => {
  ...
  test('2. 验证左侧导航菜单显示6个核心功能', async ({ page }) => {
    ...
    // 验证具体菜单项文本
    await expect(page.locator('.ant-menu-item').nth(0)).toContainText(
      '单代理对话'
    )
    await expect(page.locator('.ant-menu-item').nth(1)).toContainText(
      '多代理协作'
    )
    await expect(page.locator('.ant-menu-item').nth(2)).toContainText(
      '监督者编排'
    )
    await expect(page.locator('.ant-menu-item').nth(3)).toContainText(
      '基础RAG检索'
    )
    await expect(page.locator('.ant-menu-item').nth(4)).toContainText(
      '用户反馈系统'
    )
    await expect(page.locator('.ant-menu-item').nth(5)).toContainText(
      'Q-Learning算法'
    )
  ...
  test('4. 检查页面加载显示"加载中..."的Suspense fallback', async ({
    page,
  }) => {
    // 刷新页面并立即检查加载状态
    await page.reload()
