...
test.describe('Q-Learning System E2E Tests', () => {
  ...
  test('应该能够访问Q-Learning主页面并显示基本功能', async ({ page }) => {
    ...
    await expect(page.locator('text=强化学习智能体训练与策略优化平台')).toBeVisible();
  ...
  test('应该能够导航到表格Q-Learning页面', async ({ page }) => {
    ...
    // 验证页面内容
    await expect(page.locator('text=表格Q-Learning (Tabular Q-Learning)')).toBeVisible();
  ...
  test('应该能够导航到Deep Q-Network页面并查看网络架构', async ({ page }) => {
    ...
    // 验证DQN核心技术
    await expect(page.locator('text=经验回放 (Experience Replay)')).toBeVisible();
  ...
  test('应该能够导航到探索策略页面并测试不同策略', async ({ page }) => {
    ...
    // 验证页面标题
    await expect(page.locator('text=探索策略系统 (Exploration Strategies)')).toBeVisible();
  ...
  test('应该显示系统实时更新的统计数据', async ({ page }) => {
    ...
    // 记录初始统计数据
    const initialAgents = await page.locator('[data-testid="agents-count"]').textContent().catch(() => '0');
