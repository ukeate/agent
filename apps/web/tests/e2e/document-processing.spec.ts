...
test.describe('文档处理系统E2E测试', () => {
    ...
    await page.route('/api/v1/documents/upload*', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          title: 'test.txt',
          file_type: 'txt',
          content: 'This is a test document with some sample content for processing.',
          metadata: {
            file_size: 64,
            pages: 1
          },
  ...
  test('批量上传功能测试', async ({ page }) => {
    ...
    await page.route('/api/v1/documents/batch-upload*', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          failed: 0,
          results: [
            { doc_id: 'doc1', title: 'test.txt', file_type: 'txt', status: 'success' },
            { doc_id: 'doc2', title: 'test.md', file_type: 'md', status: 'success' },
            { doc_id: 'doc3', title: 'test.py', file_type: 'py', status: 'success' }
          ]
        })
      })
    })
    ...
    await page.route('/api/v1/documents/upload*', async route => {
      ...
      const filename = url.includes('test.txt') ? 'test.txt' : 
                     url.includes('test.md') ? 'test.md' : 'test.py'
  ...
  test('文档标签功能测试', async ({ page }) => {
    ...
    if (await tagButton.isVisible()) {
      ...
      // 检查标签是否出现
      await expect(page.locator('text=Python').or(page.locator('[data-testid="tag-chip"]'))).toBeVisible({ timeout: 5000 })
    }
  })

  test('文档关系分析功能测试', async ({ page }) => {
    // 模拟关系分析API
    await page.route('/api/v1/documents/*/analyze-relationships', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
  ...
  test('支持格式查询测试', async ({ page }) => {
    ...
    await page.route('/api/v1/documents/supported-formats', async route => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          formats: ['.pdf', '.docx', '.xlsx', '.pptx', '.txt', '.md', '.py', '.js'],
          categories: {
...
test.describe('并发处理测试', () => {
  test('多文件同时上传', async ({ page }) => {
    ...
    // 模拟快速连续上传多个文件
    const files = ['test.txt', 'test.md', 'test.py', 'test.js'].map(f => join(testDir, f))
