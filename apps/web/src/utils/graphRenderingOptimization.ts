  ...
  getMetrics() {
    ...
    const avgFrameRate =
      this.metrics.frameRate.length > 0
        ? this.metrics.frameRate.reduce((a, b) => a + b, 0) /
          this.metrics.frameRate.length
        : 0
  ...
  private measureFrameRate(): void {
    const measure = (currentTime: number) => {
      ...
      if (currentTime - this.lastFrameTime >= 1000) {
        const fps =
          this.frameCount / ((currentTime - this.lastFrameTime) / 1000)
...
export class VirtualizationRenderer {
  ...
  getVisibleNodes(): Set<string> {
    ...
    if (this.visibleNodes.size > PERFORMANCE_THRESHOLDS.MAX_VISIBLE_NODES) {
      const nodes = Array.from(this.visibleNodes).slice(
        0,
  ...
  private calculateBounds(nodes: GraphNode[]): {
    x: number
    y: number
    width: number
    height: number
...
export class LODRenderer {
  ...
  calculateLOD(
    ...
    const distance = Math.sqrt(
      Math.pow(nodePosition.x - centerX, 2) +
        Math.pow(nodePosition.y - centerY, 2)
    )

    // 基于距离和缩放级别确定LOD
    const normalizedDistance =
      distance / (viewport.zoom * Math.max(viewport.width, viewport.height))
...
export class ProgressiveLoader {
  ...
  async loadNextChunk(cy: CytoscapeCore): Promise<boolean> {
    ...
    try {
      ...
      const chunkData = this.chunkArray(
        chunk.data as any[],
        PERFORMANCE_THRESHOLDS.CHUNK_SIZE
      )[0]
      ...
      // 如果还有剩余数据，重新加入队列
      const remainingData = (chunk.data as any[]).slice(
        PERFORMANCE_THRESHOLDS.CHUNK_SIZE
      )
...
export function getOptimalRenderingEngine(
  nodeCount: number,
  edgeCount: number
): 'canvas' | 'webgl' | 'svg' {
  if (
    nodeCount > PERFORMANCE_THRESHOLDS.LARGE_GRAPH ||
    edgeCount > PERFORMANCE_THRESHOLDS.LARGE_GRAPH * 5
  ) {
    return 'webgl'
  } else if (
    nodeCount > PERFORMANCE_THRESHOLDS.MEDIUM_GRAPH ||
    edgeCount > PERFORMANCE_THRESHOLDS.MEDIUM_GRAPH * 3
  ) {
    return 'canvas'
  } else {
    return 'svg'
