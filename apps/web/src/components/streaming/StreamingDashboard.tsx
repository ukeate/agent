    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                {state.systemMetrics?.total_tokens_processed?.toLocaleString() ||
                  0}
              </p>
            </div>
          ...
          <p className="text-xs text-gray-500 mt-2">
            事件:{' '}
            {state.systemMetrics?.total_events_processed?.toLocaleString() || 0}
          </p>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p
                className={`text-2xl font-bold ${
                  state.backpressureStatus
                    ? getThrottleLevelColor(
                        state.backpressureStatus.throttle_level
                      )
                    : 'text-gray-500'
                }`}
              >
          ...
          {state.backpressureStatus && (
            <p className="text-xs text-gray-500 mt-2">
              缓冲区使用率:{' '}
              {(state.backpressureStatus.buffer_usage_ratio * 100).toFixed(1)}%
            </p>
          )}
        </div>
      </div>
              ...
              {state.flowControlMetrics.rate_limiter_stats && (
                <div className="border rounded p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      限制: {state.flowControlMetrics.rate_limiter_stats.rate}
                      /秒
                    </div>
                    <div>
                      突发: {state.flowControlMetrics.rate_limiter_stats.burst}
                    </div>
                    <div>
                      当前额度:{' '}
                      {state.flowControlMetrics.rate_limiter_stats.current_allowance.toFixed(
                        1
                      )}
                    </div>
                    <div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {state.queueStatus && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium">
                    {state.queueStatus.system_summary.total_queues}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium">
                    {(
                      state.queueStatus.system_summary.system_utilization * 100
                    ).toFixed(1)}
                    %
                  </span>
                </div>
                <div>
                  ...
                  <span
                    className={`ml-2 font-medium ${
                      state.queueStatus.system_summary.is_running
                        ? 'text-green-600'
                        : 'text-red-600'
                    }`}
                  >
              ...
              {/* 队列详情 */}
              {Object.entries(state.queueStatus.queue_metrics).map(
                ([name, metrics]) => (
                  <div key={name} className="border rounded p-3">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span
                        className={`text-sm ${metrics.is_overloaded ? 'text-red-600' : 'text-green-600'}`}
                      >
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      <div>
                        大小: {metrics.current_size}/{metrics.max_size}
                      </div>
                      <div>
                        利用率: {(metrics.utilization * 100).toFixed(1)}%
                      </div>
