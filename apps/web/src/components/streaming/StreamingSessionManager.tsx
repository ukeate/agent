...
export const StreamingSessionManager: React.FC<StreamingSessionManagerProps> = ({
  onSessionCreated,
  onSessionStopped
}) => {
  ...
  const [activeStreams, setActiveStreams] = useState<Record<string, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">创建流式会话</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setCreateForm(prev => ({ ...prev, buffer_size: Number(e.target.value) }))}
              className="w-full border border-gray-300 rounded-md px-3 py-2"
              min="10"
              max="1000"
            />
              ...
              <div key={sessionId} className="border rounded-lg p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">智能体: {session.agent_id}</p>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {activeStreams[sessionId] && (
                      <span className={`text-xs ${getConnectionStatusColor(activeStreams[sessionId].status)}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                  <div>
                    ...
                    <span className="ml-1 font-medium">{session.token_count}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">事件数:</span>
                    <span className="ml-1 font-medium">{session.event_count}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">速率:</span>
                    <span className="ml-1 font-medium">{session.tokens_per_second.toFixed(1)} t/s</span>
                  </div>
                  <div>
                    <span className="text-gray-600">错误数:</span>
                    <span className="ml-1 font-medium">{session.error_count}</span>
                  </div>
                </div>
                ...
                <div className="flex space-x-2">
                  {!activeStreams[sessionId] ? (
                    <>
                      <button
                        onClick={() => startSSEStream(sessionId, createForm.message)}
                ...
                {/* 流消息显示 */}
                {activeStreams[sessionId] && activeStreams[sessionId].messages.length > 0 && (
                  <div className="mt-4">
                    <h5 className="font-medium text-gray-700 mb-2">流消息:</h5>
                    <div 
                      ref={streamContainerRef}
                      className="bg-gray-50 border rounded p-3 max-h-40 overflow-y-auto"
                    >
                      {activeStreams[sessionId].messages.map((message, index) => (
                        <div key={index} className="mb-2 text-xs">
                          <span className={`inline-block px-2 py-1 rounded text-white text-xs mr-2 ${
                            message.type === 'token' ? 'bg-blue-500' :
                            message.type === 'complete' ? 'bg-green-500' :
                            message.type === 'error' ? 'bg-red-500' :
                            'bg-gray-500'
                          }`}>
                            {message.type}
                          </span>
                          <span className="font-mono">{JSON.stringify(message.data)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
