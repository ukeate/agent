  ...
  const fetchSchedulingStats = async () => {
    ...
    try {
      const response = await apiFetch(
        buildApiUrl('/api/v1/batch/scheduling/stats')
      )
  ...
  const workerColumns = [
    ...
    },
    {
      ...
      render: (status: string) => (
        <Tag color={getWorkerStatusColor(status)}>{status.toUpperCase()}</Tag>
      ),
    },
    {
      title: '当前负载',
    ...
    },
    {
      ...
      render: (_: any, record: SLARequirement) => (
        <div>
          <div>
            完成时间:{' '}
            {record.current_performance.avg_completion_time.toFixed(2)}s
          </div>
          <div>
            失败率: {(record.current_performance.failure_rate * 100).toFixed(1)}
            %
          </div>
              ...
              <Col span={12}>
                <div className="mb-2">
                  ...
                  <Progress
                    ...
                    strokeColor={getResourceColor(
                      stats.system_resources.cpu_usage
                    )}
                    size="small"
                  />
                </div>
                <div>
                  ...
                  <Progress
                    ...
                    strokeColor={getResourceColor(
                      stats.system_resources.memory_usage
                    )}
                    size="small"
                  />
                </div>
              ...
              <Col span={12}>
                <div className="mb-2">
                  <div className="flex justify-between mb-1">
                    <span>I/O</span>
                    <span>
                      {stats.system_resources.io_utilization.toFixed(1)}%
                    </span>
                  </div>
                  <Progress
                    percent={stats.system_resources.io_utilization}
                    strokeColor={getResourceColor(
                      stats.system_resources.io_utilization
                    )}
                    size="small"
                  />
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <span>网络</span>
                    <span>
                      {stats.system_resources.network_usage.toFixed(1)}%
                    </span>
                  </div>
                  <Progress
                    percent={stats.system_resources.network_usage}
                    strokeColor={getResourceColor(
                      stats.system_resources.network_usage
                    )}
                    size="small"
                  />
                </div>
          ...
          <Card title="预测性调度建议" size="small">
            <Space direction="vertical" style={{ width: '100%' }}>
              <div className="flex items-center space-x-2">
                {getScalingActionIcon(
                  stats.predictive_scheduling.scaling_recommendations.action
                )}
                <span className="font-medium">
                  {stats.predictive_scheduling.scaling_recommendations
                    .action === 'scale_up' && '建议扩容'}
                  {stats.predictive_scheduling.scaling_recommendations
                    .action === 'scale_down' && '建议缩容'}
                  {stats.predictive_scheduling.scaling_recommendations
                    .action === 'maintain' && '保持当前规模'}
                </span>
              </div>
              ...
              <Alert
                message={
                  stats.predictive_scheduling.scaling_recommendations.reason
                }
                type="info"
                showIcon
              />
