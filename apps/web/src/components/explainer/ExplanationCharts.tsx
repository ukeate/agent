...
const ExplanationCharts: React.FC<ExplanationChartsProps> = ({
  ...
  const [selectedChart, setSelectedChart] = useState<
    'factors' | 'tree' | 'confidence' | 'timeseries' | 'correlation'
  >('factors')
  ...
  const treeDepth = treeNodes.length
  ...
  const treeAvgConfidence = treeNodes.length
    ? treeNodes.reduce((sum, node) => sum + node.confidence, 0) /
      treeNodes.length
    : 0
  ...
  const confidenceAvgCalibration = confidenceDistribution.length
    ? confidenceDistribution.reduce(
        (sum, point) => sum + point.calibration_score,
        0
      ) / confidenceDistribution.length
    : 0
  const confidenceTotalSamples = confidenceDistribution.reduce(
    (sum, point) => sum + point.count,
    0
  )
  const confidenceMaxError = confidenceDistribution.length
    ? Math.max(
        ...confidenceDistribution.map(p =>
          Math.abs(p.accuracy - (p.bin_start + p.bin_end) / 2)
        )
      )
    : 0

  const renderDecisionTreeNode = (
                    ...
                    .map((factor, index) => (
                      <div key={factor.factor_name} className="space-y-2">
                        ...
                        <div className="relative">
                          <Progress
                            value={factor.importance_score * 100}
                            className="h-3"
                          />
                ...
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {(treeAvgConfidence * 100).toFixed(1)}%
                      </div>
                    </div>
                    <div>
                      <span className="text-blue-600">样本覆盖:</span>
                      <div className="font-medium">
                        {decisionTree.samples} 条
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        >
                          <div className="flex items-center space-x-4">
                            <span className="font-mono">
                              [{(bin.bin_start * 100).toFixed(0)}%,{' '}
                              {(bin.bin_end * 100).toFixed(0)}%]
                            </span>
                            <span>样本数: {bin.count}</span>
                          </div>
                          <div className="flex items-center space-x-4">
                            <span>
                              准确率: {(bin.accuracy * 100).toFixed(1)}%
                            </span>
                            <span>
                              校准分数: {bin.calibration_score.toFixed(3)}
                            </span>
                          </div>
                        </div>
                      ))}
                  ...
                  {timeSeriesData.slice(-20).map((item, index) => (
                    ...
                    >
                      ...
                      <span className="text-gray-700">
                        置信度 {(item.confidence * 100).toFixed(1)}% · 准确率{' '}
              ...
              {correlationMatrix.length > 0 ? (
                <div className="space-y-2">
                  {correlationMatrix.slice(0, 50).map((item, index) => (
                    <div
                      key={`${item.factor1}-${item.factor2}-${index}`}
                      className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded"
                    >
                      <span className="text-gray-700">
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <div className="text-sm text-blue-700 space-y-1">
                <p>
                  • <strong>因素重要性</strong>
                  ：基于SHAP值量化各特征对预测结果的贡献
                </p>
                <p>
                  • <strong>决策树</strong>：可视化模型的决策路径和分支逻辑
                </p>
                <p>
                  • <strong>置信度校准</strong>
                  ：评估预测置信度与实际准确率的匹配程度
                ...
                <p>
                  • <strong>时间序列</strong>：监控模型性能指标的时间变化趋势
                </p>
                <p>
                  • <strong>相关性矩阵</strong>：分析特征间的线性和非线性关系
                </p>
              </div>
            </div>
          </div>
