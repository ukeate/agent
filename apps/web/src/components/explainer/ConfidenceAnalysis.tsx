...
const ConfidenceAnalysis: React.FC<ConfidenceAnalysisProps> = ({
  ...
  const [selectedView, setSelectedView] = useState<
    'overview' | 'breakdown' | 'uncertainty' | 'calibration'
  >('overview')
  ...
  const getConfidenceLevel = (confidence: number) => {
    ...
    if (confidence >= 0.8)
      return { level: '高', color: 'text-green-600', bgColor: 'bg-green-100' }
    if (confidence >= 0.7)
      return {
        level: '中等偏高',
        color: 'text-blue-600',
  ...
  const getUncertaintyLevel = (uncertainty: number) => {
    ...
    if (uncertainty <= 0.4)
      return { level: '中等偏高', color: 'text-orange-600' }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              <div className="relative w-24 h-24 mx-auto mb-3">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span
                    className={`text-xl font-bold ${confidenceLevel.color}`}
                  >
              ...
              <div>
                ...
                <Badge
                  className={`${confidenceLevel.bgColor} ${confidenceLevel.color} mt-1`}
                >
            ...
            <div className="text-center">
              <div className="relative w-24 h-24 mx-auto mb-3">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span
                    className={`text-xl font-bold ${uncertaintyLevel.color}`}
                  >
              ...
              <div>
                ...
                <Badge
                  className={`${uncertaintyLevel.color} mt-1`}
                  variant="outline"
                >
            ...
            <div className="text-center">
              <div className="mb-3">
                <div className="text-2xl font-bold text-gray-700">
                  [{((metrics.confidence_interval_lower || 0) * 100).toFixed(0)}
                  %,{' '}
                  {((metrics.confidence_interval_upper || 1) * 100).toFixed(0)}
                  %]
                </div>
              </div>
              <div>
                ...
                <div className="mt-2">
                  ...
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                    <span>
                      {((metrics.confidence_interval_lower || 0) * 100).toFixed(
                        1
                      )}
                      %
                    </span>
                    <span>
                      {((metrics.confidence_interval_upper || 1) * 100).toFixed(
                        1
                      )}
                      %
                    </span>
      ...
      >
                ...
                <div className={`p-4 rounded-lg ${confidenceLevel.bgColor}`}>
                  <div className="flex items-center space-x-2 mb-2">
                    {metrics.overall_confidence >= 0.7 ? (
                      <CheckCircle
                        className={`h-5 w-5 ${confidenceLevel.color}`}
                      />
                    ) : (
                      <AlertTriangle
                        className={`h-5 w-5 ${confidenceLevel.color}`}
                    ...
                    <span className={`font-semibold ${confidenceLevel.color}`}>
                      ...
                      {(metrics.overall_confidence * 100).toFixed(1)}%)
                    </span>
                  </div>
                  <p className="text-sm text-gray-700">
                    ...
                    {metrics.overall_confidence >= 0.7 &&
                      metrics.overall_confidence < 0.8 &&
                      '决策可信度良好，建议谨慎采用并收集更多证据。'}
                    {metrics.overall_confidence >= 0.6 &&
                      metrics.overall_confidence < 0.7 &&
                      '决策可信度中等，建议进一步验证或获取专家意见。'}
                    {metrics.overall_confidence < 0.6 &&
                      '决策可信度较低，建议重新评估或收集更多高质量数据。'}
                  </p>
                </div>
                    ...
                    <div className="space-y-2 text-sm">
                      {metrics.prediction_confidence && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {(metrics.prediction_confidence * 100).toFixed(1)}%
                          </span>
                        </div>
                      )}
                      {metrics.evidence_confidence && (
                        <div className="flex justify-between">
                          <span>证据置信度:</span>
                          <span className="font-medium">
                            {(metrics.evidence_confidence * 100).toFixed(1)}%
                          </span>
                        </div>
                      )}
                      {metrics.model_confidence && (
                        <div className="flex justify-between">
                          <span>模型置信度:</span>
                          <span className="font-medium">
                            {(metrics.model_confidence * 100).toFixed(1)}%
                          </span>
                        </div>
                      )}
                      {metrics.variance && (
                  ...
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-gray-500">
                        <span>
                          可靠性: {(source.reliability * 100).toFixed(0)}%
                        </span>
                        <span>
                          加权贡献:{' '}
                          {(source.score * source.weight * 100).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <TabsContent value="uncertainty" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {Object.entries(defaultUncertaintyFactors).map(
                  ([factor, value]) => (
                    <div key={factor} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`font-semibold ${getUncertaintyLevel(value).color}`}
                        >
                          {(value * 100).toFixed(1)}%
                        </span>
                      </div>
                      ...
                      <div className="text-xs text-gray-600">
                        ...
                        {factor === 'model_complexity' &&
                          '模型复杂度带来的不确定性'}
                        {factor === 'feature_reliability' &&
                          '特征变量的可靠性评估'}
                        {factor === 'temporal_distance' &&
                          '时间距离对预测的影响'}
                        {factor === 'context_similarity' &&
                          '上下文匹配度的影响'}
                ...
                {metrics.calibration_score && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div
                        className={`text-2xl font-bold ${getConfidenceLevel(metrics.calibration_score).color}`}
                      >
                        {(metrics.calibration_score * 100).toFixed(1)}%
                      </div>
                      <Progress
                        value={metrics.calibration_score * 100}
                        className="h-2 mt-2"
                      />
                    </div>
                    <div>
