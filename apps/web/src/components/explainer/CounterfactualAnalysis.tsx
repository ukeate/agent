...
const CounterfactualAnalysis: React.FC<CounterfactualAnalysisProps> = ({
  ...
  const [selectedView, setSelectedView] = useState<
    'scenarios' | 'sensitivity' | 'comparison' | 'simulation'
  >('scenarios')
  ...
  // 模拟敏感性数据
  const defaultSensitivities: FactorSensitivity[] =
    factorSensitivities.length > 0
      ? factorSensitivities
      : [
          {
            factor_name: '信用评分',
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-500">
                置信度: {(baselineConfidence * 100).toFixed(1)}%
              </div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <div
                className={`text-lg font-semibold ${getImpactColor(Math.max(...scenarios.map(s => Math.abs(s.impact_difference))))}`}
              >
                {(
                  Math.max(
                    ...scenarios.map(s => Math.abs(s.impact_difference))
                  ) * 100
                ).toFixed(1)}
                %
              </div>
            ...
            {scenarios.map((scenario, index) => {
              ...
              return (
                <Card
                  ...
                  onClick={() =>
                    setSelectedScenario(
                      isSelected ? null : scenario.scenario_id
                    )
                  }
                >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            <Badge
                              className={getRiskColor(scenario.risk_level)}
                              variant="outline"
                            >
                      ...
                      <div className="text-right">
                        <div className="flex items-center space-x-1">
                          ...
                          <span
                            className={`font-semibold ${getImpactColor(scenario.impact_difference)}`}
                          >
                            {scenario.impact_difference > 0 ? '+' : ''}
                            {(scenario.impact_difference * 100).toFixed(1)}%
                          </span>
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {Object.entries(scenario.changed_factors).map(
                              ([factor, value]) => (
                                <div
                                  key={factor}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <div
                              className={`text-lg font-semibold ${getImpactColor(scenario.confidence_change)}`}
                            >
                              {scenario.confidence_change > 0 ? '+' : ''}
                              {(scenario.confidence_change * 100).toFixed(1)}%
                            </div>
                            <Progress
                            ...
                            <div className="grid grid-cols-2 gap-4 text-xs">
                              {scenario.metadata.computation_time && (
                                <div>
                                  ...
                                  <div className="font-medium">
                                    {scenario.metadata.computation_time}ms
                                  </div>
                                </div>
                              )}
                              {scenario.metadata.model_confidence && (
                                <div>
                                  ...
                                  <div className="font-medium">
                                    {(
                                      scenario.metadata.model_confidence * 100
                                    ).toFixed(1)}
                                    %
                                  </div>
                                </div>
                              ...
                              {scenario.metadata.sensitivity_score && (
                                <div>
                                  ...
                                  <div className="font-medium">
                                    {scenario.metadata.sensitivity_score.toFixed(
                                      3
                                    )}
                                  </div>
                                </div>
                              ...
                              {scenario.metadata.plausibility && (
                                <div>
                                  ...
                                  <div className="font-medium">
                                    {(
                                      scenario.metadata.plausibility * 100
                                    ).toFixed(1)}
                                    %
                                  </div>
                                </div>
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="text-sm">
                          ...
                          <span className="font-medium">
                            {formatFactorValue(sensitivity.original_value)}
                          </span>
                        </div>
                        <div className="text-sm">
                          <span className="text-gray-600">弹性系数: </span>
                          <span className="font-medium">
                            {sensitivity.elasticity.toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded mb-3">
                      <div className="flex justify-between items-end h-32 space-x-1">
                        {sensitivity.impact_curve.map((point, i) => {
                          ...
                          const isOriginal =
                            point.value === sensitivity.original_value
                          const isCritical =
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-3 text-sm">
                      <div className="text-center p-2 bg-blue-50 rounded">
                        <div className="text-blue-600 font-medium">
                          {sensitivity.sensitivity_range[0]} -{' '}
                          {sensitivity.sensitivity_range[1]}
                        </div>
                      ...
                      <div className="text-center p-2 bg-green-50 rounded">
                        <div className="text-green-600 font-medium">
                          {Math.max(
                            ...sensitivity.impact_curve.map(p =>
                              Math.abs(p.impact)
                            )
                          ).toFixed(2)}
                        </div>
                      ...
                      <div className="text-center p-2 bg-purple-50 rounded">
                        <div className="text-purple-600 font-medium">
                          {sensitivity.elasticity > 0.7
                            ? '高'
                            : sensitivity.elasticity > 0.4
                              ? '中'
                              : '低'}
                        </div>
                ...
                <table className="w-full border-collapse border border-gray-200">
                  ...
                  <tbody>
                    {scenarios
                      .sort(
                        (a, b) =>
                          Math.abs(b.impact_difference) -
                          Math.abs(a.impact_difference)
                      )
                      .map((scenario, index) => {
                              ...
                              <div className="flex items-center justify-center space-x-1">
                                ...
                                <span
                                  className={getImpactColor(
                                    scenario.impact_difference
                                  )}
                                >
                                  {(
                                    Math.abs(scenario.impact_difference) * 100
                                  ).toFixed(1)}
                                  %
                                </span>
                              </div>
                    ...
                    <div className="border rounded p-4 bg-gray-50">
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between">
                          <span>置信度:</span>
                          <span className="font-medium">
                            {(baselineConfidence * 100).toFixed(1)}%
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>风险等级:</span>
