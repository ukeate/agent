...
const RagResultsList: React.FC<RagResultsListProps> = ({
    ...
    switch (sortType) {
      ...
      case 'time':
        return sorted.sort((a, b) => {
          const timeA =
            a.metadata?.updated_at || a.metadata?.created_at || '1970-01-01'
          const timeB =
            b.metadata?.updated_at || b.metadata?.created_at || '1970-01-01'
  ...
  const highlightContent = useCallback(
    (content: string, searchQuery: string) => {
  ...
  const truncateContent = useCallback(
    (content: string, maxLength: number = 200) => {
      ...
      return (
        ...
        >
          ...
          <Space size="small" wrap>
            {item.content_type && <Tag color="blue">{item.content_type}</Tag>}

            {item.metadata?.language && <Tag>{item.metadata.language}</Tag>}

            {item.metadata?.size && (
              <Tag>{formatFileSize(item.metadata.size)}</Tag>
            )}
      ...
      {results && results.length > 0 && (
        <Card size="small" style={{ marginBottom: 16 }}>
          <Row align="middle" justify="space-between">
            <Col>
              <Space>
                <Text strong>找到 {results.length} 个结果</Text>
                {query && <Text type="secondary">for "{query}"</Text>}
              </Space>
            </Col>
              ...
              <Row gutter={[16, 8]}>
                ...
                {selectedItem.content_type && (
                  <>
                    ...
                    <Col span={16}>
                      <Tag color="blue">{selectedItem.content_type}</Tag>
                    </Col>
                  </>
                )}
              </Row>
