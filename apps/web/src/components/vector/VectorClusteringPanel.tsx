...
const VectorClusteringPanel: React.FC = () => {
  ...
  const [anomalyMethod, setAnomalyMethod] = useState<
  ...
  const [clusteringStats, setClusteringStats] =
    useState<ClusteringStats | null>(null)
  ...
  const handleClustering = async () => {
    ...
    try {
      ...
      setClusteringStats({
        ...
        silhouette_score: Math.max(
          0,
          1 - query_performance.average_execution_time_ms / 1000
        ),
        calinski_harabasz_score: query_performance.total_queries,
        davies_bouldin_score: Math.max(
          0,
          query_performance.max_execution_time_ms -
            query_performance.min_execution_time_ms
        ),
    ...
    <Card title="聚类配置" size="small">
      <Form form={form} layout="vertical">
        <Form.Item label="聚类算法">
          ...
          >
            <Radio value="kmeans">
              <Tooltip title="K-means聚类，适用于球形聚类">K-means</Tooltip>
            </Radio>
            <Radio value="dbscan">
              <Tooltip title="DBSCAN密度聚类，适用于任意形状">DBSCAN</Tooltip>
            </Radio>
            <Radio value="hierarchical">
              <Tooltip title="层次聚类，生成聚类树">层次聚类</Tooltip>
            </Radio>
          </Radio.Group>
        </Form.Item>
        ...
        <Row gutter={[16, 16]}>
          <Col span={12}>
            <Statistic
              ...
              valueStyle={{
                color:
                  clusteringStats.silhouette_score > 0.5
                    ? '#3f8600'
                    : '#faad14',
              }}
            />
            <Progress
              ...
              size="small"
              strokeColor={
                clusteringStats.silhouette_score > 0.5 ? '#52c41a' : '#faad14'
              }
            />
          </Col>
          <Col span={12}>
          ...
          <Col span={12}>
            <Text type="secondary">
              DB指数: {clusteringStats.davies_bouldin_score.toFixed(3)}
            </Text>
          </Col>
          <Col span={12}>
            <Text type="secondary">
              异常比例: {(clusteringStats.anomaly_ratio * 100).toFixed(1)}%
            </Text>
          </Col>
        </Row>
      </Card>
  ...
  const clusterColumns = [
    ...
    },
    {
      ...
      dataIndex: 'inertia',
      key: 'inertia',
      render: (inertia: number) => <Text>{inertia.toFixed(2)}</Text>,
    },
    {
      title: '质量',
      key: 'quality',
      render: (record: ClusterResult) => {
        ...
        return (
          <Progress
            ...
            size="small"
            strokeColor={
              quality > 80 ? '#52c41a' : quality > 60 ? '#1890ff' : '#faad14'
            }
          />
        )
      },
