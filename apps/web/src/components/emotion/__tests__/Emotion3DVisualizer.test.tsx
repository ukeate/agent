...
describe('Emotion3DVisualizer Utils', () => {
  ...
  it('validates timestamp ordering in history', () => {
    const sortedHistory = [...mockHistory].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    
    expect(sortedHistory[0].timestamp.getTime()).toBeLessThan(sortedHistory[1].timestamp.getTime());
  });
});