...
const PerformanceMonitorPanel: React.FC = () => {
      ...
      setMetrics([
        {
          ...
          p95_latency_ms:
            qp.p95_execution_time_ms || qp.max_execution_time_ms || 0,
          p99_latency_ms:
            qp.p99_execution_time_ms || qp.max_execution_time_ms || 0,
  ...
  const formatTargetValue = (value: number, record: PerformanceTarget) => {
    if (
      record.unit === 'ratio' ||
      record.metric.includes('率') ||
      record.metric.includes('命中')
    ) {
  ...
  const targetColumns = [
    ...
    },
    {
      ...
      render: (value: number, record: PerformanceTarget) =>
        formatTargetValue(value, record),
    },
    {
      title: '目标值',
      dataIndex: 'target',
  ...
  const achievementRate = totalTargets
    ? Math.round((achievedCount / totalTargets) * 100)
    : 0
  ...
  return (
    <div>
        ...
        <Col span={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{
                color:
                  (latestMetrics?.avg_latency_ms || 0) < 50
                    ? '#3f8600'
                    : '#cf1322',
              }}
            />
        ...
        <Col span={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{
                color:
                  (latestMetrics?.p95_latency_ms || 0) < 100
                    ? '#3f8600'
                    : '#cf1322',
              }}
            />
        ...
        <Col span={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{
                color:
                  (latestMetrics?.cache_hit_rate || 0) > 0.7
                    ? '#3f8600'
                    : '#faad14',
              }}
            />
      ...
      <Card title="性能目标达成情况" extra={<TrophyOutlined />}>
        <Table
          dataSource={targets.map((target, index) => ({
            ...target,
            key: index,
          }))}
          ...
          <Col span={16}>
            <Alert
              ...
              description={
                <ul>
                  <li>
                    当前平均延迟: {latestMetrics?.avg_latency_ms?.toFixed(1)}ms
                    {avgTarget
                      ? `，目标: <${avgTarget.target.toFixed(1)}ms`
                      : ''}
                  </li>
                  <li>
                    缓存命中率:{' '}
                    {((latestMetrics?.cache_hit_rate || 0) * 100).toFixed(1)}%
                  </li>
                  <li>
                    量化使用率:{' '}
                    {((latestMetrics?.quantization_ratio || 0) * 100).toFixed(
                      1
                    )}
                    %
                  </li>
