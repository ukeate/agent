  ...
  const resultColumns = [
    {
      ...
      render: (mode: string) => (
        <Tag
          color={mode === 'int4' ? 'red' : mode === 'int8' ? 'orange' : 'green'}
        >
          {mode.toUpperCase()}
        </Tag>
      ),
        ...
        <Col span={12}>
          <Card title="量化配置" extra={<ThunderboltOutlined />}>
            <Form layout="vertical">
              ...
              <Form.Item
                label={`精度阈值: ${(config.precision_threshold * 100).toFixed(0)}%`}
              >
                <Slider
                  min={0.8}
                  max={0.99}
                  ...
                  onChange={value =>
                    setConfig({ ...config, precision_threshold: value })
                  }
                  marks={{
                    0.8: '80%',
                    0.9: '90%',
                ...
                />
                <Text type="secondary">自适应模式下的最小精度要求</Text>
              </Form.Item>
              ...
              <Form.Item label="启用动态量化">
                <Switch
                  ...
                  onChange={checked =>
                    setConfig({ ...config, enable_dynamic: checked })
                  }
                />
                <div style={{ marginTop: 8 }}>
                  <Text type="secondary">根据向量特征动态选择量化策略</Text>
                </div>
              </Form.Item>
            ...
            {testResults.length > 0 && (
              <div style={{ marginTop: 24 }}>
                ...
                {testResults.map((result, index) => {
                  const savingPercent =
                    ((result.original_size - result.quantized_size) /
                  ...
                  return (
                    <div key={index} style={{ marginBottom: 12 }}>
                      ...
                      <Progress
                        ...
                        strokeColor={getCompressionColor(
