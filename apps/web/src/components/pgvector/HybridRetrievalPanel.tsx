...
const HybridRetrievalPanel: React.FC = () => {
  ...
  const resultColumns = [
    ...
    },
    {
      ...
      sorter: (a: SearchResult, b: SearchResult) =>
    ...
    },
    {
      ...
      render: (sources: string[]) => (
        <Tag color={getSourceColor(sources)}>{getSourceText(sources)}</Tag>
      ),
    },
  ]
    ...
    },
    {
      ...
      render: (rate: number) => (
        <Progress
          ...
          strokeColor={
            rate > 0.9 ? '#52c41a' : rate > 0.7 ? '#faad14' : '#f5222d'
          }
        />
      ),
    },
      ...
      render: (score: number | null | undefined) => {
        ...
        return (
          <span
            style={{
              color:
                score > 0.8 ? '#52c41a' : score > 0.6 ? '#faad14' : '#f5222d',
            }}
          >
            {score.toFixed(3)}
          </span>
              ...
              <Card title="搜索查询" extra={<SearchOutlined />}>
                  ...
                  <Col span={8}>
                    <Form.Item label={`返回结果: ${searchConfig.top_k}`}>
                      <Slider
                        ...
                        onChange={value =>
                          setSearchConfig({ ...searchConfig, top_k: value })
                        }
                      />
                    </Form.Item>
                  </Col>
                ...
                {searchConfig.search_mode === 'hybrid' && (
                  <Row gutter={16}>
                    <Col span={12}>
                      <Form.Item
                        label={`pgvector权重: ${searchConfig.pg_weight}`}
                      >
                        <Slider
                          min={0}
                          max={1}
                    ...
                    <Col span={12}>
                      <Form.Item
                        label={`Qdrant权重: ${searchConfig.qdrant_weight}`}
                      >
                        <Slider
                          min={0}
                          max={1}
              ...
              <Card title="搜索指标" extra={<BarChartOutlined />}>
                {searchMetrics ? (
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    <div>
                      <Text type="secondary">
                        pgvector: {searchMetrics.pg_time_ms?.toFixed(1) || 0}ms
                      </Text>
                    </div>
                    <div>
                      <Text type="secondary">
                        Qdrant: {searchMetrics.qdrant_time_ms?.toFixed(1) || 0}
                        ms
                      </Text>
                    </div>
                    <div>
                      <Text type="secondary">
                        融合: {searchMetrics.fusion_time_ms?.toFixed(1) || 0}ms
                      </Text>
                    </div>
        ...
        >
          <Card title="检索方法性能对比" extra={<LineChartOutlined />}>
            ...
            {benchmarkResults.length > 0 && (
              <>
                <Table
                  dataSource={benchmarkResults.map((result, index) => ({
                    ...result,
                    key: index,
                  }))}
