...
const QLearningTrainingPanel: React.FC<QLearningTrainingPanelProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'running': return <LoadingOutlined style={{ color: '#52c41a' }} />
        ...
        return (
          <div>
            <Progress 
              ...
              format={(percent) => `${record.current_episode}/${record.total_episodes}`}
            />
        ...
        <Row gutter={16}>
          ...
          <Col span={6}>
            <Card>
              <Statistic
                title="总训练时长"
                value={trainingSessions.reduce((sum, s) => sum + (s.elapsed_time || 0), 0)}
            ...
            <Card>
              <Statistic
                ...
                value={completedSessions.length > 0 
                  ? (completedSessions.filter(s => s.convergence_achieved).length / completedSessions.length * 100)
                  : 0
                }
                precision={1}
                suffix="%"
          ...
          <Card title={
            ...
            <Row gutter={16}>
              {runningSessions.map(session => {
                ...
                const progress = (session.current_episode / session.total_episodes) * 100
                    ...
                    <Card size="small" hoverable>
                      <Space direction="vertical" style={{ width: '100%' }}>
                        ...
                        <Progress 
                          percent={Math.round(progress)}
                          strokeColor="#52c41a"
                          format={() => `${session.current_episode}/${session.total_episodes}`}
                        />
          ...
          {trainingSessions.length === 0 ? (
            ...
            <Table
              ...
              onRow={(record) => ({
                ...
                style: {
                  backgroundColor: selectedSession === record.session_id ? '#f0f8ff' : undefined,
                  cursor: 'pointer'
                }
              })}
            />
