...
export const AgentMetrics: React.FC = () => {
  const { agentMetrics, status, loading, loadMetrics, loadStatus } =
    useSupervisorStore()
  ...
  if (!agentMetrics || agentMetrics.length === 0) {
    return (
      <div className="text-center py-12 text-gray-500">暂无智能体指标数据</div>
    )
  }
  ...
  return (
    <div className="agent-metrics space-y-6">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {agentMetrics && agentMetrics.length > 0
                  ? formatPercentage(
                      (agentMetrics || []).reduce(
                        (sum, m) => sum + m.availability_score,
                        0
                      ) / agentMetrics.length
                    )
                  : '0.0%'}
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex space-x-3">
                  <span
                    className={`px-3 py-1 text-sm font-medium rounded-full ${getLoadColor(currentLoad)}`}
                  >
                    当前负载: {formatPercentage(currentLoad)}
                  </span>
                  {metric && (
                    <span
                      className={`px-3 py-1 text-sm font-medium rounded-full ${getAvailabilityColor(metric.availability_score)}`}
                    >
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="text-lg font-semibold text-gray-900">
                      {metric.average_task_time
                        ? formatDuration(metric.average_task_time)
                        : '-'}
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="text-lg font-semibold text-blue-600">
                      {metric.response_time_avg
                        ? formatDuration(metric.response_time_avg)
                        : '-'}
                    </div>
                  </div>
              ...
              {metric && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="flex justify-between items-center text-sm text-gray-600">
                    <div>
                      统计窗口: {formatDate(metric.window_start)} -{' '}
                      {formatDate(metric.window_end)}
                    </div>
                    <div>最后更新: {formatDate(metric.updated_at)}</div>
                  </div>
                </div>
              )}
            </div>
      ...
      {agentMetrics &&
        ...
        agentMetrics.some(m => m.success_rate !== undefined) && (
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              智能体性能对比
            </h3>
                ...
                <div key={metric.id} className="border rounded-lg p-4">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="text-sm text-gray-500">
                      {formatDate(metric.window_start)} -{' '}
                      {formatDate(metric.window_end)}
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div
                        className={`font-medium ${
                          (metric.success_rate || 0) >= 0.9
                            ? 'text-green-600'
                            : (metric.success_rate || 0) >= 0.7
                              ? 'text-yellow-600'
                              : 'text-red-600'
                        }`}
                      >
