...
export const ReasoningHistory: React.FC = () => {
  ...
  const [selectedChain, setSelectedChain] = useState<ReasoningChain | null>(
    null
  )
  ...
  const applyFilters = () => {
    ...
    if (filters.searchText) {
      ...
      filtered = filtered.filter(
        chain =>
          ...
          (chain.conclusion &&
            chain.conclusion.toLowerCase().includes(searchLower))
      )
    }
    ...
    if (filters.confidence) {
      ...
      filtered = filtered.filter(chain => {
        ...
        return (
          chain.confidence_score >= min / 100 &&
          chain.confidence_score <= max / 100
        )
      })
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (confidence: number) => {
        ...
        const color =
          percentage >= 80 ? 'green' : percentage >= 60 ? 'orange' : 'red'
  ...
  const getStatistics = () => {
    ...
    const avgConfidence =
      filteredData.reduce((sum, c) => sum + (c.confidence_score || 0), 0) /
        total || 0
    const totalSteps = filteredData.reduce(
              ...
              <Timeline>
                {selectedChain.steps?.map((step, index) => (
                  ...
                  >
                    <div>
                      <Text strong>
                        步骤 {step.step_number} - {step.step_type}
                      </Text>
