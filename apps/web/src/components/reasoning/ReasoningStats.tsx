...
export const ReasoningStats: React.FC = () => {
  ...
  const loadData = async () => {
    await Promise.all([getReasoningHistory(), getReasoningStats()])
  }
  ...
  const calculateMetrics = () => {
    ...
    if (selectedStrategy !== 'all') {
      filteredChains = filteredChains.filter(
        chain => chain.strategy === selectedStrategy
      )
    }
    ...
    const completionRate =
      totalChains > 0 ? (completedChains / totalChains) * 100 : 0
    ...
    const totalSteps = filteredChains.reduce(
      (sum, c) => sum + (c.steps?.length || 0),
      0
    )
    ...
    const strategyDistribution = Array.from(strategyMap.entries()).map(
      ([strategy, data]) => ({
        strategy,
        count: data.count,
        avgConfidence:
          data.confidences.length > 0
            ? data.confidences.reduce((sum: number, c: number) => sum + c, 0) /
    ...
    const stepTypeDistribution = Array.from(stepTypeMap.entries()).map(
      ([type, data]) => ({
        type,
        count: data.count,
        avgConfidence:
    ...
    const mediumConfidence = confidences.filter(
      c => c >= 0.6 && c <= 0.8
    ).length
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      key: 'percentage',
                      render: (_, record) => {
                        const total = metrics.stepTypeDistribution.reduce(
