...
const ExperimentConfigForm: React.FC<ExperimentConfigFormProps> = ({
  ...
  const updateConfig = useCallback(
    (field: keyof ExperimentConfig, value: any) => {
  ...
  const updateVariant = useCallback(
    (id: string, field: keyof Variant, value: any) => {
  ...
  const deleteVariant = useCallback(
    (id: string) => {
      updateConfig(
        'variants',
        config.variants.filter(v => v.id !== id)
      )
    },
    [config.variants, updateConfig]
  )
  ...
  const updateMetric = useCallback(
    (id: string, field: keyof Metric, value: any) => {
  ...
  const deleteMetric = useCallback(
    (id: string) => {
      updateConfig(
        'metrics',
        config.metrics.filter(m => m.id !== id)
      )
    },
    [config.metrics, updateConfig]
  )
  ...
  // 更新定向规则
  const updateTargetingRule = useCallback(
    (id: string, field: keyof TargetingRule, value: any) => {
  ...
  // 删除定向规则
  const deleteTargetingRule = useCallback(
    (id: string) => {
      updateConfig(
        'targetingRules',
        config.targetingRules.filter(r => r.id !== id)
      )
    },
    [config.targetingRules, updateConfig]
  )
          ...
          <Grid container spacing={3}>
            ...
            <Grid {...({ item: true, xs: 12, md: 6 } as any)}>
              <TextField
                ...
                onChange={e =>
                  updateConfig('startDate', new Date(e.target.value))
                }
          ...
          <Stack spacing={2}>
            ...
            {!isReadOnly && (
              <Stack direction="row" spacing={2}>
                ...
                <Button onClick={autoDistributeTraffic} variant="outlined">
                  平均分配流量
                </Button>
              </Stack>
            )}
                    ...
                    <Grid {...({ item: true, xs: 12, md: 2 } as any)}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          value={metric.type}
                          onChange={e =>
                            updateMetric(metric.id, 'type', e.target.value)
                          }
                          disabled={isReadOnly}
                          label="类型"
                        >
                  ...
                  <Grid container spacing={2} alignItems="center">
                    <Grid {...({ item: true, xs: 12, md: 2 } as any)}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          value={rule.type}
                          onChange={e =>
                            updateTargetingRule(rule.id, 'type', e.target.value)
                          }
                          disabled={isReadOnly}
                          label="类型"
                        >
                    ...
                    <Grid {...({ item: true, xs: 12, md: 3 } as any)}>
                      <TextField
                        ...
                        value={rule.field}
                        onChange={e =>
                          updateTargetingRule(rule.id, 'field', e.target.value)
                        }
                        disabled={isReadOnly}
          ...
          <Grid container spacing={3}>
            <Grid {...({ item: true, xs: 12, md: 6 } as any)}>
              <Box sx={{ px: 2 }}>
                ...
                <Slider
                  ...
                  onChange={(_, value) =>
                    updateConfig('confidenceLevel', value as number)
                  }
                  disabled={isReadOnly}
                  valueLabelDisplay="auto"
                  min={80}
            ...
            <Grid {...({ item: true, xs: 12, md: 6 } as any)}>
              <TextField
                ...
                onChange={e =>
                  updateConfig('sampleSize', parseInt(e.target.value))
                }
                disabled={isReadOnly}
                helperText="留空表示自动计算"
              />
            ...
            <Grid {...({ item: true, xs: 12, md: 6 } as any)}>
              <TextField
                ...
                onChange={e =>
                  updateConfig('mutuallyExclusiveGroup', e.target.value)
                }
                disabled={isReadOnly}
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={e =>
                      updateConfig('enableDataQualityChecks', e.target.checked)
                    }
                    disabled={isReadOnly}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={e =>
                      updateConfig('enableAutoStop', e.target.checked)
                    }
                    disabled={isReadOnly}
                  />
                }
