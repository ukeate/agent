...
export const ExperimentDetail: React.FC<ExperimentDetailProps> = ({
  ...
  const successRate = experiment.total_trials > 0 
          ...
          <div className="space-y-4">
            ...
            {experiment.config.n_trials && (
              <div>
                <div className="flex justify-between text-sm text-gray-600 mb-2">
                  <span>完成进度</span>
                  <span>{experiment.total_trials}/{experiment.config.n_trials}</span>
                </div>
                <Progress 
                  value={(experiment.total_trials / experiment.config.n_trials) * 100} 
                  className="h-3" 
                />
              </div>
            )}
        ...
        {experiment.best_value !== undefined && (
          <Card className="p-6">
            ...
            <div className="space-y-3">
              <div>
                <span className="text-sm font-medium text-gray-500">最佳值</span>
              ...
              {experiment.best_params && (
                <div>
                  <span className="text-sm font-medium text-gray-500">最佳参数</span>
                  <div className="mt-2 space-y-1">
                    {Object.entries(experiment.best_params).map(([key, value]) => (
            ...
            {trials.slice().reverse().map((trial) => (
              <tr key={trial.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 whitespace-nowrap text-sm">
                  <span className={`font-medium ${trialStateColors[trial.state as keyof typeof trialStateColors] || 'text-gray-600'}`}>
                    {trial.state}
                  </span>
                </td>
  ...
  const tabsData = [
    ...
    { id: 'trials', label: `试验历史 (${trials.length})`, content: renderTrials() }
  ];
