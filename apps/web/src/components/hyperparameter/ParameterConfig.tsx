...
import { Form, Input, Select, InputNumber, Button, Space, Card, Row, Col, Checkbox, Modal, message, Empty, Statistic } from 'antd';
...
const ParameterConfig: React.FC<ParameterConfigProps> = ({
  ...
  const [parameters, setParameters] = useState<{ [key: string]: ParameterRange }>({});
  const [newParamName, setNewParamName] = useState('');
  const [newParamType, setNewParamType] = useState<'float' | 'int' | 'categorical'>('float');
  ...
  const handleAddParameter = () => {
    ...
    const newParam: ParameterRange = {
      ...
      ...(newParamType === 'categorical' ? { choices: ['option1', 'option2'] } : {})
    };
  ...
  const handleParameterChange = (key: string, field: string, value: any) => {
    ...
    if (field === 'low' || field === 'high') {
      ...
      if (param.low !== undefined && param.high !== undefined && param.low > param.high) {
    ...
    return (
      <div>
        <Empty description="暂无参数配置">
          <Space>
            <Button type="primary" icon={<PlusOutlined />} onClick={() => setNewParamName('new_param')}>
              添加参数
            </Button>
  ...
  const stats = {
    ...
    numeric: parameterKeys.filter(k => ['float', 'int'].includes(parameters[k].type)).length,
    categorical: parameterKeys.filter(k => parameters[k].type === 'categorical').length
  };
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={8}>
          <Statistic title="分类参数" value={`分类参数: ${stats.categorical}`} />
        </Col>
      </Row>
      ...
      {parameterKeys.map(key => {
        ...
        return (
          <Card key={key} title={key} style={{ marginBottom: 16 }}
                extra={<Button danger size="small" icon={<DeleteOutlined />} onClick={() => handleDeleteParameter(key)}>删除</Button>}>
            <Form layout="horizontal">
              <Form.Item label="参数类型">
                <Select value={param.type} onChange={(value) => handleParameterChange(key, 'type', value)}>
              ...
              {(param.type === 'float' || param.type === 'int') && (
                <>
                  <Form.Item label="最小值" validateStatus={errors[key] ? 'error' : ''} help={errors[key]}>
                    <InputNumber value={param.low} onChange={(value) => handleParameterChange(key, 'low', value)} />
                  </Form.Item>
                  <Form.Item label="最大值">
                    <InputNumber value={param.high} onChange={(value) => handleParameterChange(key, 'high', value)} />
                  </Form.Item>
                  {param.type === 'float' && (
                    <Form.Item>
                      <Checkbox checked={param.log} onChange={(e) => handleParameterChange(key, 'log', e.target.checked)}>
                        对数刻度
                      </Checkbox>
                    </Form.Item>
                  )}
                  {param.type === 'int' && (
                    <Form.Item label="步长">
                      <InputNumber value={param.step} onChange={(value) => handleParameterChange(key, 'step', value)} />
                    </Form.Item>
                  )}
                </>
              )}
              ...
              {param.type === 'categorical' && (
                <Form.Item label="选项">
                  <Input value={param.choices?.join(',')} 
                         onChange={(e) => handleParameterChange(key, 'choices', e.target.value.split(','))} />
                </Form.Item>
              )}
      ...
      {newParamName && (
        <Card title="新参数" style={{ marginTop: 16 }}>
          <Form layout="inline">
            <Form.Item label="参数名">
              <Input placeholder="参数名" value={newParamName} onChange={(e) => setNewParamName(e.target.value)} />
            </Form.Item>
            <Form.Item label="类型">
            ...
            <Form.Item>
              <Button type="primary" onClick={handleAddParameter}>确认添加</Button>
      ...
      >
        <pre style={{ background: '#f5f5f5', padding: 10, borderRadius: 4 }}>
          ...
          {JSON.stringify(parameters, null, 2).includes('learning_rate') && <span>"learning_rate"</span>}
        </pre>
      </Modal>
    </div>
  );
