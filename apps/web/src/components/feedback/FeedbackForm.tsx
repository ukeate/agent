...
export const FeedbackForm: React.FC<FeedbackFormProps> = ({
  ...
  // 验证评论内容
  const validateComment = useCallback((comment: string): boolean => {
    const errors: string[] = [];
  ...
  // 处理点赞/踩状态变化
  const handleLikeStateChange = useCallback((likeState: 'none' | 'like' | 'dislike') => {
  ...
  const getFeedbackSummary = () => {
    const summary = [];
    if (feedbackState.rating > 0) summary.push(`评分: ${feedbackState.rating}/5`);
    if (feedbackState.likeState !== 'none') summary.push(`态度: ${feedbackState.likeState === 'like' ? '喜欢' : '不喜欢'}`);
      ...
      <div className="flex flex-wrap items-center gap-4">
        {showRating && (
          <div className="flex flex-col space-y-1">
            {!compact && <label className="text-sm font-medium text-gray-700">评分</label>}
            <RatingComponent
              itemId={itemId}
              userId={userId}
              sessionId={sessionId}
              ...
              onSubmitSuccess={(rating) => onSubmitSuccess?.({ type: 'rating', rating, ...feedbackState })}
        ...
        {showLikeDislike && (
          <div className="flex flex-col space-y-1">
            {!compact && <label className="text-sm font-medium text-gray-700">喜好</label>}
            <LikeDislikeComponent
              ...
              onSubmitSuccess={(action) => onSubmitSuccess?.({ type: 'like', action, ...feedbackState })}
              onSubmitError={onSubmitError}
            />
          </div>
        )}
        ...
        {showBookmark && (
          <div className="flex flex-col space-y-1">
            {!compact && <label className="text-sm font-medium text-gray-700">收藏</label>}
            <BookmarkComponent
              itemId={itemId}
              userId={userId}
              sessionId={sessionId}
              ...
              onSubmitSuccess={(bookmarked) => onSubmitSuccess?.({ type: 'bookmark', bookmarked, ...feedbackState })}
              onSubmitError={onSubmitError}
            />
          </div>
        )}
