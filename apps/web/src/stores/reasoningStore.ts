    ...
    (set, get) => ({
      ...
      executeReasoning: async (request: ReasoningRequest) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : '推理执行失败'
          set({
            error: errorMessage,
            isExecuting: false,
          })
      ...
      streamReasoning: async (request: ReasoningRequest) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : '流式推理失败'
          set({
            error: errorMessage,
            isExecuting: false,
          })
      ...
      validateChain: async (chainId: string, stepNumber?: number) => {
        ...
        try {
          const validation = await reasoningService.validateChain(
            chainId,
            stepNumber
          )
          set({
      ...
      createBranch: async (
        ...
        try {
          const branchId = await reasoningService.createBranch(
            chainId,
            parentStepNumber,
            reason
          )
      ...
      getReasoningChain: async (chainId: string) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : '获取推理链失败'
          set({
            error: errorMessage,
            isLoading: false,
          })
      ...
      getReasoningHistory: async (limit = 20, offset = 0) => {
        ...
        try {
          const history = await reasoningService.getReasoningHistory(
            limit,
            offset
          )
          set({
      ...
      deleteReasoningChain: async (chainId: string) => {
        ...
        try {
          ...
          // 从历史中移除
          set(state => ({
            reasoningHistory: state.reasoningHistory.filter(
              chain => chain.id !== chainId
            ),
            isLoading: false,
            // 如果删除的是当前链，清空当前链
            currentChain:
              state.currentChain?.id === chainId ? null : state.currentChain,
          }))
        } catch (error) {
          const errorMessage =
