  ...
  devtools(
    (set, get) => ({
      ...
      loadStatus: async () => {
        ...
        try {
          const status =
            await supervisorApiService.getStatus(currentSupervisorId)
      ...
      loadTasks: async (page = 1, pageSize = 20) => {
        ...
        try {
          const result = await supervisorApiService.getTasks(
            currentSupervisorId,
            page,
            pageSize
          )
      ...
      loadDecisions: async (page = 1, pageSize = 20) => {
        ...
        try {
          const result = await supervisorApiService.getDecisionHistory(
            currentSupervisorId,
            page,
            pageSize
          )
      ...
      loadConfig: async () => {
        ...
        try {
          const config =
            await supervisorApiService.getConfig(currentSupervisorId)
      ...
      loadMetrics: async () => {
        ...
        try {
          const agentMetrics =
            await supervisorApiService.getAgentMetrics(currentSupervisorId)
          set({ agentMetrics, loading: { ...get().loading, metrics: false } })
        } catch (error) {
          set({
            error:
              error instanceof Error ? error.message : '加载智能体指标失败',
      ...
      submitTask: async (
        ...
        try {
          const result = await supervisorApiService.submitTask(
            currentSupervisorId,
            taskRequest
          )
      ...
      updateConfig: async (configUpdate: Partial<SupervisorConfig>) => {
        ...
        try {
          const config = await supervisorApiService.updateConfig(
            currentSupervisorId,
            configUpdate
          )
          set({ config, loading: { ...get().loading, config: false } })
