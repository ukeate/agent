...
interface RagState {
  ...
  retrievalProgress: {
    ...
    progress: number;
    message: string;
    stage: 'analysis' | 'expansion' | 'retrieval' | 'validation' | 'composition' | 'explanation' | 'complete';
  } | null;
...
interface RagActions {
  ...
  addToHistory: (query: string, type: 'basic' | 'agentic', results_count: number, processing_time: number) => void;
  clearHistory: () => void;
...
export const useRagStore = create<RagState & RagActions>()(
  persist(
    (set, get) => ({
      ...
      resetSearchPreferences: () => set({ searchPreferences: defaultSearchPreferences }),
