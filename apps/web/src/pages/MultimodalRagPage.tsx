...
import { Card, Row, Col, Tabs, message, Typography, Space, Statistic, Button, Input, Upload, Modal } from 'antd';
import {
...
import { multimodalRagService, MultimodalRagStats } from '../services/multimodalRagService';
...
const MultimodalRagPage: React.FC = () => {
  ...
  const [systemDetails, setSystemDetails] = useState<MultimodalRagStats | null>(null);
  ...
  const handleBatchUpload = async (files: File[]) => {
    ...
    try {
      ...
      message.success(`批量上传完成: ${response.successful || 0}/${files.length} 个文件`);
