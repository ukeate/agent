...
const TestCoveragePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await apiClient.get<CoverageReport>('/testing/coverage')
  ...
  const files = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => a.coverage_percentage - b.coverage_percentage)
  }, [report, filter])
  ...
  if (!report || report.total_files === 0) {
    return (
      <div className="p-6">
        ...
        <p className="text-gray-600">暂无覆盖率数据（未找到可用的 .coverage 文件）</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">数据来源：后端读取最新 .coverage 并计算文件级覆盖率</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <span className={`text-2xl font-bold ${getCoverageColor(report.overall_percentage)}`}>
      ...
      <div className="bg-white rounded-lg shadow p-4">
            ...
            <tbody>
              {files.slice(0, 200).map((f) => (
                <tr key={f.path} className="border-t">
                  ...
                  <td className={`py-2 pr-4 whitespace-nowrap ${getCoverageColor(f.coverage_percentage)}`}>
        ...
        {files.length > 200 && (
          <div className="text-xs text-gray-500 mt-3">仅显示前 200 条，可通过过滤缩小范围。</div>
        )}
      </div>
    </div>
  )
