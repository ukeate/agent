...
const ModelEvaluationOverviewPage: React.FC = () => {
  ...
  const loadOverviewData = async () => {
    try {
      ...
      const runningEvals = history.evaluations.filter(
      ...
      const avgAccuracy =
        validScores.length === 0
          ? 0
          : validScores.reduce((sum, e) => sum + e.score, 0) /
            validScores.length

      const cpuUsage = metrics?.system?.cpu_percent ?? 0
      ...
      setOverview({
        ...
        queuedTasks: history.evaluations.filter(e => e.status === 'queued')
          .length,
      })
      ...
      const evaluationsData: RecentEvaluation[] = history.evaluations
        .slice(0, 10)
        .map(evaluation => ({
          id: evaluation.id,
      ...
      items: [
        {
          label: 'CPU使用率',
          value:
            systemMetrics?.system?.cpu_percent != null
              ? `${systemMetrics.system.cpu_percent.toFixed(1)}%`
              : '-',
          color:
            systemMetrics?.system?.cpu_percent &&
            systemMetrics.system.cpu_percent > 80
              ? 'red'
              : 'green',
        },
        {
          label: '内存使用率',
          value:
            systemMetrics?.system?.memory_percent != null
              ? `${systemMetrics.system.memory_percent.toFixed(1)}%`
              : '-',
          color:
            systemMetrics?.system?.memory_percent &&
            systemMetrics.system.memory_percent > 80
              ? 'red'
              : 'green',
        },
        {
          label: '磁盘使用率',
          value:
            systemMetrics?.system?.disk_percent != null
              ? `${systemMetrics.system.disk_percent.toFixed(1)}%`
              : '-',
          color:
            systemMetrics?.system?.disk_percent &&
            systemMetrics.system.disk_percent > 85
              ? 'red'
              : 'green',
        },
        {
          label: '请求错误率',
          value:
            systemMetrics?.performance?.error_rate != null
              ? `${(systemMetrics.performance.error_rate * 100).toFixed(2)}%`
              : '-',
          color:
            systemMetrics?.performance?.error_rate &&
            systemMetrics.performance.error_rate > 0.05
              ? 'red'
              : 'green',
        },
      ],
      ...
      <Alert
        ...
        type={
          ...
          systemMetrics.performance.error_rate > 0.05
            ? 'error'
            : systemMetrics?.performance?.error_rate &&
                systemMetrics.performance.error_rate > 0.01
              ? 'warning'
              : 'success'
        }
        showIcon
          ...
          >
            <div style={{ marginBottom: '16px' }}>
              ...
              <Progress
                ...
                status={
                  overview.systemUtilization > 85 ? 'exception' : 'active'
                }
                strokeColor={{
                  from: '#108ee9',
                  to: '#87d068',
