...
const HybridSearchAdvancedPage: React.FC = () => {
  ...
  const normalizePerformanceMetrics = (metricsResp: any) => {
    ...
    const endTime =
      metricsResp.report_period?.end_time || new Date().toISOString()
    return [
      {
        timestamp: endTime,
  ...
  const loadOverview = async () => {
    ...
    try {
      setError(null)
      const [targetsResp, metricsResp, quantConfig, healthResp] =
        await Promise.all([
  ...
  const latestMetric = performanceMetrics.length
    ? performanceMetrics[performanceMetrics.length - 1]
    : null

  const formatTargetValue = (item: any) => {
    if (typeof item.current !== 'number') return '-'
    if (item.metric.includes('延迟'))
      return `${(item.current * 1000).toFixed(1)} ms`
    if (item.metric.includes('率') || item.metric.includes('使用率'))
      return `${(item.current * 100).toFixed(1)}%`
    return item.current
  }
  ...
  const performanceDisplay = latestMetric
    ? [
        {
          label: '平均查询延迟',
          value: `${latestMetric.avg_latency_ms.toFixed(1)} ms`,
        },
        {
          label: 'P95查询延迟',
          value: `${latestMetric.p95_latency_ms.toFixed(1)} ms`,
        },
        {
          label: '缓存命中率',
          value: `${(latestMetric.cache_hit_rate * 100).toFixed(1)}%`,
        },
        {
          label: '量化使用率',
          value: `${(latestMetric.quantization_ratio * 100).toFixed(1)}%`,
        },
      ]
  ...
  const systemFeatures = [
    ...
    },
    {
      ...
      description: latestMetric
        ? `最近查询 ${latestMetric.search_count || 0} 次`
        : '暂无性能数据',
    ...
    {
      title: '量化配置',
      description: quantizationConfig
        ? `模式: ${quantizationConfig.mode}`
        : '未配置',
    ...
    },
    {
      title: '缓存状态',
      description: latestMetric
        ? `命中率 ${(latestMetric.cache_hit_rate * 100).toFixed(1)}%`
        : '暂无数据',
      icon: <ApiOutlined />,
      status: latestMetric ? 'normal' : 'unknown',
    },
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <Text type="secondary">
          结合PostgreSQL
          pgvector和Qdrant向量数据库的混合检索系统，提供高性能的语义搜索能力
        </Text>
            ...
            <Col span={24}>
              <Card title="技术架构" extra={<DatabaseOutlined />}>
                <Paragraph>
                  ...
                  <ol>
                    <li>
                      <strong>查询预处理</strong>：文本清理、分词、查询扩展
                    </li>
                    <li>
                      <strong>并行检索</strong>：pgvector语义搜索 +
                      PostgreSQL全文搜索
                    </li>
                    <li>
                      <strong>结果融合</strong>：RRF（Reciprocal Rank
                      Fusion）算法融合
                    </li>
                    <li>
                      <strong>重排序</strong>：可选的交叉编码器精排
                    </li>
                    <li>
                      <strong>结果返回</strong>：格式化输出，包含相关性分数
                    </li>
                  </ol>
                </Paragraph>
                ...
                <Paragraph>
                  ...
                  <ul>
                    <li>
                      <strong>向量量化</strong>：支持float32/int8/int4量化压缩
                    </li>
                    <li>
                      <strong>索引优化</strong>：HNSW索引优化，支持动态参数调整
                    </li>
                    <li>
                      <strong>缓存策略</strong>：热点查询缓存，减少检索延迟
                    </li>
                    <li>
                      <strong>分片存储</strong>：大规模向量数据分片管理
                    </li>
                  </ul>
                </Paragraph>
              </Card>
