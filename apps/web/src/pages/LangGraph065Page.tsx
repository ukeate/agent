...
const LangGraph065Page: React.FC = () => {
  ...
  const [durability, setDurability] = useState<'exit' | 'async' | 'sync'>(
    'async'
  )
  const [cachingEnabled, setCachingEnabled] = useState(true)
  const [hooksEnabled, setHooksEnabled] = useState(true)
  const [workflowInput, setWorkflowInput] = useState(
    '演示LangGraph 0.6.5的新特性'
  )
  ...
  const loadCacheStats = async () => {
    try {
      const response = await apiFetch(
        buildApiUrl('/api/v1/langgraph/cache/stats')
      )
  ...
  const clearCache = async () => {
    try {
      const response = await apiFetch(
        buildApiUrl('/api/v1/langgraph/cache/clear'),
        {
          method: 'POST',
        }
      )
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">
          LangGraph 0.6.5 核心特性演示
        </h1>
        <p className="text-gray-600">
          体验LangGraph 0.6.5的新Context API、durability控制、Node
          Caching和Pre/Post Model Hooks
        </p>
      </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <ul className="text-sm text-gray-700 space-y-1">
                  <li>
                    • <strong>Fast Cache:</strong> TTL 60秒，适合频繁变化的节点
                  </li>
                  <li>
                    • <strong>Standard Cache:</strong> TTL 300秒，适合一般节点
                  </li>
                  <li>
                    • <strong>Long Cache:</strong> TTL
                    3600秒，适合计算密集型节点
                  </li>
                  <li>
                    • <strong>Context Aware:</strong> 包含上下文哈希的缓存
                  </li>
                </ul>
              </div>
            </CardContent>
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>
                        • <strong>MessageCompressionHook:</strong>{' '}
                        压缩消息历史，减少token使用
                      </li>
                      <li>
                        • <strong>InputSanitizationHook:</strong>{' '}
                        清理与校验输入内容
                      </li>
                      <li>
                        • <strong>ContextEnrichmentHook:</strong> 丰富上下文信息
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    <h4 className="font-medium text-green-900 mb-2">
                      Post Model Hooks (后处理)
                    </h4>
                    <ul className="text-sm text-green-800 space-y-1">
                      <li>
                        • <strong>ResponseFilterHook:</strong> 过滤不当内容
                      </li>
                      <li>
                        • <strong>QualityCheckHook:</strong> 输出质量检查
                      </li>
                      <li>
                        • <strong>ResponseEnhancementHook:</strong>{' '}
                        增强输出元数据
                      </li>
                    </ul>
                  </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm">
                    {execution.caching_enabled ? '已启用' : '已禁用'}
                  </div>
                </div>
              </div>
