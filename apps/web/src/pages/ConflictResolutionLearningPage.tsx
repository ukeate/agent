...
const ConflictResolutionLearningPage: React.FC = () => {
  ...
  const [selectedScenario, setSelectedScenario] =
    useState<ConflictScenario | null>(null)
  const [resolutionResults, setResolutionResults] = useState<
    ResolutionResult[]
  >([])
  const [threeWayDemo, setThreeWayDemo] = useState<ThreeWayMergeDemo | null>(
    null
  )
    ...
    lastWriterWins: (
      ...
      const steps = [
        ...
        localTimestamp >= remoteTimestamp
          ? 'Local 时间更新或相等 -> 选择 Local'
          : 'Remote 时间更新 -> 选择 Remote',
      ]
    ...
    semanticMerge: (local: any, remote: any, fieldType: string) => {
      ...
      switch (fieldType) {
        case 'array':
          ...
          const mergedArray = [
            ...new Set([...localArray, ...remoteArray]),
          ].sort()
  ...
  // 执行冲突解决策略
  const executeResolutionStrategy = async (
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <Badge
                        color={getConflictTypeColor(scenario.conflict_type)}
                      >
                  ...
                  <div>
                    ...
                    <div className="mt-4">
                      ...
                      <div className="space-y-2">
                        {selectedScenario.suggested_strategies.map(
                          (strategy, index) => (
                        ...
                        threeWayDemo.conflicts.map((conflict, index) => (
                          ...
                          >
                            ...
                            <span className="text-sm text-red-700">
                              字段 '{conflict.field}' 存在{' '}
                              {conflict.conflict_type} 冲突
                            </span>
                          </div>
                        ))
                      )}
