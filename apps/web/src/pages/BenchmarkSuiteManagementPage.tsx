...
const BenchmarkSuiteManagementPage: React.FC = () => {
  ...
  const loadSuites = async () => {
    try {
      setLoading(true);
      const res = await apiFetch(buildApiUrl('/api/v1/model-evaluation/benchmark-suites'));
    ...
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          ...
          <Text type="secondary">数据来自 /api/v1/model-evaluation/benchmark-suites</Text>
        </div>
        <Space>
          <Button onClick={loadSuites} icon={<ReloadOutlined />} loading={loading}>
            刷新
          </Button>
        </Space>
      </div>
        ...
        <Table
            ...
            },
            {
              ...
              render: (benchmarks: BenchmarkSuite['benchmarks']) => (
                <Space wrap>
                  ...
                  {(benchmarks || []).length > 6 ? <Tag>+{benchmarks.length - 6}</Tag> : null}
                </Space>
              ),
            },
          ]}
          expandable={{
            expandedRowRender: (record) => (
              <pre style={{ margin: 0, whiteSpace: 'pre-wrap' }}>{JSON.stringify(record, null, 2)}</pre>
            ),
          }}
        />
      </Card>
