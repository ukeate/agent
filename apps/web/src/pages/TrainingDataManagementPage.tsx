...
const TrainingDataManagementPage: React.FC = () => {
  ...
  const [versionStats, setVersionStats] = useState<VersionStatistics | null>(
    null
  )
  ...
  const [newAnnotationTask, setNewAnnotationTask] =
    useState<AnnotationTaskCreate>({
      name: '',
      description: '',
      task_type: '',
  ...
  const loadOverviewData = async () => {
    setLoading(true)
    try {
      const [sources, records, tasks, stats, versionStats, queue] =
        await Promise.all([
          trainingDataService.listDataSources().catch(() => []),
          trainingDataService
            .getDataRecords({ limit: 10 })
            .catch(() => ({ records: [], count: 0, offset: 0, limit: 10 })),
          trainingDataService
            .listAnnotationTasks({ limit: 10 })
            .catch(() => ({ tasks: [], count: 0, offset: 0, limit: 10 })),
  ...
  const handleCreateAnnotationTask = async () => {
    ...
    try {
      if (!authService.getToken()) throw new Error('未登录，请先登录')
      if (!newAnnotationTask.data_records.length)
        throw new Error('请填写至少一个数据记录ID')
      if (!newAnnotationTask.created_by) throw new Error('创建者不能为空')
      if (!Object.keys(newAnnotationTask.annotation_schema || {}).length)
        throw new Error('标注模式不能为空')
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-green-50 p-4 rounded-lg">
          ...
          <p className="text-2xl font-bold text-green-900">
            {collectionStats?.total_records ?? dataRecords.length}
          </p>
        ...
        <div className="bg-yellow-50 p-4 rounded-lg">
          ...
          <p className="text-sm text-yellow-600">
            进行中:{' '}
            {annotationTasks.filter(t => t.status === 'in_progress').length}
          </p>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-xl font-bold">
                {(collectionStats.quality_stats?.average ?? 0).toFixed(2)}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">最高质量分数</p>
              <p className="text-xl font-bold">
                {(collectionStats.quality_stats?.maximum ?? 0).toFixed(2)}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">最低质量分数</p>
              <p className="text-xl font-bold">
                {(collectionStats.quality_stats?.minimum ?? 0).toFixed(2)}
              </p>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {dataSources.map(source => (
                <tr key={source.id}>
                  ...
                  <td className="border border-gray-300 p-2">
                    <span
                      className={`px-2 py-1 rounded text-xs ${source.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                    >
                ...
                <tr key={record.id}>
                  ...
                  <td className="border border-gray-300 p-2">
                    {record.quality_score
                      ? record.quality_score.toFixed(2)
                      : 'N/A'}
                  </td>
                  <td className="border border-gray-300 p-2">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid grid-cols-2 gap-4">
          <input
            ...
            onChange={e =>
              setNewAnnotationTask(prev => ({ ...prev, name: e.target.value }))
            }
            className="border p-2 rounded"
          />
          <input
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-sm">
                总大小: {versionStats.storage_usage.total_size} MB
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">
                {collectionStats.total_records}
              </p>
              <p className="text-sm text-gray-600">总记录数</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">
                {collectionStats.status_distribution?.raw ?? 0}
              </p>
              <p className="text-sm text-gray-600">原始记录</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-600">
                {collectionStats.status_distribution?.processed ?? 0}
              </p>
              <p className="text-sm text-gray-600">已处理记录</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-red-600">
                {collectionStats.status_distribution?.error ?? 0}
              </p>
          ...
          <div className="mt-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                  {Object.entries(
                    collectionStats.source_distribution || {}
                  ).map(([sourceId, stat]) => (
                    <tr key={sourceId}>
                      ...
                      <td className="border border-gray-300 p-2">
                        {(stat.average_quality ?? 0).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-sm">
                总存储:{' '}
                {(versionStats.total_size_bytes / (1024 * 1024)).toFixed(2)} MB
              </p>
              <p className="text-sm">
                平均版本大小:{' '}
                {(
                  versionStats.average_size_per_version /
                  (1024 * 1024)
                ).toFixed(2)}{' '}
                MB
              </p>
