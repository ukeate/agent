...
const HypothesisTestingPage: React.FC = () => {
  ...
  const runOneSampleTTest = async () => {
    try {
      ...
      setTestResults(prev => [
        ...prev,
        { id: Date.now(), type: '单样本t检验', result },
      ])
  ...
  const runTwoSampleTTest = async () => {
    try {
      ...
      setTestResults(prev => [
        ...prev,
        { id: Date.now(), type: '双样本t检验', result },
      ])
  ...
  const runABTestComparison = async () => {
    try {
      ...
      setTestResults(prev => [
        ...prev,
        { id: Date.now(), type: 'A/B测试比较', result },
      ])
            ...
            <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
              ...
              <Col span={8}>
                <Button
                  ...
                  disabled={
                    loading || controlTotal === 0 || treatmentTotal === 0
                  }
                  style={{ width: '100%', height: 40 }}
                >
                  执行A/B测试
              ...
              <Row gutter={[16, 8]}>
                <Col span={12}>
                  <Text type="secondary">
                    对照组转化率:{' '}
                    {controlTotal > 0
                      ? ((controlConversions / controlTotal) * 100).toFixed(2)
                      : 0}
                    %
                  </Text>
                </Col>
                <Col span={12}>
                  <Text type="secondary">
                    实验组转化率:{' '}
                    {treatmentTotal > 0
                      ? ((treatmentConversions / treatmentTotal) * 100).toFixed(
                          2
                        )
                      : 0}
                    %
                ...
                <Col span={24}>
                  <Text type="secondary">
                    相对提升:{' '}
                    {controlTotal > 0 && treatmentTotal > 0
                      ? (
          ...
          {testResults.length === 0 ? (
            ...
            <>
              ...
              {testResults.map(test =>
                renderTestResult(test.result, test.type)
              )}
            </>
          )}
        </Space>
            ...
            <Col span={6}>
              <Card>
                <Statistic
                  title="显著结果"
                  value={
                    testResults.filter(t => t.result?.result?.is_significant)
                      .length
                  }
                    ...
                    testResults.length > 0
                      ? (
                          testResults
                            ...
                            .reduce(
                              (sum, t) => sum + t.result.result.effect_size,
                              0
                            ) /
                          testResults.filter(t => t.result?.result?.effect_size)
                            .length
                        ).toFixed(3)
                      : '0.000'
                  }
                ...
                <Statistic
                  ...
                  value={
                    testResults.length > 0
                      ? (
                          testResults.reduce(
                            (sum, t) => sum + (t.result?.result?.p_value || 0),
                            0
                          ) / testResults.length
                        ).toExponential(2)
                      : '0.00e+0'
