    ...
    <div style={{ padding: 24 }}>
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          ...
          <Text type="secondary">
            所有数据直接来源 /api/v1/documents，无本地静态数据
          </Text>
        </Col>
        <Col>
          <Button
      ...
      <Card style={{ marginBottom: 16 }}>
        <Dragger
          {...uploadProps}
          accept=".pdf,.doc,.docx,.txt,.md,.json,.xml,.html,.png,.jpg"
        >
              ...
              <List.Item>
                <List.Item.Meta
                  ...
                  description={
                    <div>
                      <Text type="secondary">
                        类型: {doc.file_type}，上传时间:{' '}
                        {new Date(doc.upload_time).toLocaleString()}
                      </Text>
                      <div style={{ marginTop: 4 }}>
                        <Tag
                          color={
                            doc.status === 'completed' ? 'green' : 'orange'
                          }
                        >
                          {doc.status}
                        </Tag>
