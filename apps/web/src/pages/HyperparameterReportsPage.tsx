...
const HyperparameterReportsPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(buildApiUrl('/api/v1/hyperparameter-optimization/experiments'));
        ...
        <Card title="超参实验报告">
          <Table
            ...
            columns={[
              ...
              { title: '下载', render: (_, r) => r.download_url ? <a href={r.download_url} target="_blank" rel="noreferrer">查看</a> : '-' }
            ]}
          />
        </Card>
      </Space>
