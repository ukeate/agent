...
const UnifiedMonitorPage: React.FC = () => {
  ...
  const loadUnifiedData = async () => {
    ...
    try {
      ...
      setUnifiedMetrics(prev => metrics ?? prev)
      setModulesStatus(prev => modules ?? prev)
      setSystemMetrics(prev => sysMetrics ?? prev)
      setMonitoringSummary(prev => summary ?? prev)
      setMonitoringAlerts(prev => alerts ?? prev)
    } catch (error) {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center p-4 bg-slate-50 rounded-lg">
                      <div className="text-2xl font-bold text-slate-700">
                        {modulesStatus.summary.total_attempted}
                      </div>
                      <div className="text-sm text-slate-500">模块总数</div>
                    </div>
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <div className="text-2xl font-bold text-green-600">
                        {modulesStatus.summary.loaded}
                      </div>
                      <div className="text-sm text-green-700">运行中</div>
                    </div>
                    <div className="text-center p-4 bg-red-50 rounded-lg">
                      <div className="text-2xl font-bold text-red-600">
                        {modulesStatus.summary.failed}
                      </div>
                      <div className="text-sm text-red-700">异常</div>
                    </div>
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <div className="text-2xl font-bold text-blue-600">
                        {modulesStatus.summary.success_rate}
                      </div>
              ...
              {modulesStatus && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <div className="text-sm text-gray-500">
                    最后更新:{' '}
                    {new Date(
                      modulesStatus.timestamp || Date.now()
                    ).toLocaleString()}
                  </div>
                </div>
              )}
            ...
            {monitoringSummary && (
              <div className="bg-white shadow rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  <div className="text-center p-4 bg-green-50 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">
                      {monitoringSummary.health_score}%
                    </div>
                  ...
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">
                      {monitoringSummary.performance_metrics?.avg_response_time}
                      ms
                    </div>
                  ...
                  <div className="text-center p-4 bg-purple-50 rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">
                      {
                        monitoringSummary.performance_metrics
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 border rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-sm text-gray-600">
                        {monitoringSummary.resource_usage?.cpu_percent}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${monitoringSummary.resource_usage?.cpu_percent}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  <div className="p-4 border rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-sm text-gray-600">
                        {monitoringSummary.resource_usage?.memory_percent}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${monitoringSummary.resource_usage?.memory_percent}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  <div className="p-4 border rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-sm text-gray-600">
                        {monitoringSummary.resource_usage?.disk_percent}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${monitoringSummary.resource_usage?.disk_percent}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        <span>磁盘使用率:</span>
                        <span className="font-medium">
                          {systemMetrics.system?.disk_usage}%
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>系统运行时间:</span>
                        <span className="font-medium">
                          {Math.floor(systemMetrics.system?.uptime / 3600)}小时
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span>活跃请求:</span>
                        <span className="font-medium">
                          {systemMetrics.application?.active_requests}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>总请求数:</span>
                        <span className="font-medium">
                          {systemMetrics.application?.total_requests?.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>错误率:</span>
                        <span className="font-medium">
                          {(
                            systemMetrics.application?.error_rate * 100
                          ).toFixed(2)}
                          %
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>平均响应时间:</span>
                        <span className="font-medium">
                          {systemMetrics.application?.average_response_time}ms
                        </span>
                      </div>
                    </div>
                  </div>
                      ...
                      ([serviceName, service]: [string, any]) => (
                        ...
                        >
                          ...
                          <div className="text-sm text-gray-600 space-y-1">
                            ...
                            {service.hit_rate && (
                              <div>
                                命中率: {(service.hit_rate * 100).toFixed(1)}%
                              </div>
                            )}
                          </div>
                        </div>
