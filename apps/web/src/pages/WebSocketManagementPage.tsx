...
import { webSocketConnectionService, type WebSocketConnectionsResponse, type WebSocketConnection } from '../services/webSocketConnectionService'
...
const WebSocketManagementPage: React.FC = () => {
  ...
  const connect = () => {
    ...
    try {
      ...
      wsRef.current.onclose = (event) => {
        ...
        message.info(`WebSocket连接关闭 (${event.code}: ${event.reason || '正常关闭'})`)
  ...
  const sendMessage = (content: any, messageType?: string) => {
    ...
    try {
      ...
      const newMessage: WebSocketMessage = {
        ...
        messageType: messageType || (typeof content === 'object' ? content.type : 'custom')
      }
  ...
  const disconnectSpecificConnection = async (connectionIndex: number) => {
    try {
      const result = await webSocketConnectionService.disconnectConnection(connectionIndex)
      if (result.success) {
  ...
  const broadcastMessage = async () => {
    ...
    try {
      ...
      const result = await webSocketConnectionService.broadcastMessage(messageData)
      if (result.success) {
  ...
  const sendToSpecificConnection = async (connectionIndex: number) => {
    ...
    try {
      ...
      const result = await webSocketConnectionService.sendMessageToConnection(connectionIndex, messageData)
      if (result.success) {
    ...
    <div style={{ padding: '24px' }}>
        ...
        <Col span={6}>
          <Card>
            <Statistic
              ...
              prefix={isConnected ? <CheckCircleOutlined /> : <WarningOutlined />}
                  ...
                  <List.Item.Meta
                    ...
                    description={
                      <Space direction="vertical" size="small">
                        <div>
                          <Tag color={item.state === 'connected' ? 'green' : 'red'}>
                            {item.state}
                          </Tag>
                          <Text type="secondary">
                            连接时间: {dayjs(item.connected_at).format('MM-DD HH:mm:ss')}
                          </Text>
                        </div>
                        {item.client_info && (
                        ...
                        {item.messages_sent !== undefined && (
                          <Space size="large">
                            <Text type="secondary">发送: {item.messages_sent || 0}</Text>
                            <Text type="secondary">接收: {item.messages_received || 0}</Text>
                          </Space>
                        )}
                      </Space>
                    }
      ...
      <Card title="连接管理与消息发送" style={{ marginTop: 24 }}>
        <Space direction="vertical" style={{ width: '100%' }} size="large">
          ...
          <Row gutter={[16, 16]}>
            <Col span={12}>
              <Button
                ...
                disabled={!connectionStatus || connectionStatus.active_connections === 0}
                block
              >
          ...
          {/* 消息发送区域 */}
          <div>
            <Text strong style={{ marginBottom: 8, display: 'block' }}>消息内容:</Text>
            <TextArea
              value={customMessage}
            ...
            <Space wrap>
              ...
              <Button
                ...
                disabled={!customMessage.trim() || !connectionStatus || connectionStatus.active_connections === 0}
              >
                广播到所有连接
              </Button>
              <Button
