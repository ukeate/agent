...
const AnomalyDetectionPage: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      setLoading(true)
      const [typesResponse, methodsResponse, healthResponse] =
        await Promise.all([
    ...
    try {
      ...
      if (response.success) {
        ...
        setStatistics(prev => ({
          ...
          highSeverity: response.anomalies.filter(a => a.severity === 'high')
            .length,
          mediumSeverity: response.anomalies.filter(
            a => a.severity === 'medium'
          ).length,
          lowSeverity: response.anomalies.filter(a => a.severity === 'low')
            .length,
        }))
  ...
  const handleSRMCheck = async (values: any) => {
    try {
      ...
      const response =
        await anomalyDetectionService.checkSampleRatioMismatch(request)
  ...
  const handleSetupRealTimeMonitoring = async (values: any) => {
    try {
      ...
      const response =
        await anomalyDetectionService.setupRealTimeMonitoring(request)
      ...
      if (response.success) {
        ...
        notification.success({
          message: '实时监控已启动',
          description:
            response.message || `实验 ${values.experiment_id} 的实时监控已开始`,
        })
      }
    } catch (error) {
      notification.error({
  ...
  const handleBatchDetectAnomalies = async (values: any) => {
    try {
      ...
      const response = await anomalyDetectionService.batchDetectAnomalies(
        experiments,
        metrics
      )
      ...
      if (response.success) {
        ...
        // 计算总异常数量
        const totalAnomalies = Object.values(response.results).reduce(
          (total, expResults) => {
            return (
              total +
              Object.values(expResults as any).reduce(
                (expTotal, metricResult) => {
          ...
          <Card>
            <div>
              ...
              <Progress
                ...
                width={60}
                format={percent =>
                  statistics.qualityScore === null ? '暂无数据' : `${percent}%`
                }
                strokeColor={{
                  '0%': '#ff4d4f',
                  '50%': '#faad14',
        ...
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          ...
          >
            ...
            >
              ...
              <Form.Item name="timestamps" label="时间戳列表（可选）">
                <Input.TextArea placeholder="输入逗号分隔的时间戳" rows={2} />
              </Form.Item>
          ...
          <TabPane
            ...
            key="srm"
          >
            <Form form={srmForm} layout="vertical" onFinish={handleSRMCheck}>
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item
                    name="experiment_id"
          ...
          >
            ...
            >
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item name="methods" label="检测方法">
                    <Select
                      mode="multiple"
                      placeholder="选择检测方法"
                      style={{ width: '100%' }}
              ...
              <Row gutter={16}>
                ...
                <Col span={8}>
                  <Form.Item name="min_samples" label="最小样本数">
                    <InputNumber min={10} style={{ width: '100%' }} />
                  </Form.Item>
                </Col>
                <Col span={8}>
                  <Form.Item name="z_threshold" label="Z-score阈值">
                    <InputNumber
                      min={2.0}
                      step={0.1}
                      style={{ width: '100%' }}
              ...
              <Row gutter={16}>
                <Col span={8}>
                  <Form.Item name="iqr_multiplier" label="IQR乘数">
                    <InputNumber
                      min={1.0}
                      step={0.1}
                      style={{ width: '100%' }}
          ...
          <TabPane
            ...
            key="summary"
          >
            <Form layout="vertical" onFinish={handleGetAnomalySummary}>
              <Row gutter={16}>
                <Col span={8}>
                  <Form.Item
                    name="experiment_id"
          ...
          <TabPane
            ...
            key="realtime"
          >
            <Form layout="vertical" onFinish={handleSetupRealTimeMonitoring}>
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item
                    name="experiment_id"
                ...
                <Row gutter={16}>
                  <Col span={8}>
                    <Statistic
                      ...
                      valueStyle={{
                        color: monitoringStatus.active ? '#52c41a' : '#ff4d4f',
                      }}
                    />
                  </Col>
                  <Col span={8}>
          ...
          <TabPane
            ...
            key="batch"
          >
            <Form layout="vertical" onFinish={handleBatchDetectAnomalies}>
              <Form.Item
                name="experiments"
                label="实验ID列表"
