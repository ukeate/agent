...
const AuthManagementPage: React.FC = () => {
  ...
  const handleCheckPermission = async (values: { resource: string; action: string }) => {
    setLoading(true);
    try {
      const ok = await authService.checkPermission(values.resource, values.action);
  ...
  const sessionColumns = [
    ...
    { title: '最后活动', dataIndex: 'last_activity', key: 'last_activity' },
    {
      ...
      key: 'is_current',
      render: (v: boolean) => (v ? <Tag color="blue">当前</Tag> : <Tag>其他</Tag>),
    },
    {
      title: '操作',
      key: 'actions',
      render: (_: unknown, record: SessionItem) => (
        <Button danger size="small" onClick={() => handleRevokeSession(record.session_id)}>
          撤销
        </Button>
      ),
    },
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Paragraph type="secondary">覆盖 /api/v1/auth 下的认证、权限、会话与安全相关接口。</Paragraph>

      {!authed && (
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          <Alert message="未登录：请先登录后再查看会话与权限信息" type="warning" showIcon />
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          ...
          >
            <Space direction="vertical" size={8} style={{ width: '100%' }}>
              ...
              <Space wrap>
                <Text>权限:</Text>
                {(permissionSummary?.permissions || user.permissions || []).map((p) => (
                  <Tag key={p}>{p}</Tag>
                ))}
              </Space>
              <Card title="权限检查" size="small" type="inner">
                <Form layout="inline" onFinish={handleCheckPermission} initialValues={{ resource: 'users', action: 'read' }}>
                  <Form.Item name="resource" rules={[{ required: true, message: '请输入资源' }]}>
                    <Input placeholder="resource" style={{ width: 180 }} />
                  </Form.Item>
                  <Form.Item name="action" rules={[{ required: true, message: '请输入动作' }]}>
          ...
          <Card
            ...
            size="small"
            extra={
              <Button danger onClick={handleRevokeAllSessions} loading={loading} disabled={sessions.length === 0}>
                撤销全部会话
              </Button>
            }
          >
      ...
      <Modal title="注册" open={registerOpen} onCancel={() => setRegisterOpen(false)} footer={null} destroyOnClose>
      ...
      <Modal title="编辑资料" open={profileOpen} onCancel={() => setProfileOpen(false)} footer={null} destroyOnClose>
      ...
      <Modal title="修改密码" open={passwordOpen} onCancel={() => setPasswordOpen(false)} footer={null} destroyOnClose>
