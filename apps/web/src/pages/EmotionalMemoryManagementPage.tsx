...
export default function EmotionalMemoryManagementPage() {
  ...
  const [predictResult, setPredictResult] = useState<Record<
    string,
    any
  > | null>(null)
          ...
          <Space>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                setLoading(true)
                try {
                  const out = await emotionalMemoryService.exportMemories(
                    userId,
                    'json'
                  )
        ...
        <Card title="统计" loading={loading}>
          {stats ? (
            <Descriptions size="small" column={4}>
              <Descriptions.Item label="总记忆数">
                {stats.total_memories}
              </Descriptions.Item>
              <Descriptions.Item label="平均强度">
                {stats.avg_intensity.toFixed(3)}
              </Descriptions.Item>
              <Descriptions.Item label="存储分布">
                {JSON.stringify(stats.storage_distribution)}
              </Descriptions.Item>
              <Descriptions.Item label="情感分布">
                {JSON.stringify(stats.emotion_distribution)}
              </Descriptions.Item>
            </Descriptions>
          ) : (
          ...
          extra={
            <Input.Search
              ...
              onSearch={async v => {
                ...
                setLoading(true)
                try {
                  const out = await emotionalMemoryService.searchMemories({
                    query: v.trim(),
                    limit: 20,
                  })
      ...
      >
        <Form
          ...
          initialValues={{
            ...
            storage_layer: StorageLayer.HOT,
          }}
        ...
        >
          ...
          >
            <Input.TextArea
              rows={4}
              placeholder="context 字段（后端会作为 content 参与处理）"
            />
          </Form.Item>
        ...
        <Form
          ...
          onFinish={async values => {
            ...
            try {
              ...
              const out = await emotionalMemoryService.predictRisk(
                userId,
                context
              )
