...
const HyperparameterSchedulerPage: React.FC = () => {
  ...
  const fetchJson = async <T,>(url: string, options?: RequestInit): Promise<T> => {
    const res = await apiFetch(url, { headers: { 'Content-Type': 'application/json' }, ...options });
    return res.json();
  };
  ...
  const load = async () => {
    ...
    try {
      ...
      if (selected) {
        const ts = await fetchJson<Trial[]>(`${API}/experiments/${selected}/trials`);
        setTrials(ts);
      }
    } catch (e: any) {
  ...
  const loadTrials = async (experimentId: string) => {
    ...
    setError(null);
    try {
      const ts = await fetchJson<Trial[]>(`${API}/experiments/${experimentId}/trials`);
      setTrials(ts);
    } catch (e: any) {
      setError(e?.message || '加载失败');
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-500">{experiments.length} 个</span>
          </div>
          <div className="space-y-2 max-h-[420px] overflow-auto">
            {experiments.map(exp => (
              <div key={exp.id} className={`p-3 border rounded-md ${selected === exp.id ? 'border-blue-500' : 'border-gray-200'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-semibold">{exp.name}</div>
                    <div className="text-xs text-gray-500">{exp.algorithm} · {exp.objective}</div>
                  </div>
                  <Badge className="bg-blue-100 text-blue-800">{formatState(exp.status)}</Badge>
                </div>
                <div className="mt-2 flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => loadTrials(exp.id)}>查看</Button>
                  <Button size="sm" variant="outline" onClick={() => startExperiment(exp.id)}>启动</Button>
                  <Button size="sm" variant="outline" onClick={() => stopExperiment(exp.id)}>停止</Button>
                </div>
              </div>
            ))}
            {experiments.length === 0 && <div className="text-sm text-gray-500">暂无实验，请先通过 API 创建实验。</div>}
          </div>
        </Card>
        ...
        <Card className="p-4 lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            ...
            <Input
              ...
              onPressEnter={(e) => loadTrials((e.target as HTMLInputElement).value)}
            />
          </div>
          {selected === '' && <div className="text-sm text-gray-500">选择或输入实验ID以查看试验。</div>}
          {selected !== '' && (
              ...
              {trials.map(t => (
                <div key={t.id} className="p-3 border rounded-md">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className="bg-green-100 text-green-800">{formatState(t.state)}</Badge>
                  </div>
                  <div className="text-xs text-gray-600 mt-1">结果: {t.value ?? '暂无'}</div>
                  <div className="text-xs text-gray-600 mt-1">
                    参数: {Object.entries(t.parameters || {}).map(([k, v]) => `${k}=${v}`).join(', ') || '无'}
                  </div>
                </div>
              ))}
              {trials.length === 0 && <div className="text-sm text-gray-500">未查询到试验记录。</div>}
            </div>
          )}
        </Card>
