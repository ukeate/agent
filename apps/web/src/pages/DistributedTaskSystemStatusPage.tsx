...
const DistributedTaskSystemStatusPage: React.FC = () => {
  ...
  const [alerts, setAlerts] = useState<
    { id: string; level: 'info' | 'warning' | 'error'; message: string; timestamp: string }[]
  >([]);
    ...
    try {
      ...
      if (agents?.agents) {
        const derivedAlerts = agents.agents
          .filter((a: any) => a.resource_usage?.cpu_usage > 80 || a.status !== 'online')
  ...
  return (
    <div style={{ padding: '24px', background: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <Button icon={<ReloadOutlined />} onClick={fetchClusterData} loading={loading} style={{ marginBottom: 16 }}>
        刷新
      </Button>

      {!systemStatus && <Alert type="info" message="正在加载集群状态..." style={{ marginBottom: 16 }} />}
        ...
        <>
          ...
          <Row gutter={16}>
            <Col span={16}>
              <Card 
                title="系统详细信息"
                extra={<Button icon={<ReloadOutlined />} onClick={fetchClusterData} loading={loading}>刷新</Button>}
              >
                <Descriptions bordered column={2}>
                  ...
                  <Descriptions.Item label="存储使用">
                    <Progress percent={Math.min(100, (systemStatus.task_throughput || 0) % 100)} size="small" />
                  </Descriptions.Item>
                  <Descriptions.Item label="内存使用">
                    <Progress percent={Math.min(100, systemStatus.avg_memory_usage || 0)} size="small" />
                  </Descriptions.Item>
                  <Descriptions.Item label="CPU使用">
                    <Progress percent={Math.min(100, systemStatus.avg_cpu_usage || 0)} size="small" />
                  </Descriptions.Item>
                </Descriptions>
              </Card>
            </Col>
              ...
              <Card title="系统告警">
                <Timeline size="small">
                  {alerts.map(alert => (
                    <Timeline.Item
                      key={alert.id}
                      color={alert.level === 'warning' ? 'orange' : alert.level === 'error' ? 'red' : 'green'}
                    >
                      <div>
