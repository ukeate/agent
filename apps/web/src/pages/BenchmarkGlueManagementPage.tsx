...
type BenchmarkRow = { name: string; tasks?: string[]; description?: string; type?: string; difficulty?: string };
...
const BenchmarkGlueManagementPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(buildApiUrl('/api/v1/model-evaluation/benchmarks'));
        ...
        <Card>
          {loading ? (
            ...
            <Table
              ...
              locale={{ emptyText: '暂无基准数据，请先在后端 BenchmarkManager 注册。' }}
            />
          )}
        </Card>
      </Space>
