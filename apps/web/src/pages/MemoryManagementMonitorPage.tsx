...
const MemoryManagementMonitorPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'memories' | 'analytics' | 'patterns' | 'cleanup'>('overview');
  ...
  const handleSearch = async () => {
    ...
    try {
      const results = await memoryManagementService.searchMemories({
        query: searchQuery,
        memory_types: filters.memoryTypes.length > 0 ? filters.memoryTypes : undefined,
        status: filters.status.length > 0 ? filters.status : undefined,
        min_importance: filters.minImportance > 0 ? filters.minImportance : undefined,
        max_importance: filters.maxImportance < 1 ? filters.maxImportance : undefined,
  ...
  const handleCreateMemory = async () => {
    ...
    setLoading(true);
    try {
      await memoryManagementService.createMemory(createForm, selectedSession || undefined);
      setCreateForm({
        type: MemoryType.WORKING,
        content: '',
        importance: 0.5,
      ...
      {analytics && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold text-green-900">{analytics.avg_importance.toFixed(2)}</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <h3 className="text-lg font-semibold text-purple-800">访问总数</h3>
            <p className="text-2xl font-bold text-purple-900">{analytics.total_access_count}</p>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg">
            <h3 className="text-lg font-semibold text-orange-800">存储使用(MB)</h3>
            <p className="text-2xl font-bold text-orange-900">{analytics.storage_usage_mb.toFixed(2)}</p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="space-y-2">
              {Object.entries(analytics.memories_by_type).map(([type, count]) => (
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="space-y-2">
              {Object.entries(analytics.memories_by_status).map(([status, count]) => (
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-xl font-bold">{graphStats.graph_overview.total_nodes}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">连接数</p>
              <p className="text-xl font-bold">{graphStats.graph_overview.total_edges}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">图密度</p>
              <p className="text-xl font-bold">{graphStats.graph_overview.density.toFixed(3)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">连通分量</p>
              <p className="text-xl font-bold">{graphStats.graph_overview.connected_components}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setCreateForm({...createForm, type: e.target.value as MemoryType})}
          ...
          <div>
            <label className="block text-sm font-medium mb-2">重要性 (0-1)</label>
            <input
              type="number"
              min="0"
              max="1"
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-2 py-1 rounded text-xs ${
                        memory.type === 'working' ? 'bg-blue-100 text-blue-800' :
                        memory.type === 'episodic' ? 'bg-green-100 text-green-800' :
                      ...
                      <span className={`px-2 py-1 rounded text-xs ${
                        ...
                        memory.status === 'archived' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      ...
                      <span>创建时间: {new Date(memory.created_at).toLocaleString()}</span>
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{trends.summary.total_memories}</p>
              <p className="text-sm text-gray-600">总记忆数</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">{trends.summary.avg_daily_creation}</p>
              <p className="text-sm text-gray-600">日均创建数</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">{trends.summary.growth_rate.toFixed(2)}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(patterns.time_patterns.daily_distribution).map(([date, count]) => (
              ...
              <div>
                ...
                <div className="space-y-1">
                  {patterns.usage_patterns.most_active_days.map(([date, count]) => (
                    <div key={date} className="flex justify-between">
                      <span className="text-sm">{date}</span>
                      <span className="font-medium">{count}</span>
                    </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(patterns.content_patterns.tag_frequency).map(([tag, count]) => (
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-xl font-bold text-green-600">{analytics.memories_by_status.active || 0}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">已归档</p>
              <p className="text-xl font-bold text-yellow-600">{analytics.memories_by_status.archived || 0}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">已压缩</p>
              <p className="text-xl font-bold text-blue-600">{analytics.memories_by_status.compressed || 0}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">存储使用</p>
              <p className="text-xl font-bold text-purple-600">{analytics.storage_usage_mb.toFixed(2)} MB</p>
            </div>
          </div>
        </div>
      )}
