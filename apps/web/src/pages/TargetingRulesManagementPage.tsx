...
const TargetingRulesManagementPage: React.FC = () => {
  ...
  const [testResult, setTestResult] = useState<UserEvaluationResponse | null>(
    null
  )
    ...
    try {
      const request: CreateRuleRequest = {
        ...
        experiment_ids: values.experiment_ids
          ? values.experiment_ids.split(',').map((id: string) => id.trim())
          : [],
        variant_ids: values.variant_ids
          ? values.variant_ids.split(',').map((id: string) => id.trim())
          : [],
        metadata: {},
      }
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                  ...
                  <Col span={12}>
                    <Card title="规则类型分布">
                      <Space direction="vertical" style={{ width: '100%' }}>
                        {Object.entries(statistics.rule_types).map(
                          ([type, count]) => (
                            <div
                              key={type}
                              style={{
                ...
                <Col span={8}>
                  <Card title="运算符参考">
                    {operators && (
                      <Space direction="vertical" style={{ width: '100%' }}>
                        <Text strong>
                          规则运算符: {operators.rule_operators.length}
                        </Text>
                        <Text>
                          逻辑运算符: {operators.logical_operators.length}
                        </Text>
        ...
        >
          <Form form={form} layout="vertical" onFinish={createRule}>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label="实验ID列表" name="experiment_ids">
                  <Input placeholder="多个ID用逗号分隔" />
                </Form.Item>
              </Col>
              <Col span={12}>
                <Form.Item label="变体ID列表" name="variant_ids">
                  <Input placeholder="多个ID用逗号分隔" />
                </Form.Item>
              </Col>
            </Row>
        ...
        <Modal
          ...
          width={800}
        >
          <Form form={editForm} layout="vertical" onFinish={updateRule}>
            <Row gutter={16}>
              <Col span={12}>
            ...
            <Row gutter={16}>
              ...
              <Col span={8}>
                <Form.Item label="匹配值" name="value">
        ...
        <Modal
          ...
          width={800}
        >
          <Form form={testForm} layout="vertical" onFinish={testRule}>
            <Form.Item
              label="用户ID"
              name="user_id"
                ...
                <Button
                  onClick={() => {
                    testForm.setFieldsValue({
                      ...
                      user_context:
                        '{"country": "US", "user_type": "premium", "device_type": "mobile", "age": 25}',
                    })
                  }}
                >
                  填充示例数据
        ...
        >
          ...
          >
            <Form.Item label="实验ID (可选)" name="experiment_id">
  ...
  {"user_id": "user_003", "country": "JP", "user_type": "premium", "device_type": "mobile"}
          ...
          {batchTestResult && (
            <div style={{ marginTop: '24px' }}>
              ...
              <div>
                <Text strong>详细结果:</Text>
                {batchTestResult.detailed_results
                  ?.slice(0, 5)
                  .map((result: any, index: number) => (
                    <Card key={index} size="small" style={{ marginTop: '8px' }}>
                      <Space direction="vertical" style={{ width: '100%' }}>
                        ...
                        <Text type="secondary">
                          规则类型:{' '}
                          {result.matched_rule_types?.join(', ') || '无匹配'}
                ...
                {batchTestResult.detailed_results?.length > 5 && (
                  ...
                  >
                    显示前5个结果，总共{' '}
                    {batchTestResult.detailed_results.length} 个用户
                  </Text>
                )}
              </div>
            </div>
                ...
                <Card size="small" style={{ marginTop: '8px' }}>
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    >
                      <Text>匹配规则:</Text>
                      <Text>
                        {eligibilityResult.matched_rules?.join(', ') || '无'}
                      </Text>
                    </div>
        ...
        >
          {ruleTemplates ? (
            <div>
              ...
              <Row gutter={16}>
                {Object.entries(ruleTemplates.templates || {}).map(
                  ([name, template]: [string, any]) => (
                    <Col span={8} key={name}>
                      <Card
                        title={template.name}
                        size="small"
                        ...
                        extra={
                          <Tag
                            color={
                              template.rule_type === 'blacklist'
                                ? 'red'
                                : template.rule_type === 'whitelist'
                                  ? 'green'
                                  : 'blue'
                            }
                          >
