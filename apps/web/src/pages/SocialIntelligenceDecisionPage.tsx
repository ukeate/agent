...
const SocialIntelligenceDecisionPage: React.FC = () => {
  ...
  const loadStatus = async () => {
    ...
    setError(null);
    try {
      const data = await fetchJson<SystemStatus>('/api/v1/social-emotion/status');
      setStatus(data);
    } catch (e: any) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const data = await fetchJson<AnalysisResult>('/api/v1/social-emotion/analyze', {
        method: 'POST',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={loadStatus} disabled={loading}>刷新状态</Button>
      </div>
      ...
      <Card className="p-4 space-y-2">
        ...
        <div className="text-sm text-gray-700">
          状态: {status?.status || '未知'}<br />
          模式: {status?.mode || '-'}<br />
          隐私级别: {status?.privacy_level || '-'}<br />
      ...
      <Card className="p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <Button onClick={analyze} disabled={loading || !text.trim()}>提交分析</Button>
          <Button variant="outline" onClick={() => { setText(''); setResult(null); }}>清空</Button>
        </div>
        {result && (
