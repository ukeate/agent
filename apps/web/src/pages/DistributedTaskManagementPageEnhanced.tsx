...
const DistributedTaskManagementPageEnhanced: React.FC = () => {
  ...
  const [alerts, setAlerts] = useState<
    Array<{ type: 'success' | 'error' | 'warning'; message: string }>
  >([])
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [checkpointData, healthData, analysisData, clusterData] =
        await Promise.all([
  ...
  const handleCreateCheckpoint = async () => {
    ...
    try {
      await distributedTaskServiceEnhanced.createCheckpoint(
        name,
        '手动创建的检查点'
      )
  ...
  const handleGracefulShutdown = async () => {
    try {
      ...
      showAlert(
        'success',
        `优雅关闭完成：${result.tasks_completed}个任务完成，${result.tasks_cancelled}个任务取消`
      )
    } catch (error) {
                        ...
                        >
                          <div>
                            ...
                            >
                              ...
                              <div>
                                状态大小:{' '}
                                {(checkpoint.state_size / 1024 / 1024).toFixed(
                                  1
                                )}{' '}
                                MB
                              </div>
                          ...
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <Button
                              onClick={() =>
                                handleRollbackCheckpoint(checkpoint.name)
                              }
                              style={{
                            ...
                            <Button
                              onClick={() =>
                                handleDeleteCheckpoint(checkpoint.name)
                              }
                              style={{
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div style={{ fontWeight: 'bold' }}>
                            {Math.floor(
                              systemHealth.uptime / (24 * 60 * 60 * 1000)
                            )}{' '}
                            天
                          </div>
                        </div>
                    ...
                    >
                      <Card style={{ border: '1px solid #e5e7eb' }}>
                        <div style={{ padding: '12px' }}>
                          ...
                          >
                            {(
                              taskAnalysis.performance_trends.completion_rate[
                                taskAnalysis.performance_trends.completion_rate
                                  .length - 1
                              ]?.rate * 100
                            ).toFixed(1)}
                      ...
                      <Card style={{ border: '1px solid #e5e7eb' }}>
                        <div style={{ padding: '12px' }}>
                          ...
                          >
                            {taskAnalysis.performance_trends.average_duration[
                              taskAnalysis.performance_trends.average_duration
                                .length - 1
                            ]?.duration.toFixed(0)}
                            s
                ...
                <Card>
                  <div style={{ padding: '16px' }}>
                    ...
                    <div style={{ display: 'grid', gap: '8px' }}>
                      {taskAnalysis.performance_trends.error_patterns.map(
                        pattern => (
                          <Card
                            key={pattern.type}
                            style={{ border: '1px solid #e5e7eb' }}
                    ...
                    <div style={{ display: 'grid', gap: '16px' }}>
                      <div>
                        ...
                        <div style={{ display: 'grid', gap: '4px' }}>
                          {taskAnalysis.bottleneck_analysis.critical_paths.map(
                            (path, index) => (
                              <div
                                key={index}
                                style={{
                      ...
                      <div>
                        ...
                        <div style={{ display: 'grid', gap: '4px' }}>
                          {taskAnalysis.bottleneck_analysis.recommendations.map(
                            (rec, index) => (
                              <div
                                key={index}
                                style={{
                  ...
                  >
                    ...
                    <Card style={{ border: '1px solid #e5e7eb' }}>
                      <div style={{ padding: '12px' }}>
                        ...
                        <div style={{ fontWeight: 'bold' }}>
                          {clusterStatus.healthy_nodes}/
                          {clusterStatus.total_nodes} 健康
                        </div>
                      </div>
                    </Card>
                    <Card style={{ border: '1px solid #e5e7eb' }}>
                      <div style={{ padding: '12px' }}>
                        ...
                        <div
                          style={{
                            fontWeight: 'bold',
                            color:
                              clusterStatus.consensus_state === 'stable'
                                ? '#10b981'
                                : '#ef4444',
                          }}
                        >
                  ...
                  <div style={{ display: 'grid', gap: '12px' }}>
                    ...
                    >
                      ...
                      <span style={{ fontWeight: 'bold' }}>
                        {new Date(clusterStatus.last_election).toLocaleString()}
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontWeight: 'bold',
                          color: clusterStatus.network_partitions
                            ? '#ef4444'
                            : '#10b981',
                        }}
                      >
                        {clusterStatus.network_partitions
                          ? '存在分区'
                          : '网络正常'}
                      </span>
                    </div>
                    ...
                    >
                      <span>数据复制延迟:</span>
                      <span style={{ fontWeight: 'bold' }}>
                        {clusterStatus.data_replication_lag}ms
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            <Card>
              <div style={{ padding: '16px' }}>
                ...
                <Card
                  style={{ border: '1px solid #e5e7eb', marginTop: '16px' }}
                >
                  <div style={{ padding: '16px' }}>
                    <h4 style={{ fontWeight: 'bold', marginBottom: '12px' }}>
                      智能任务重调度
