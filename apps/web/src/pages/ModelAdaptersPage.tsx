  ...
  const handleCreateAdapter = async () => {
    try {
      ...
      const loraConfig = {
        ...
        target_modules: values.targetModules?.length
          ? values.targetModules
          : undefined,
  ...
  const columns = [
      ...
      render: (text: string, record: any) => (
        <div>
          ...
          <Text type="secondary" style={{ fontSize: '12px' }}>
            {record.config?.model_name ||
              record.config?.model_architecture ||
              ''}
          </Text>
        </div>
      ),
    ...
    },
    {
      ...
      render: (_: string, record: TrainingJob) => {
        ...
        const color =
          type === 'qlora' ? 'purple' : type === 'lora' ? 'blue' : 'default'
      ...
      render: (record: any) => (
        <Space>
          <Button
            size="small"
            icon={<EditOutlined />}
            onClick={() => handleViewAdapter(record)}
          >
            查看
          </Button>
          <Button
            size="small"
            icon={<ExportOutlined />}
            onClick={() => handleDownloadAdapter(record.job_id)}
          >
            导出
          </Button>
          <Button
            ...
            size="small"
            icon={<DeleteOutlined />}
            onClick={() => handleDeleteAdapter(record.job_id)}
          >
            删除
          </Button>
        </Space>
                  ...
                  <Form form={form} layout="vertical">
                    ...
                    <Form.Item label="Dropout" name="dropout">
                      <div>
                        ...
                        />
                        <Text type="secondary">防止过拟合的正则化参数</Text>
                      </div>
                    </Form.Item>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <Tag color={modeColor}>{trainingMode || '-'}</Tag>
                        </div>
                  ...
                  {resourceMetrics && (
                    <>
                      ...
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <Progress
                          percent={resourceMetrics.memory}
                          strokeColor="#1890ff"
                        />
                      </div>
                      ...
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <Progress
                          percent={resourceMetrics.disk}
                          strokeColor="#52c41a"
                        />
                      </div>

                      <Descriptions bordered size="small">
                        <Descriptions.Item label="网络吞吐">
                          {resourceMetrics.network.toFixed(2)} MB/s
                        </Descriptions.Item>
                        <Descriptions.Item label="请求速率">
                          {resourceMetrics.requestsRate.toFixed(2)} req/s
                        </Descriptions.Item>
