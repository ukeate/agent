...
const ACLProtocolManagementPage: React.FC = () => {
  ...
  const ruleColumns = [
    ...
    },
    {
      ...
      render: (priority: number) => (
        <Tag
          color={priority >= 200 ? 'red' : priority >= 150 ? 'orange' : 'blue'}
        >
          {priority}
        </Tag>
      ),
  ...
  const groupColumns = [
    ...
    },
    {
      title: '关联规则',
      key: 'rules',
      render: (record: ACLGroup) => <Text>{record.rules.length} 条规则</Text>,
    },
    {
      title: '创建时间',
      dataIndex: 'createdAt',
  ...
  const auditColumns = [
    ...
    },
    {
      ...
      render: (event: string) => (
        <Tag
          color={
            event.includes('granted')
              ? 'green'
              : event.includes('denied')
                ? 'red'
                : event.includes('created') || event.includes('updated')
                  ? 'blue'
                  : 'orange'
          }
        >
        ...
        <div>
          ...
          <Row gutter={[16, 8]}>
            ...
            <Col span={8}>
              <Text strong>状态: </Text>
              <Badge
                status={rule.status === 'active' ? 'success' : 'default'}
                text={rule.status}
              />
            </Col>
            <Col span={12}>
  ...
  const filteredRules = aclRules.filter(rule => {
    const matchesPrincipal =
      !filterPrincipal ||
      rule.source.toLowerCase().includes(filterPrincipal.toLowerCase())
    const matchesResource =
      !filterResource ||
      rule.target.toLowerCase().includes(filterResource.toLowerCase())
    return matchesPrincipal && matchesResource
  })

  const getSecurityStatus = () => {
    if (!securityMetrics)
      return { status: 'success', text: '安全', color: 'green' }

    const totalRequests =
      securityMetrics.allowed_requests + securityMetrics.blocked_requests
    const blockRate =
      totalRequests > 0 ? securityMetrics.blocked_requests / totalRequests : 0

    if (blockRate < 0.01)
      return { status: 'success', text: '安全', color: 'green' }
    if (blockRate < 0.05)
      return { status: 'warning', text: '警告', color: 'orange' }
          ...
          <Col xs={12} sm={6}>
            <Card>
              ...
              <div style={{ marginTop: '8px' }}>
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  拒绝率:{' '}
                  {securityMetrics.allowed_requests +
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="source" label="源" rules={[{ required: true }]}>
                <Input
                  placeholder="例如: task-agent-*, admin-group"
                  name="acl-rule-source"
                />
              </Form.Item>
            </Col>
            <Col span={12}>
              ...
              >
                <Input
                  placeholder="例如: agents.tasks.>, system.admin.*"
                  name="acl-rule-target"
                />
              </Form.Item>
            </Col>
          </Row>
