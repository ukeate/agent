...
type QAItem = { id: string; task_id?: string; issue?: string; severity?: string; created_at?: string };
...
const AnnotationQualityControlPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(buildApiUrl('/api/v1/training-data/annotation-tasks-issues'));
