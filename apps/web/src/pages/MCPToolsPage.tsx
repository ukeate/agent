        ...
        <Tabs>
          <TabPane tab="读取文件" key="read">
            ...
            <Button
              ...
              loading={loading}
              onClick={async () => {
                const values = await fileSystemForm.validateFields([
                  'read_path',
                  'read_encoding',
                ])
            ...
            <List.Item.Meta
              ...
              title={
                <Space>
                  <Tag
                    color={
                      item.server_type === 'filesystem'
                        ? 'blue'
                        : item.server_type === 'database'
                          ? 'green'
                          : 'orange'
                    }
                  >
              ...
              description={
                <div>
                  <div className="text-xs text-gray-500">
                    参数:{' '}
                    {JSON.stringify(item.arguments, null, 2).substring(0, 100)}
                  </div>
                  {item.error && (
                    <Alert type="error" message={item.error} className="mt-1" />
                  )}
                  {item.result && (
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
              ...
              <Statistic
                ...
                value={
                  metrics?.monitoring_stats.total_calls
                    ? (
                        (metrics.monitoring_stats.successful_calls /
                          metrics.monitoring_stats.total_calls) *
                        100
                      ).toFixed(1)
                    : 0
        ...
        {healthStatus && (
          <Row gutter={16} className="mb-4">
            {Object.entries(healthStatus.servers).map(
              ([serverType, status]) => (
        ...
        <Row gutter={16}>
          ...
          <Col span={8}>{renderExecutionHistory()}</Col>
        </Row>
      </div>
    </div>
  )
