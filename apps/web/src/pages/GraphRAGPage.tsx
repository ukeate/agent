...
import { Card, Button, Input, Select, Switch, Spin, Alert, Tabs, Tag, message, Statistic, Form, DatePicker, Modal, Space } from 'antd'
import {
...
const GraphRAGPage: React.FC = () => {
  ...
  const [retrievalMode, setRetrievalMode] = useState<'vector' | 'graph' | 'hybrid' | 'adaptive'>('hybrid')
  ...
  useEffect(() => {
    const loadRagSystemStatus = async () => {
      ...
      try {
        ...
        setRagStats({ ...(rawStats?.data || {}), timestamp: rawStats?.timestamp })
  ...
  const handleMultiSourceFusion = async (values: any) => {
    try {
      ...
      const fusion = await graphRAGService.multiSourceFusion({
        ...
        graph_results,
        reasoning_results,
        confidence_threshold: Number.isFinite(confidence_threshold) ? confidence_threshold : undefined,
      })
  ...
  // 获取性能统计
  const handleGetPerformanceStats = async (startTime?: string, endTime?: string) => {
    try {
      setStatsQueryLoading(true)
      const stats = await graphRAGService.getPerformanceStats(startTime, endTime)
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {/* RAG系统状态概览 */}
        <Card title={<span><DatabaseOutlined className="mr-2" />RAG系统状态</span>} className="mb-6">
          {statsLoading ? (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      <Statistic
                        title="成功"
                        value={ragStats.query_statistics?.successful_queries || 0}
                    ...
                    <div className="text-center p-3 bg-orange-50 rounded-lg">
                      <Statistic
                        title="平均响应"
                        value={ragStats.query_statistics?.average_response_time || 0}
                        suffix="ms"
                    ...
                    <div className="text-center p-3 bg-red-50 rounded-lg">
                      <Statistic
                        title="缓存命中率"
                        value={(ragStats.performance_metrics?.cache_hit_rate || 0) * 100}
                        suffix="%"
                        precision={1}
                    ...
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-sm font-semibold text-gray-900 mt-1">
                        {ragStats.timestamp ? new Date(ragStats.timestamp).toLocaleTimeString() : '-'}
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="text-center p-3 bg-green-50 rounded-lg">
                          ...
                          <Tag
                            color={ragHealth.status === 'healthy' ? 'green' : ragHealth.status === 'degraded' ? 'orange' : 'red'}
                            className="mt-2"
                          >
                        ...
                        <div className="text-center p-3 bg-blue-50 rounded-lg">
                          <Statistic
                            title="运行时间"
                            value={Math.floor((ragHealth.components?.api?.uptime_seconds || 0) / 3600)}
                            suffix="小时"
                        ...
                        <div className="text-center p-3 bg-purple-50 rounded-lg">
                          <Statistic
                            title="内存使用"
                            value={ragHealth.metrics?.process?.memory_rss_mb || 0}
                            suffix="MB"
                            valueStyle={{ fontSize: '18px' }}
                          />
                        </div>
                    ...
                    <div>
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2">
                          {Object.entries(ragHealth.components || {}).map(([component, detail]: any) => (
                            <div key={component} className="flex justify-between items-center">
                              <span className="text-gray-700 capitalize">
                                {component.replace('_', ' ')}
                              </span>
                              <Tag color={detail?.status === 'healthy' ? 'green' : detail?.status === 'degraded' ? 'orange' : 'red'}>
                        ...
                        <div className="mt-3 text-xs text-gray-500">
                          <div>活跃连接: {ragHealth.components?.api?.active_connections || 0}</div>
                          <div>最后检查: {ragHealth.timestamp ? new Date(ragHealth.timestamp).toLocaleString() : '-'}</div>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <Card title="高级功能" className="mb-6">
          <Tabs defaultActiveKey="analysis" type="card">
            <TabPane tab={<span><BulbOutlined />查询分析</span>} key="analysis">
              <div className="space-y-4">
                ...
                {queryAnalysis && (
                  <div className="space-y-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <p><strong>策略:</strong> {queryAnalysis.decomposition.decomposition_strategy}</p>
                      <p><strong>复杂度:</strong> {queryAnalysis.decomposition.complexity_score}</p>
                      <p><strong>子查询数:</strong> {queryAnalysis.analysis.sub_queries_count}</p>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg">
                      <h4 className="font-semibold mb-2">统计</h4>
                      <p><strong>检测类型:</strong> {queryAnalysis.analysis.detected_query_type}</p>
                      <p><strong>复杂度分数:</strong> {queryAnalysis.analysis.complexity_score}</p>
                      <p><strong>实体查询数:</strong> {queryAnalysis.analysis.entities_count}</p>
                      <p><strong>关系查询数:</strong> {queryAnalysis.analysis.relations_count}</p>
                    </div>
                    {queryAnalysis.decomposition.sub_queries?.length > 0 && (
                      <div className="bg-purple-50 p-4 rounded-lg">
                        <h4 className="font-semibold mb-2">子查询</h4>
                        <ul className="list-disc list-inside">
                          {queryAnalysis.decomposition.sub_queries.map((q: string, index: number) => (
                            <li key={index}>{q}</li>
                          ))}
                        </ul>
                      </div>
            ...
            <TabPane tab={<span><ShareAltOutlined />推理查询</span>} key="reasoning">
              <div className="space-y-4">
                <Button 
                  type="primary" 
            ...
            <TabPane tab={<span><BranchesOutlined />多源融合</span>} key="fusion">
              <div className="space-y-4">
                <Button 
                  type="primary" 
                ...
                {fusionResult && (
                  <div className="space-y-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <p><strong>输出文档:</strong> {fusionResult.statistics?.output_documents ?? 0}</p>
                      <p><strong>一致性:</strong> {(((fusionResult.fusion_results?.consistency_score as number) || 0) * 100).toFixed(1)}%</p>
                      <p><strong>融合策略:</strong> {String(fusionResult.fusion_results?.resolution_strategy || '-')}</p>
                      <p><strong>冲突数:</strong> {(fusionResult.fusion_results?.conflicts_detected || []).length}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <TabPane tab={<span><BarChartOutlined />性能监控</span>} key="performance">
              <div className="space-y-4">
                <div className="flex gap-4">
                  <Button 
                    type="primary" 
                ...
                {performanceStats && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <div className="space-y-2">
                        <p>总查询数: {performanceStats.query_statistics.total_queries}</p>
                        <p>成功查询: {performanceStats.query_statistics.successful_queries}</p>
                        <p>失败查询: {performanceStats.query_statistics.failed_queries}</p>
                        <p>平均响应时间: {performanceStats.query_statistics.average_response_time.toFixed(2)}ms</p>
                      </div>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg">
                      <h4 className="font-semibold mb-3">性能指标</h4>
                      <div className="space-y-2">
                        <p>平均检索时间: {performanceStats.performance_metrics.average_retrieval_time.toFixed(2)}ms</p>
                        <p>平均推理时间: {performanceStats.performance_metrics.average_reasoning_time.toFixed(2)}ms</p>
                        <p>平均融合时间: {performanceStats.performance_metrics.average_fusion_time.toFixed(2)}ms</p>
                        <p>缓存命中率: {(performanceStats.performance_metrics.cache_hit_rate * 100).toFixed(1)}%</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <TabPane tab={<span><BugOutlined />调试工具</span>} key="debug">
              <div className="space-y-4">
                <Button 
                  type="primary" 
                ...
                {debugResult && (
                  <div className="space-y-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <p><strong>查询:</strong> {debugResult.query}</p>
                      <p><strong>路径数:</strong> {debugResult.reasoning_paths_count}</p>
              ...
              <div className="space-y-4">
                ...
                {consistencyCheck && (
                  <div className="bg-green-50 p-4 rounded-lg mt-4">
                    ...
                    <p><strong>一致性得分:</strong> {(consistencyCheck.consistency_score * 100).toFixed(1)}%</p>
                    <p><strong>文档数:</strong> {consistencyCheck.documents_checked}</p>
                    <p><strong>查询ID:</strong> {consistencyCheck.query_id}</p>
                  </div>
                )}
              </div>
            </TabPane>
            ...
            <Tabs defaultActiveKey="answer">
              <TabPane tab="检索结果" key="answer">
                        ...
                        <div key={index} className="bg-white p-3 rounded border">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-sm text-gray-600">
                              分数: {(((doc.final_score ?? doc.score) || 0) * 100).toFixed(1)}%
                            </span>
                          </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  <Tag color="blue">
                    一致性: {(((result.fusion_results?.consistency_score as number) || 0) * 100).toFixed(1)}%
                  </Tag>
                  <Tag color="green">
                    用时: {(((result.performance_metrics.total_time as number) || 0) * 1000).toFixed(0)}ms
                  </Tag>
                  <Tag color="purple">
              ...
              {includeReasoning && result.reasoning_results.length > 0 && (
                <TabPane tab={`推理路径 (${result.reasoning_results.length})`} key="reasoning">
                  <div className="space-y-4">
              ...
              <TabPane tab="性能指标" key="performance">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center p-4 bg-blue-50 rounded">
                    <div className="text-2xl font-bold text-blue-600">
                      {(((result.performance_metrics.total_time as number) || 0) * 1000).toFixed(0)}ms
                    </div>
                    <div className="text-sm text-gray-600">总用时</div>
                  </div>
                  <div className="text-center p-4 bg-green-50 rounded">
                    <div className="text-2xl font-bold text-green-600">
                      {(((result.performance_metrics.retrieval_time as number) || 0) * 1000).toFixed(0)}ms
                    </div>
                    <div className="text-sm text-gray-600">检索用时</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded">
                    <div className="text-2xl font-bold text-purple-600">
                      {(((result.performance_metrics.reasoning_time as number) || 0) * 1000).toFixed(0)}ms
                    </div>
                    <div className="text-sm text-gray-600">推理用时</div>
                  </div>
                  <div className="text-center p-4 bg-orange-50 rounded">
                    <div className="text-2xl font-bold text-orange-600">
                      {(((result.performance_metrics.fusion_time as number) || 0) * 1000).toFixed(0)}ms
                    </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    <div className="flex justify-between items-center mb-2">
                      <span>策略: {String(result.fusion_results?.resolution_strategy || '-')}</span>
                      <span>一致性: {(((result.fusion_results?.consistency_score as number) || 0) * 100).toFixed(1)}%</span>
                    </div>
                    <div className="text-sm text-gray-600">
                      冲突数: {(result.fusion_results?.conflicts_detected || []).length}
                    </div>
                  </div>
                </div>
              </TabPane>
        ...
        >
          ...
          >
            ...
            <Form.Item>
              <Space>
                <Button type="primary" htmlType="submit" loading={reasoningLoading}>
                  开始推理
                </Button>
        ...
        >
          <Form
            layout="vertical"
            onFinish={handleMultiSourceFusion}
          >
            <Form.Item
              name="vector_results"
              label="向量检索结果 (JSON数组)"
            ...
            >
              <TextArea
                ...
                placeholder={JSON.stringify(
                  [
                    { content: '示例内容', score: 0.82, metadata: { source: 'vector_store' } },
                  ],
                  null,
                  2
                )}
            ...
            >
              <TextArea rows={4} placeholder={JSON.stringify({ entities: [], relations: [] }, null, 2)} />
            </Form.Item>
            <Form.Item
            ...
            <Form.Item name="confidence_threshold" label="置信度阈值" initialValue={0.6}>
              <Input type="number" min={0} max={1} step={0.05} />
            </Form.Item>
            <Form.Item>
              <Space>
                <Button type="primary" htmlType="submit" loading={fusionLoading}>
                  开始融合查询
                </Button>
        ...
        >
          ...
          >
            ...
            <Form.Item>
              <Space>
                <Button type="primary" htmlType="submit" loading={benchmarkLoading}>
                  开始测试
                </Button>
        ...
        >
          <Form
            layout="vertical"
            onFinish={handlePerformanceComparison}
          >
            <Form.Item
              name="baselineDate"
              label="基线日期"
            ...
            <Form.Item>
              <Space>
                <Button type="primary" htmlType="submit" loading={comparisonLoading}>
                  开始对比
                </Button>
