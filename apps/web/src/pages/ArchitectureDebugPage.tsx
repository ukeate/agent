...
const ArchitectureDebugPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const nextComponents: Component[] = componentEntries.map(
        ([name, data]) => {
          const responseTime =
            typeof (data as any).response_time_ms === 'number'
              ? (data as any).response_time_ms
              : typeof (data as any).response_time === 'number'
                ? (data as any).response_time
                : null
          const uptimeSeconds =
            typeof (data as any).uptime_seconds === 'number'
              ? (data as any).uptime_seconds
              : null
          const componentType: Component['type'] =
            name.includes('database') || name.includes('redis')
              ? 'database'
              : name === 'api'
                ? 'api'
                : 'service'
      ...
      const sessions: DebugSession[] = alerts.map(
    ...
    const groups: Array<{ key: string; title: string; items: Component[] }> = [
      {
        key: 'api',
        title: 'API层',
        items: components.filter(c => c.type === 'api'),
      },
      {
        key: 'services',
        title: '服务层',
        items: components.filter(c => c.type === 'service'),
      },
      {
        key: 'database',
        title: '数据层',
        items: components.filter(c => c.type === 'database'),
      },
  ...
  const debugColumns = [
    ...
    },
    {
      ...
      render: (status: keyof typeof statusSessionColors) => (
        <Badge
          status={statusSessionColors[status]}
          text={
            status === 'investigating'
              ? '调查中'
              : status === 'resolved'
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Space>
            <Button icon={<BugOutlined />}>开始调试会话</Button>
            <Button
              icon={<ReloadOutlined />}
              onClick={loadData}
              loading={loading}
                ...
                <Card title={`${selectedComponent.name} 详情`}>
                  <Descriptions column={2} bordered size="small">
                    ...
                    <Descriptions.Item label="响应时间">
                      {selectedComponent.metrics.responseTime !== null
                        ? `${selectedComponent.metrics.responseTime}ms`
                        : '-'}
                    </Descriptions.Item>
                    <Descriptions.Item label="错误率">
                      {selectedComponent.metrics.errorRate !== null
