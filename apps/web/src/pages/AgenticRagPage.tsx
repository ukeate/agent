...
const AgenticRagPage: React.FC = () => {
  ...
  // 策略选择
  const [expansionStrategies, setExpansionStrategies] = useState<
    ExpansionStrategyType[]
  >([ExpansionStrategyType.SEMANTIC, ExpansionStrategyType.CONTEXTUAL])
  const [retrievalStrategies, setRetrievalStrategies] = useState<
    RetrievalStrategyType[]
  >([RetrievalStrategyType.HYBRID, RetrievalStrategyType.KEYWORD])

  // 查询结果
  const [queryResult, setQueryResult] = useState<AgenticQueryResponse | null>(
    null
  )
            ...
            <Panel
              ...
              header={
                <div>
                  ...
                  <span className="ml-2">
                    置信度: {(eq.confidence * 100).toFixed(1)}%
                  </span>
                </div>
              }
            >
        ...
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            ...
            <Progress
              ...
              strokeColor={
                validation.overall_quality > 0.7 ? '#52c41a' : '#faad14'
              }
            />
          </div>
          <div>
        ...
        <Timeline>
          {streamEvents.map((event, idx) => (
            <Timeline.Item
              key={idx}
              color={
                event.event_type === StreamEventType.ERROR ? 'red' : 'blue'
              }
              dot={
      ...
      <div className="mb-6">
        ...
        <Card className="mb-4">
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Row gutter={16}>
              <Col span={12}>
                ...
                <Select
                  ...
                  disabled={loading}
                >
                  <Option value={ExpansionStrategyType.SEMANTIC}>
                    语义扩展
                  </Option>
                  <Option value={ExpansionStrategyType.SYNONYM}>
                    同义词扩展
                  </Option>
                  <Option value={ExpansionStrategyType.CONTEXTUAL}>
                    上下文扩展
                  </Option>
                  <Option value={ExpansionStrategyType.MULTILINGUAL}>
                    多语言扩展
                  </Option>
                  <Option value={ExpansionStrategyType.DECOMPOSITION}>
                    问题分解
                  </Option>
                </Select>
              </Col>
              <Col span={12}>
                ...
                >
                  ...
                  <Option value={RetrievalStrategyType.SEMANTIC}>
                    语义检索
                  </Option>
                  <Option value={RetrievalStrategyType.KEYWORD}>
                    关键词检索
                  </Option>
                  <Option value={RetrievalStrategyType.STRUCTURED}>
                    结构化检索
                  </Option>
                </Select>
              </Col>
            ...
            {streaming && <Progress percent={streamProgress} status="active" />}
          </Space>
        </Card>
                ...
                {queryResult?.success && (
                  <Card title="结果评价" size="small">
                    <div className="text-center">
                      ...
                      <Rate
                        onChange={value =>
                          handleFeedback(queryResult.query_id, value)
                        }
                      />
                    </div>
                  </Card>
