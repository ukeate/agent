...
const ModelCompressionEvaluationPage: React.FC = () => {
  ...
  const onLoad = async (values: any) => {
    ...
    try {
      ...
      const [r, rep] = await Promise.allSettled([
        loadResult(jobId),
        loadReport(jobId),
      ])
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Paragraph style={{ marginTop: 8 }}>
        通过 job_id 查询压缩结果与评估报告：`/api/v1/model-compression/results/
        {'{job_id}'}` 与 `/api/v1/model-compression/results/{'{job_id}'}
        /report`。
      </Paragraph>
      ...
      <Card title="查询" style={{ marginBottom: 16 }}>
        <Form form={form} layout="inline" onFinish={onLoad}>
          <Form.Item
            name="job_id"
            rules={[{ required: true, message: '请输入 job_id' }]}
          >
