    ...
    const parameterRelations = paramImportance.length >= 2 ? [{
      ...
      values: trials
        .filter(trial => 
          trial.parameters && 
          trial.value !== undefined &&
          trial.parameters[paramImportance[0].param_name] !== undefined &&
          trial.parameters[paramImportance[1].param_name] !== undefined
        )
        .map(trial => ({
      ...
      <Card className="p-6">
        ...
        <div className="h-64 relative">
          <svg className="w-full h-full" viewBox="0 0 800 200">
            {/* 坐标轴 */}
            <line x1="50" y1="170" x2="750" y2="170" stroke="#e5e5e5" strokeWidth="2"/>
            <line x1="50" y1="20" x2="50" y2="170" stroke="#e5e5e5" strokeWidth="2"/>
            ...
            {chartData.trials.map((trial, index) => {
              ...
              const bestY = 170 - ((trial.best_value_so_far - minValue) / valueRange) * 150;
              ...
              return (
                <g key={index}>
                  ...
                  {index > 0 && (
                    <line
                      x1={50 + ((index - 1) / (chartData.trials.length - 1)) * 700}
                      y1={170 - ((chartData.trials[index - 1].best_value_so_far - minValue) / valueRange) * 150}
                      x2={x}
                      y2={bestY}
                      stroke="#3b82f6"
                      strokeWidth="2"
            ...
            <g transform="translate(600, 30)">
              ...
              <text x="10" y="3" fontSize="12" fill="#666">试验值</text>
              <circle cx="0" cy="20" r="2" fill="#3b82f6"/>
              <text x="10" y="23" fontSize="12" fill="#666">最佳值</text>
            </g>
          </svg>
        </div>
      ...
      <Card className="p-6">
        ...
        <div className="h-64 relative">
          <svg className="w-full h-full" viewBox="0 0 300 200">
            {/* 坐标轴 */}
            <line x1="40" y1="170" x2="270" y2="170" stroke="#e5e5e5" strokeWidth="2"/>
            <line x1="40" y1="20" x2="40" y2="170" stroke="#e5e5e5" strokeWidth="2"/>
            
            {/* 数据点 */}
            {relation.values.map((point, index) => {
              const x = 40 + ((point.x - xMin) / (xMax - xMin)) * 230;
              const y = 170 - ((point.y - yMin) / (yMax - yMin)) * 150;
              const colorIntensity = (point.objective - objMin) / (objMax - objMin);
            ...
            <text x="15" y="100" textAnchor="middle" fontSize="12" fill="#666" transform="rotate(-90, 15, 100)">
              {relation.param2}
            </text>
          </svg>
        </div>
        <div className="flex justify-center mt-4">
          <div className="flex items-center space-x-2 text-sm">
            <div className="w-4 h-4 rounded" style={{background: 'linear-gradient(to right, hsl(0, 70%, 50%), hsl(120, 70%, 50%))'}}></div>
      ...
      <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {experiments.map((experiment) => (
                ...
                >
                  ...
                  <h3 className="font-medium text-gray-900 mb-1">{experiment.name}</h3>
            ...
            {visualizations && (
              <Card className="p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Button
                    variant="outline"
                    onClick={() => window.open(visualizations.optimization_history, '_blank')}
                  ...
                  <Button
                    variant="outline"
                    onClick={() => window.open(visualizations.param_importance, '_blank')}
                    className="h-20 flex-col"
                  >
                  ...
                  <Button
                    variant="outline"
                    onClick={() => window.open(visualizations.parallel_coordinate, '_blank')}
                    className="h-20 flex-col"
                  >
                  ...
                  <Button
                    variant="outline"
                    onClick={() => window.open(visualizations.contour, '_blank')}
                    className="h-20 flex-col"
                  >
        ...
        {!selectedExperiment && experiments.length > 0 && (
          <Card className="p-8 text-center">
            <div className="text-gray-500 mb-2">请选择一个实验来查看可视化分析</div>
