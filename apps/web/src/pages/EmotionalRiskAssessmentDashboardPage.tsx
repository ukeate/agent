...
type Risk = { assessment_id: string; user_id?: string; risk_level?: string; risk_score?: number; timestamp?: string };
...
const EmotionalRiskAssessmentDashboardPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(buildApiUrl('/api/v1/emotional-intelligence/risk-assessment'));
