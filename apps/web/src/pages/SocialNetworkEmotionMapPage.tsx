...
const SocialNetworkEmotionMapPage: React.FC = () => {
  const [currentNetwork, setCurrentNetwork] = useState<EmotionNetwork | null>(
    null
  )
  const [networkAnalytics, setNetworkAnalytics] =
    useState<NetworkAnalytics | null>(null)
  const [loading, setLoading] = useState(false)
  const [selectedLayout, setSelectedLayout] = useState('force')
  const [selectedNode, setSelectedNode] = useState<string | null>(null)
  const [showBuildModal, setShowBuildModal] = useState(false)
  const [viewMode, setViewMode] = useState<
    'network' | 'clusters' | 'influence'
  >('network')
  ...
  const computeNetworkAnalytics = (
    ...
    const averageStrength = totalConnections
      ? edgeEntries.reduce((sum, [, weight]) => sum + weight, 0) /
        totalConnections
      : 0
    ...
    const clusteringCoefficient = clusteringCoefficients.length
      ? clusteringCoefficients.reduce((sum, value) => sum + value, 0) /
    ...
    nodeIds.forEach(id => {
      ...
      const maxDistance = Object.values(distances).reduce(
    ...
    const avgDominant = dominantScores.length
      ? dominantScores.reduce((sum, value) => sum + value, 0) /
        dominantScores.length
      : 0
    const variance = dominantScores.length
      ? dominantScores.reduce(
          (sum, value) => sum + (value - avgDominant) ** 2,
          0
        ) / dominantScores.length
      : 0
    ...
    return {
      ...
      stability_score: Math.max(
        0,
        Math.min(1, 1 - (network.polarization_level || 0))
      ),
    }
  }
  ...
  const loadNetworkData = async () => {
    ...
    try {
      const networkResult =
        await socialNetworkApi.buildEmotionNetwork(lastBuildPayload)
  ...
  const renderNetwork = () => {
    ...
    const links = Object.entries(currentNetwork.edges).map(
      ([edgeKey, weight]) => {
    ...
    node
      ...
      .on('mouseover', function (event, d: any) {
        ...
        const tooltip = svg
          ...
          .attr(
            'transform',
            `translate(${d.x + margin.left + 20},${d.y + margin.top})`
          )
  ...
  const drawInfluencePaths = (
    g: any,
    influencePaths: Record<string, string[][]>,
    nodes: any[]
  ) => {
    ...
    try {
      const payload = {
        ...
        social_context: {
          ...
          interaction_history: parseJsonInput(
        ...
        <Card>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 'bold', color: '#722ed1' }}>
              {currentNetwork
                ? Math.round(currentNetwork.network_cohesion * 100)
                : 0}
              %
            </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {selectedNode && (
            ...
            >
              ...
              <div>
                <Text strong>角色:</Text>{' '}
                {currentNetwork.nodes[selectedNode]?.role}
              </div>
              <div>
                <Text strong>影响力:</Text>
                <Progress
                  percent={Math.round(
                    (currentNetwork.nodes[selectedNode]?.emotional_influence ||
                      0) * 100
                  )}
                  size="small"
                  style={{ marginTop: 4 }}
  ...
  const renderNodeList = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'emotion_state',
        render: (state: any) => {
          const dominantEmotion = Object.entries(state.emotions).sort(
  ...
  const renderInfluencePaths = () => {
    ...
    return (
      ...
      >
        {Object.entries(currentNetwork.influence_paths).map(
          ([influencer, paths]) => (
      ...
      {networkAnalytics ? (
        <div>
          <Row gutter={[16, 16]}>
            ...
            <Col span={12}>
              <div>
                ...
                <Progress
                  percent={Math.round(
                    networkAnalytics.clustering_coefficient * 100
                  )}
                  size="small"
                  style={{ marginTop: 4 }}
                  strokeColor="#52c41a"
          ...
          <div>
            ...
            <span
              style={{ marginLeft: 8, fontSize: '16px', fontWeight: 'bold' }}
            >
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ marginTop: 8 }}>
              {Object.entries(networkAnalytics.influence_distribution).map(
                ([level, value]) => (
  ...
  const renderBuildModal = () => (
    <Modal
      ...
      width={600}
    >
      <Form form={buildForm} layout="vertical" onFinish={buildNewNetwork}>
        <Alert
          message="社交网络构建"
        ...
        <Form.Item label="文化背景" name="cultural_context">
