...
const empathyApi = {
  ...
  ): Promise<{ success: boolean; data?: EmpathyResponse; error?: string }> {
    try {
          ...
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <div>
              ...
              <Select
                ...
                onChange={value =>
                  setEmotionForm({ ...emotionForm, emotion: value })
                }
                allowClear
              >
            ...
            <div>
              <Text strong>
                情感强度: {Math.round(emotionForm.intensity * 100)}%
              </Text>
              <Slider
                style={{ marginTop: 8 }}
                value={emotionForm.intensity}
                onChange={value =>
                  setEmotionForm({ ...emotionForm, intensity: value })
                }
                min={0}
                max={1}
                step={0.1}
          ...
          <Space direction="vertical" style={{ width: '100%' }}>
            <div>
              ...
              <Select
                ...
                onChange={value =>
                  setRequest({ ...request, preferred_empathy_type: value })
                }
                allowClear
              >
            ...
            <div>
              ...
              <Select
                ...
                onChange={value =>
                  setRequest({ ...request, cultural_context: value })
                }
                allowClear
              >
            ...
            <div>
              ...
              <Slider
                ...
                onChange={value =>
                  setRequest({ ...request, max_response_length: value })
                }
                min={50}
                max={500}
                step={10}
            ...
            <div>
              <Text strong>
                紧急程度: {Math.round(request.urgency_level * 100)}%
              </Text>
              <Slider
                style={{ marginTop: 8 }}
                value={request.urgency_level}
                onChange={value =>
                  setRequest({ ...request, urgency_level: value })
                }
                min={0}
                max={1}
                step={0.1}
        ...
        {strategies.available_strategies.map((strategy: Strategy) => (
          <Col key={strategy.type} span={8}>
            <Card
              title={
                <span>
                  <Tag color={getEmpathyTypeColor(strategy.type)}>
                    {strategy.type}
                  </Tag>
                  {strategy.name}
                </span>
    ...
    return (
      <Row gutter={24}>
          ...
          <Card title="系统性能">
            <Space direction="vertical" style={{ width: '100%' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Text>总请求数:</Text>
                <Text strong>
                  {system_performance.total_requests.toLocaleString()}
                </Text>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Text>成功率:</Text>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Text>平均响应时间:</Text>
                <Text strong>
                  {Math.round(system_performance.average_response_time)}ms
                </Text>
              </div>
            </Space>
          </Card>
        ...
        <Col span={12}>
          <Card title="策略使用分布">
            <Space direction="vertical" style={{ width: '100%' }}>
              {Object.entries(system_performance.strategy_usage).map(
                ([strategy, count]) => (
                  <div
                    key={strategy}
                    style={{
                  ...
                  >
                    <Tag color={getEmpathyTypeColor(strategy)}>{strategy}</Tag>
            ...
            <Timeline.Item key={resp.id} color={index === 0 ? 'green' : 'blue'}>
              <div>
                ...
                >
                  ...
                  <Text type="secondary">{resp.generation_time_ms}ms</Text>
                </div>
