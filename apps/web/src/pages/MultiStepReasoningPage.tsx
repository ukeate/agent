...
const MultiStepReasoningPage: React.FC = () => {
  ...
  const [executionStatus, setExecutionStatus] = useState<
    'idle' | 'decomposing' | 'scheduling' | 'executing' | 'completed' | 'failed'
  >('idle')
  ...
  const [resultsExecutionId, setResultsExecutionId] = useState<string | null>(
    null
  )
  ...
  // 获取系统指标
  useEffect(() => {
    const stop = multiStepReasoningApi.monitorSystemMetrics(metrics => {
  ...
  useEffect(() => {
    ...
    const executionId = currentExecution?.execution_id
    if (!executionId) return
    if (resultsExecutionId === executionId && (executionResults || resultsError))
      return
  ...
  const handleStartDecomposition = async () => {
    ...
    try {
      ...
      // 开始轮询执行状态
      pollStopRef.current = multiStepReasoningApi.pollExecutionStatus(
  ...
  const loadExecutionResults = useCallback(
    async (executionId: string) => {
      ...
      try {
        const results = await multiStepReasoningApi.getExecutionResults(
          executionId
        )
        setExecutionResults(results)
      } catch (error) {
        logger.error('获取执行结果失败:', error)
        setResultsError('获取执行结果失败')
      } finally {
  ...
  const exportPayload = executionResults
    ? { execution: currentExecution, results: executionResults }
    : currentExecution
            ...
            <Card title="执行控制">
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <div>
                  ...
                  <Badge
                    status={getStepStatusColor(currentExecution.status)}
            ...
            <Card title="任务依赖图 (DAG)" style={{ marginBottom: '24px' }}>
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                >
                  ...
                  <Tag color="blue">
                    关键路径: {taskDAG.critical_path?.length || 0} 步
                  </Tag>
                </div>
                      ...
                      <Col span={12} key={node.id}>
                        <Card
                          ...
                          style={{
                            cursor: 'pointer',
                            borderColor:
                              selectedStep === node.id ? '#1890ff' : undefined,
                          }}
                        >
                          <Space direction="vertical" size="small">
                            ...
                            <Tag
                              size="small"
                              color={getStepStatusColor(node.status)}
                            >
                              {node.task_type}
                            </Tag>
                            <Text style={{ fontSize: '11px', color: '#666' }}>
                              难度: {node.complexity_score} | 耗时:{' '}
                ...
                {selectedStep && (
                  <Card size="small" title="步骤详情">
                    {(() => {
                      const step = taskDAG.nodes?.find(
                        (n: any) => n.id === selectedStep
                      )
                      return step ? (
                        <div>
                          <p>
                            <Text strong>步骤ID:</Text> {step.id}
                          </p>
                          <p>
                            <Text strong>类型:</Text> {step.task_type}
                          </p>
                          <p>
                            <Text strong>状态:</Text>{' '}
                            <Badge
                              status={getStepStatusColor(step.status)}
                              text={step.status}
                            />
                          </p>
                          <p>
                            <Text strong>复杂度:</Text> {step.complexity_score}
                          </p>
                          <p>
            ...
            {systemMetrics ? (
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="rounded border bg-white p-4">
                  ...
                  <div className="text-2xl font-semibold">
                    {systemMetrics.queue_depth}
                  </div>
                </div>
                <div className="rounded border bg-white p-4">
                  <div className="text-gray-600">平均等待</div>
                  <div className="text-2xl font-semibold">
                    {systemMetrics.average_wait_time.toFixed(1)}s
                  </div>
                </div>
                <div className="rounded border bg-white p-4">
      ...
      {executionStatus === 'completed' && (
        <Card
          ...
          extra={
            <Button
              icon={<ReloadOutlined />}
              disabled={!currentExecution?.execution_id || resultsLoading}
              onClick={() => {
                if (currentExecution?.execution_id) {
                  loadExecutionResults(currentExecution.execution_id)
                }
              }}
            >
              刷新结果
        ...
        >
          <Tabs defaultActiveKey="summary">
              ...
              {resultsLoading ? (
                ...
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  <Alert
                    message="工作流执行完成"
                    description={
                      executionResults.results.summary || '所有任务已完成。'
                    }
                    type="success"
                    showIcon
                  />
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded border bg-white p-4">
                      <div className="text-gray-600">验证得分</div>
                      <div className="text-2xl font-semibold">
                        {typeof executionResults.results.validation_score ===
                        'number'
                          ? `${executionResults.results.validation_score.toFixed(1)}/100`
                          : '-'}
                      </div>
                    </div>
                    <div className="rounded border bg-white p-4">
                      ...
                      <div className="text-2xl font-semibold">
                        {typeof executionResults.performance_metrics
                          .total_duration === 'number'
                          ? `${executionResults.performance_metrics.total_duration.toFixed(1)}s`
                          : '-'}
                      </div>
                    </div>
                    <div className="rounded border bg-white p-4">
                      <div className="text-gray-600">平均步骤耗时</div>
                      <div className="text-2xl font-semibold">
                        {typeof executionResults.performance_metrics
                          .average_step_duration === 'number'
                          ? `${executionResults.performance_metrics.average_step_duration.toFixed(1)}s`
                          : '-'}
                      </div>
                    </div>
                    <div className="rounded border bg-white p-4">
                      <div className="text-gray-600">并行效率</div>
                      <div className="text-2xl font-semibold">
                        {typeof executionResults.performance_metrics
                          .parallelization_efficiency === 'number'
                          ? `${Math.round(executionResults.performance_metrics.parallelization_efficiency * 100)}%`
                          : '-'}
                      </div>
                    </div>
                  </div>
            ...
            <TabPane tab="验证报告" key="validation">
              {executionResults ? (
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  ...
                  <Text type="secondary">综合结果</Text>
                  <Text code>
                    {JSON.stringify(
                      executionResults.results.aggregated_result,
                      null,
                      2
                    ) || '暂无数据'}
                  </Text>
            ...
            <TabPane tab="原始数据" key="raw">
              <Text code>{exportText || '暂无数据'}</Text>
            </TabPane>
              ...
              <Space>
                ...
                <Button
                  ...
                  onClick={() =>
                    download(
                      'execution.xml',
                      `<execution><![CDATA[${exportText || ''}]]></execution>`,
                      'application/xml'
                    )
                  }
                >
