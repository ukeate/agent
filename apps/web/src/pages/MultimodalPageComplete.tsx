...
const MultimodalPageComplete: React.FC = () => {
  ...
  const [processingPriority, setProcessingPriority] = useState<ModelPriority>(ModelPriority.BALANCED);
  const [processingComplexity, setProcessingComplexity] = useState<ModelComplexity>(ModelComplexity.MEDIUM);
  ...
  const getTokenUsage = (tokensUsed?: number | TokenUsage) => {
    ...
    const totalTokens = typeof tokensUsed.total_tokens === 'number'
      ? tokensUsed.total_tokens
      : (typeof tokensUsed.prompt_tokens === 'number' ? tokensUsed.prompt_tokens : 0)
        + (typeof tokensUsed.completion_tokens === 'number' ? tokensUsed.completion_tokens : 0);
    const inputTokens = typeof tokensUsed.prompt_tokens === 'number'
      ? tokensUsed.prompt_tokens
      : (typeof tokensUsed.completion_tokens === 'number' && totalTokens > 0 ? 0 : Math.floor(totalTokens / 3));
    const outputTokens = typeof tokensUsed.completion_tokens === 'number'
  ...
  const checkProcessingStatus = async () => {
    ...
    const processingTaskIds = tasks
      .filter(task => task.status === ProcessingStatus.PROCESSING || task.status === ProcessingStatus.PENDING)
  ...
  useEffect(() => {
    const loadModelConfigs = async () => {
      ...
      try {
        ...
        if (configs.length > 0 && !configs.find((config) => config.name === selectedModel)) {
  ...
  const calculateCostForTask = (status: ProcessingStatusResponse): number => {
    ...
    return multimodalService.calculateCost(status.model_used, inputTokens, outputTokens);
  };
  ...
  const handleUpload = async (file: File): Promise<boolean> => {
    try {
      ...
      setProcessingTasks(prev => new Map(prev).set(uploadResult.content_id, task));
      ...
      const processResult = await multimodalService.processContent(processRequest);
  ...
  const handleQuickAnalysis = async () => {
    ...
    try {
        ...
        content: (
          <div>
            <Descriptions column={1} bordered size="small">
              ...
              <Descriptions.Item label="模型">{result.model_used}</Descriptions.Item>
              <Descriptions.Item label="Token使用">{getTokenUsage(result.tokens_used).totalTokens}</Descriptions.Item>
              <Descriptions.Item label="成本">${result.cost.toFixed(4)}</Descriptions.Item>
              <Descriptions.Item label="处理时间">{result.processing_time}秒</Descriptions.Item>
            </Descriptions>
          </div>
        )
      });
      ...
      setCostStats(prev => ({
        ...
        totalTokens: prev.totalTokens + getTokenUsage(result.tokens_used).totalTokens
      }));
    } catch (error: any) {
  ...
  const queueColumns = [
    {
      title: batchMode && <Checkbox onChange={(e) => {
        if (e.target.checked) {
      ...
      key: 'select',
      width: 50,
      render: (_: any, record: ProcessingTask) => batchMode && (
        <Checkbox
          ...
          onChange={(e) => {
            if (e.target.checked) {
              ...
              setSelectedFiles(prev => prev.filter(id => id !== record.contentId));
            }
          }}
        />
      )
                  ...
                  content: (
                    <Descriptions column={1} bordered size="small">
                      <Descriptions.Item label="提取数据">
                        <pre>{JSON.stringify(record.result.extracted_data, null, 2)}</pre>
                      </Descriptions.Item>
  ...
  const calculateStats = () => {
    ...
    return {
      total: tasks.length,
      completed: tasks.filter(t => t.status === ProcessingStatus.COMPLETED).length,
      failed: tasks.filter(t => t.status === ProcessingStatus.FAILED).length,
      processing: tasks.filter(t => t.status === ProcessingStatus.PROCESSING).length,
            ...
            <Col span={12}>
              ...
              <Card title="文件上传">
                ...
                <div className="mt-4">
                  <Alert
                    message="支持的格式"
                    description={multimodalService.getSupportedFileTypes().join(', ')}
                    type="info"
                    showIcon
                  />
                </div>
            ...
            <Col span={12}>
              <Card title="快速图像分析" className="mb-4">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  {quickAnalysisFile && (
                    <Text type="secondary">已选择: {quickAnalysisFile.name}</Text>
                  )}
                  <TextArea
              ...
              <Card title="队列状态">
                {queueStatus ? (
                  <div>
                    <Row gutter={16}>
                      <Col span={12}>
                        <Statistic title="总任务" value={queueStatus.total_jobs} />
                      </Col>
                      <Col span={12}>
                        <Statistic title="处理中" value={queueStatus.processing_jobs} />
                      </Col>
                    </Row>
                    ...
                    <Text type="secondary">
                      ...
                      <br />
                      平均处理时间: {queueStatus.average_processing_time.toFixed(1)}秒
                    </Text>
                  </div>
                ) : (
                  <Spin />
              ...
              <Card title="模型使用分布">
                <Space direction="vertical" style={{ width: '100%' }}>
                  {(modelConfigs.length
                    ...
                    const total = Array.from(costStats.modelDistribution.values()).reduce((a, b) => a + b, 1);
          ...
          <Card title="成本计算说明" className="mt-6">
            <Table
              ...
              columns={[
                ...
                { title: '输入价格 ($/1K tokens)', dataIndex: 'input', key: 'input' },
                { title: '输出价格 ($/1K tokens)', dataIndex: 'output', key: 'output' }
              ]}
              rowKey="model"
              pagination={false}
              size="small"
