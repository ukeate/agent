...
const StructuredErrorPage: React.FC = () => {
  ...
  const [selectedError, setSelectedError] = useState<StructuredError | null>(
    null
  )
  ...
  const filteredErrors = errors.filter(error => {
    ...
    const matchesCategory =
      filterCategory === 'all' || error.category === filterCategory
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mb-4">
          统一错误代码、详细错误上下文和本地化错误消息 - 展示structured error
          handling的技术实现
        </p>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${getCategoryColor(error.category)}`}
                        >
                    ...
                    <div className="text-right">
                      <div
                        className={`text-xs font-medium ${error.resolved ? 'text-green-600' : 'text-red-600'}`}
                      >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="bg-gray-50 rounded p-3 text-sm space-y-1">
                    <div>
                      <strong>错误代码:</strong> {selectedError.code}
                    </div>
                    <div>
                      <strong>分类:</strong> {selectedError.category}
                    </div>
                    <div>
                      <strong>严重程度:</strong> {selectedError.severity}
                    </div>
                    <div>
                      <strong>时间:</strong>{' '}
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded p-3 text-sm space-y-1">
                    {Object.entries(selectedError.context).map(
                      ([key, value]) =>
                        value && (
                          <div key={key}>
      ...
      <div className="mt-8 bg-gray-50 rounded-lg p-6">
        ...
        <div className="mt-4 text-sm text-gray-700 space-y-2">
          <p>
            <strong>结构化错误:</strong>{' '}
            通过structured_errors.py实现统一错误处理
          </p>
          <p>
            <strong>错误构建器:</strong>{' '}
            ErrorBuilder支持链式调用构建复杂错误信息
          </p>
          <p>
            <strong>分类管理:</strong> 按错误类型和严重程度进行分类管理
          </p>
          <p>
            <strong>上下文追踪:</strong> 记录完整的错误上下文信息便于调试
          </p>
        </div>
      </div>
    </div>
