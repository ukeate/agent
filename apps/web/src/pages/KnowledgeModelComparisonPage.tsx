...
import {
  ...
  KnowledgeModel,
  ComparisonResult,
} from '../services/knowledgeModelService'
...
const KnowledgeModelComparisonPage: React.FC = () => {
  ...
  const handleCompare = async () => {
    ...
    setLoading(true)
    try {
      const result = await knowledgeModelService.compareModels(
        selectedModels[0],
        selectedModels[1]
      )
    ...
    },
    {
      ...
      render: (accuracy: number) => (
        <Progress
          ...
          strokeColor={
            accuracy >= 95 ? '#52c41a' : accuracy >= 90 ? '#1890ff' : '#faad14'
          }
        />
      ),
    },
    {
      ...
      render: (speed: number) => (
        <span
          style={{
            color:
              speed >= 1000 ? '#52c41a' : speed >= 500 ? '#1890ff' : '#000',
          }}
        >
        ...
        {comparisonResult && (
          <div>
            <Descriptions bordered column={2}>
              <Descriptions.Item label="模型1">
                {comparisonResult.model1}
              </Descriptions.Item>
              <Descriptions.Item label="模型2">
                {comparisonResult.model2}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="功能重叠度" span={2}>
                <Progress
                  percent={comparisonResult.metrics.feature_overlap * 100}
                />
              </Descriptions.Item>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <Space direction="vertical" style={{ width: '100%' }}>
                  {comparisonResult.details.model1_advantages && (
                    <Card
                      size="small"
                      title={`${comparisonResult.model1} 优势`}
                    >
                      {comparisonResult.details.model1_advantages.map(
                  ...
                  {comparisonResult.details.model2_advantages && (
                    <Card
                      size="small"
                      title={`${comparisonResult.model2} 优势`}
                    >
                      {comparisonResult.details.model2_advantages.map(
                        (adv: string, idx: number) => (
                          <Tag key={idx} color="green" style={{ margin: 4 }}>
                            {adv}
                          </Tag>
                        )
                      )}
