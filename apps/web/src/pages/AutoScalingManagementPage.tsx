...
const AutoScalingManagementPage: React.FC = () => {
  ...
  const [conditionType, setConditionType] = useState<'scale_up' | 'scale_down'>('scale_up');
  ...
  const simulateScaling = async (experimentId: string, days: number) => {
    setLoading(true);
    try {
      const response = await autoScalingService.simulateScaling(experimentId, days);
      if (response.success) {
  ...
  const handleAddCondition = async (values: any) => {
    ...
    try {
      ...
      const response = await autoScalingService.addCondition(selectedRule, conditionType, request);
      if (response.success) {
  ...
  const createTemplate = async (type: 'safe' | 'aggressive', experimentId: string) => {
    try {
      const response = type === 'safe' 
  ...
  const rulesColumns = [
    ...
    },
    {
      ...
      render: (count: number, record: any) => (
        <span>↑{count} / ↓{record.scale_down_conditions_count}</span>
      )
    },
    {
      title: '创建时间',
  ...
  const simulationColumns = [
    ...
    },
    {
      ...
      render: (action: string) => {
        const color = action === 'scale_up' ? 'green' : action === 'scale_down' ? 'red' : 'default';
    ...
    <div style={{ padding: '24px' }}>
      <Card title="自动扩量管理" extra={
        ...
        <Tabs defaultActiveKey="rules">
          <TabPane tab={
            <span><SettingOutlined />扩量规则</span>
          } key="rules">
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <Card title="创建扩量规则" size="small">
                  ...
                  >
                    <Form.Item name="experiment_id" rules={[{ required: true }]}>
                      <Input placeholder="实验ID" name="autoscaling-experiment-id" autoComplete="off" />
                    </Form.Item>
                    <Form.Item name="name" rules={[{ required: true }]}>
                      <Input placeholder="规则名称" name="autoscaling-rule-name" autoComplete="off" />
                    </Form.Item>
                    ...
                    <Form.Item name="variant" initialValue="treatment">
                      <Input placeholder="变体" name="autoscaling-variant" autoComplete="off" style={{ width: 100 }} />
                    </Form.Item>
              ...
              <Col span={24}>
                <Card title="扩量规则列表" size="small" extra={
                  <Space>
                    <Button
                      onClick={() => createTemplate('safe', selectedExperiment)}
                      disabled={!selectedExperiment}
                    >
                      创建安全模板
                    </Button>
                    <Button
                      onClick={() => createTemplate('aggressive', selectedExperiment)}
                      disabled={!selectedExperiment}
                    >
                      创建激进模板
                    </Button>
          ...
          <TabPane tab={
            <span><HistoryOutlined />扩量历史</span>
          } key="history">
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <Input.Search
              ...
              {history && (
                <>
                  <Col span={24}>
                    <Row gutter={16}>
                      <Col span={6}>
                        <Statistic title="当前流量百分比" value={history.current_percentage} suffix="%" />
                      </Col>
                      <Col span={6}>
                        <Statistic title="总扩量次数" value={history.total_scale_ups} />
                      </Col>
                      <Col span={6}>
                        <Statistic title="总缩量次数" value={history.total_scale_downs} />
                      </Col>
                      <Col span={6}>
                        <Statistic 
                          title="最后扩量时间" 
                          value={history.last_scaled_at ? new Date(history.last_scaled_at).toLocaleString() : '无'} 
                        />
                      </Col>
                    </Row>
                  </Col>
                      ...
                      <Timeline>
                        {history.recent_decisions.map((decision, index) => (
                          <Timeline.Item key={index} color={
                            decision.direction === ScalingDirection.UP ? 'green' :
                            decision.direction === ScalingDirection.DOWN ? 'red' : 'blue'
                          }>
                            <div>
                              <strong>{new Date(decision.timestamp).toLocaleString()}</strong>
                              <br />
                              方向: <Tag color={decision.direction === ScalingDirection.UP ? 'green' : 'red'}>
                                {decision.direction}
                              </Tag>
                              <br />
                              ...
                              原因: {decision.reason}
                              <br />
                              置信度: <Progress percent={Math.round(decision.confidence * 100)} size="small" />
                            </div>
                          </Timeline.Item>
                        ))}
                      </Timeline>
                    ...
                    <Col span={8} key={index}>
                      <Card size="small" title={`建议 ${index + 1}`}>
                        <Descriptions column={1} size="small">
                          <Descriptions.Item label="操作">{rec.action}</Descriptions.Item>
                          <Descriptions.Item label="置信度">
                            <Progress percent={Math.round(rec.confidence * 100)} size="small" />
                          </Descriptions.Item>
                          <Descriptions.Item label="原因">{rec.reason}</Descriptions.Item>
                          <Descriptions.Item label="风险等级">
                            <Tag color={
                              rec.risk_level === 'low' ? 'green' :
                              rec.risk_level === 'medium' ? 'orange' : 'red'
                            }>
                              {rec.risk_level}
                            </Tag>
                          </Descriptions.Item>
                          {rec.suggested_percentage && (
                            <Descriptions.Item label="建议流量">{rec.suggested_percentage}%</Descriptions.Item>
                          )}
                          {rec.expected_impact && (
                            <Descriptions.Item label="预期影响">{rec.expected_impact}</Descriptions.Item>
                          )}
                        </Descriptions>
                      </Card>
                    </Col>
          ...
          <TabPane tab={
            <span><ExperimentOutlined />扩量模拟</span>
          } key="simulation">
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <Card size="small" title="扩量模拟">
                  <Form layout="inline" onFinish={(values) => simulateScaling(values.experiment_id, values.days)}>
                    <Form.Item name="experiment_id" rules={[{ required: true }]}>
      ...
      >
        ...
        >
          ...
          <Form.Item name="trigger" label="触发器类型" rules={[{ required: true }]}>
          ...
          <Form.Item name="confidence_level" label="置信水平" initialValue={0.95}>
            <InputNumber min={0.5} max={0.999} step={0.01} style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="min_sample_size" label="最小样本量" initialValue={1000}>
