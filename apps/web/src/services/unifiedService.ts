...
class UnifiedService {
  ...
  async getModulesStatus(): Promise<{
    success: boolean;
    timestamp: string;
  ...
  async getMonitoringSummary(): Promise<{
    ...
    return {
      ...
      performance_metrics: {
        avg_response_time: Number(latest.response_time ?? 0),
        success_rate_percent: successRate,
      },
      resource_usage: {
        cpu_percent: Number(latest.cpu_usage ?? 0),
        memory_percent: Number(latest.memory_usage ?? 0),
        disk_percent: Number(latest.disk_usage ?? 0),
      },
      trends: null,
    };
  }
  ...
  async getMetricsByCategory(category: 'system' | 'application' | 'business' | 'ml_models'): Promise<any> {
  ...
  async aggregateMetrics(params: {
    ...
    const response = await apiClient.post(`${this.baseUrl}/metrics/aggregate`, params);
    return response.data;
  }
  ...
  async createDashboard(dashboard: Omit<UnifiedDashboard, 'id' | 'created_at' | 'updated_at'>): Promise<UnifiedDashboard> {
    const response = await apiClient.post(`${this.baseUrl}/dashboards`, dashboard);
    return response.data;
  }

  async updateDashboard(dashboardId: string, updates: Partial<UnifiedDashboard>): Promise<UnifiedDashboard> {
  ...
  async addWidget(dashboardId: string, widget: Omit<Widget, 'id'>): Promise<Widget> {
    const response = await apiClient.post(`${this.baseUrl}/dashboards/${dashboardId}/widgets`, widget);
    return response.data;
  }

  async updateWidget(dashboardId: string, widgetId: string, updates: Partial<Widget>): Promise<Widget> {
    const response = await apiClient.put(`${this.baseUrl}/dashboards/${dashboardId}/widgets/${widgetId}`, updates);
    return response.data;
  }

  async removeWidget(dashboardId: string, widgetId: string): Promise<{ success: boolean }> {
    const response = await apiClient.delete(`${this.baseUrl}/dashboards/${dashboardId}/widgets/${widgetId}`);
    return response.data;
  }
  ...
  async addDataSource(dataSource: Omit<DataSource, 'id' | 'status'>): Promise<DataSource> {
    const response = await apiClient.post(`${this.baseUrl}/datasources`, dataSource);
    return response.data;
  }
  ...
  async testDataSource(dataSourceId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/datasources/${dataSourceId}/test`);
    return response.data;
  }
  ...
  async syncDataSource(dataSourceId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/datasources/${dataSourceId}/sync`);
    return response.data;
  }

  async removeDataSource(dataSourceId: string): Promise<{ success: boolean }> {
    const response = await apiClient.delete(`${this.baseUrl}/datasources/${dataSourceId}`);
    return response.data;
  }
  ...
  async saveQuery(name: string, query: UnifiedQuery): Promise<{
    id: string;
    name: string;
    query: UnifiedQuery;
  }> {
  ...
  async getSavedQueries(): Promise<Array<{
    id: string;
    name: string;
    query: UnifiedQuery;
    created_at: string;
  ...
  async generateUnifiedReport(params: {
    ...
    const response = await apiClient.post(`${this.baseUrl}/reports/generate`, params);
    return response.data;
  }
