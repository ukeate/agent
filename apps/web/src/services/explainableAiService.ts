...
class ExplainableAiService {
  ...
  async generateExplanation(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/generate-explanation`,
      request
    )
  ...
  // 生成CoT推理解释
  async generateCoTReasoning(
    request: CoTReasoningRequest
  ): Promise<DecisionExplanation> {
    const response = await apiClient.post(
      `${this.baseUrl}/cot-reasoning`,
      request
    )
    return response.data
  }
  ...
  // 生成工作流解释
  async generateWorkflowExplanation(
    request: WorkflowExecutionRequest
  ): Promise<DecisionExplanation> {
    const response = await apiClient.post(
      `${this.baseUrl}/workflow-explanation`,
      request
    )
    return response.data
  }
  ...
  async formatExplanation(request: ExplanationFormatRequest): Promise<{
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/format-explanation`,
      request
    )
    return response.data
  }
    ...
    const request: ExplanationRequest = {
      ...
      factors: [
        ...
        },
        {
          ...
          weight: 0.2,
          impact:
            patientHistory === 'relevant'
              ? 0.6
              : patientHistory === 'partial'
                ? 0.4
                : 0.1,
        ...
        },
        {
          ...
          weight: 0.1,
          impact:
            imagingFindings === 'positive'
              ? 0.9
              : imagingFindings === 'inconclusive'
                ? 0.3
                : 0.1,
          source: 'radiology',
        },
