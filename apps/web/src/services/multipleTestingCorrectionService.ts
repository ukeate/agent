...
export class MultipleTestingCorrectionService {
  ...
  async getMethodRecommendation(request: any): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/recommendation`, request);
    return response.data;
  }

  async adjustPowerForMultipleTesting(request: any): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/power-adjustment`, request);
    return response.data;
  }

  async analyzeABTestMultipleComparisons(request: any): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/ab-test-multiple`, request);
    return response.data;
  }
}

export const multipleTestingCorrectionService = new MultipleTestingCorrectionService();