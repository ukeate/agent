...
export class TrafficRampService {
  ...
  async startRamp(planId: string): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/start`, { plan_id: planId });
    return response.data;
  }

  async pauseRamp(execId: string): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/pause`, { exec_id: execId });
    return response.data;
  }

  async resumeRamp(execId: string): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/resume`, { exec_id: execId });
    return response.data;
  }
  ...
  async listExecutions(experimentId?: string, status?: RampStatus): Promise<any> {
  ...
  async getRecommendedPlan(request: GetRecommendedPlanRequest): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/recommended-plan`, request);
    return response.data;
  }
  ...
  async listStrategies(): Promise<{ success: boolean; strategies: StrategyInfo[] }> {
  ...
  async getCurrentPhase(execId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/current-phase/${execId}`);
    return response.data;
  }
  ...
  // 便捷方法
  async createLinearRamp(experimentId: string, targetPercentage: number, durationHours: number): Promise<any> {
  ...
  async createBetaRamp(experimentId: string, durationHours?: number): Promise<any> {
    return this.quickRamp({
      experiment_id: experimentId,
  ...
  async getActiveRamps(): Promise<RampExecution[]> {
    ...
    if (response.success) {
      return response.executions.filter((exec: RampExecution) => 
        exec.status === RampStatus.RUNNING || exec.status === RampStatus.PAUSED
      );
    }
    return [];
  }

  async getAllRampsForExperiment(experimentId: string): Promise<RampExecution[]> {
  ...
  async getStrategyRecommendation(riskLevel: 'low' | 'medium' | 'high'): Promise<RampStrategy> {
    switch (riskLevel) {
      case 'low':
  ...
  async calculateOptimalSteps(durationHours: number, strategy: RampStrategy): Promise<number> {
    switch (strategy) {
