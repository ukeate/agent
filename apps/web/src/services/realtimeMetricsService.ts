...
class RealtimeMetricsService {
  ...
  async getMetricDefinition(metricName: string): Promise<MetricDefinition> {
    const response = await apiClient.get(
      `${this.baseUrl}/definitions/${metricName}`
    )
    return response.data
  }
  ...
  async getMetricSnapshot(metricName: string): Promise<MetricSnapshot> {
    const response = await apiClient.get(
      `${this.baseUrl}/snapshot/${metricName}`
    )
    return response.data
  }
  ...
  async detectAnomalies(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/anomalies/${metricName}`,
      params || {}
    )
    return response.data
  }
  ...
  async configureAlertThreshold(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/alerts/configure/${metricName}`,
      params
    )
    return response.data
  }
  ...
  async createDashboard(
    dashboard: Omit<MetricsDashboard, 'id'>
  ...
  async updateDashboard(
    dashboardId: string,
    updates: Partial<MetricsDashboard>
  ): Promise<MetricsDashboard> {
    const response = await apiClient.put(
      `${this.baseUrl}/dashboards/${dashboardId}`,
      updates
  ...
  connectMetricsStream(
    ...
    const wsUrl = buildWsUrl(
      `${this.baseUrl}/stream?metrics=${metrics.join(',')}`
    )
  ...
  async startRealtimeMonitoring(experimentId: string): Promise<{
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/start-monitoring/${experimentId}`
    )
    return response.data
  }
  ...
  async getRealtimeMetricDefinition(metricName: string): Promise<{
    ...
    const response = await apiClient.get(
      `${this.baseUrl}/metric-definition/${metricName}`
    )
    return response.data
  }
