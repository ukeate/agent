...
class AnalyticsService {
  ...
  async submitEvents(request: EventSubmissionRequest): Promise<EventSubmissionResponse> {
  ...
  async detectAnomalies(params?: {
    ...
    const response = await apiClient.post(`${this.baseUrl}/anomalies/detect`, params || {});
    return response.data;
  }
  ...
  async getUserSegments(userId: string): Promise<UserSegment[]> {
    const response = await apiClient.get(`${this.baseUrl}/users/${userId}/segments`);
    return response.data.segments || [];
  }

  async getUserJourney(userId: string, params?: {
    start_time?: string;
    end_time?: string;
  }): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/users/${userId}/journey`, { params });
    return response.data;
  }
  ...
  async downloadReport(reportId: string, format: 'json' | 'html' | 'pdf'): Promise<Blob> {
