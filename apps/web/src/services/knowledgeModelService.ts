...
class KnowledgeModelService {
  ...
  async compareModels(model1Id: string, model2Id: string): Promise<ComparisonResult> {
  ...
  async getBenchmarks(modelId: string): Promise<ModelBenchmark[]> {
    const response = await apiClient.get(`${this.baseUrl}/models/${modelId}/benchmarks`);
    return response.data;
  }

  async runBenchmark(modelId: string, dataset: string): Promise<ModelBenchmark> {
    const response = await apiClient.post(`${this.baseUrl}/models/${modelId}/benchmark`, {
      dataset
    });
    return response.data;
  }
  ...
  async getRecommendation(requirements: {
    ...
    const response = await apiClient.post(`${this.baseUrl}/recommend`, requirements);
    return response.data;
  }

  async exportComparison(modelIds: string[], format: 'json' | 'csv' | 'pdf' = 'json'): Promise<Blob> {
    const response = await apiClient.post(`${this.baseUrl}/export-comparison`, 
