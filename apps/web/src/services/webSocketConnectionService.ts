...
class WebSocketConnectionService {
  ...
  async disconnectConnection(connectionIndex: number): Promise<{
    ...
    try {
      const response = await apiClient.delete(`${this.baseUrl}/connections/${connectionIndex}`);
      return response.data;
    } catch (error) {
   ...
   * 向指定连接发送消息
   */
  async sendMessageToConnection(connectionIndex: number, message: any): Promise<{
    success: boolean;
    message: string;
  }> {
    try {
      const response = await apiClient.post(`${this.baseUrl}/connections/${connectionIndex}/send`, {
        message
      });
      return response.data;
    } catch (error) {
  ...
  async broadcastMessage(message: any): Promise<{
    ...
    try {
      const response = await apiClient.post(`${this.baseUrl}/connections/broadcast`, {
        message
      });
      return response.data;
    } catch (error) {
