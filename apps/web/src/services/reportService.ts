...
class ReportService {
  ...
  async getPGVectorPerformanceReport(): Promise<any> {
    const response = await apiClient.get(`/pgvector/monitoring/performance-report`);
    return response.data;
  }
  ...
  async getModelCompressionReport(jobId: string): Promise<any> {
    const response = await apiClient.get(`/model-compression/results/${jobId}/report`);
    return response.data;
  }
  ...
  async getAnnotationTaskQualityReport(taskId: string): Promise<any> {
    const response = await apiClient.get(`/training-data/annotation-tasks/${taskId}/quality-report`);
    return response.data;
  }
   ...
   */
  async exportExperimentReport(experimentId: string, format?: string): Promise<Blob> {
