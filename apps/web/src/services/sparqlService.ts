...
class SPARQLService {
  ...
  ): Promise<SparqlPerformanceReport> {
  ...
  // 查询模板
  async getQueryTemplates(category?: string): Promise<
    Array<{
      id: string
      name: string
      description: string
  ...
  async deleteGraph(
    uri: string
  ): Promise<{ success: boolean; deleted_triples: number }> {
    const response = await apiClient.delete(
      `${this.baseUrl}/graphs/${encodeURIComponent(uri)}`
    )
    return response.data
  }
  ...
  async cancelQuery(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/queries/${queryId}/cancel`
    )
    return response.data
  }
}
