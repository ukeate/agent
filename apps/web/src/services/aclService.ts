...
class ACLService {
  ...
  async updateRule(ruleId: string, data: UpdateACLRuleRequest): Promise<ACLRule> {
  ...
  async testRule(ruleId: string, testData: { source: string; target: string; context?: any }): Promise<{ allowed: boolean; matched_rule: ACLRule | null; reason: string }> {
    const response = await apiClient.post(`${this.baseUrl}/rules/${ruleId}/test`, testData);
    return response.data;
  }
  ...
  async importRules(file: File): Promise<{ imported: number; failed: number; errors: string[] }> {
