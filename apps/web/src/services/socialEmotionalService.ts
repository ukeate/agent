...
export interface SystemAnalytics {
  ...
  emotional_state_trends: Record<string, Array<{
    timestamp: string;
    value: number;
  }>>;
...
class SocialEmotionalService {
  ...
  async analyzeGroupEmotion(request: AnalysisRequest): Promise<GroupEmotionResponse> {
    const response = await apiClient.post(`${this.baseUrl}/analyze/group-emotion`, request);
    return response.data;
  }

  // ==================== 关系分析 ====================

  async analyzeRelationships(request: AnalysisRequest): Promise<RelationshipAnalysisResponse> {
    const response = await apiClient.post(`${this.baseUrl}/analyze/relationships`, request);
    return response.data;
  }

  // ==================== 社交上下文分析 ====================

  async analyzeSocialContext(request: AnalysisRequest): Promise<SocialContextAnalysisResponse> {
    const response = await apiClient.post(`${this.baseUrl}/analyze/social-context`, request);
    return response.data;
  }

  // ==================== 文化适配分析 ====================

  async analyzeCulturalAdaptation(request: AnalysisRequest): Promise<CulturalAdaptationResponse> {
    const response = await apiClient.post(`${this.baseUrl}/analyze/cultural-adaptation`, request);
    return response.data;
  }

  // ==================== 社交决策生成 ====================

  async generateSocialDecisions(request: AnalysisRequest): Promise<SocialDecisionResponse> {
    const response = await apiClient.post(`${this.baseUrl}/decisions/generate`, request);
    return response.data;
  }

  // ==================== 综合分析 ====================

  async comprehensiveAnalysis(request: AnalysisRequest): Promise<ComprehensiveAnalysisResponse> {
    const response = await apiClient.post(`${this.baseUrl}/comprehensive-analysis`, request);
    return response.data;
  }
