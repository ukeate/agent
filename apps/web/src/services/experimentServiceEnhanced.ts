...
class ExperimentServiceEnhanced {
  ...
  async getExperimentAnalysis(experimentId: string): Promise<ExperimentAnalysis> {
    const response = await apiClient.get(`${this.baseUrl}/${experimentId}/analysis`);
    return response.data;
  }
  ...
  async getMonitoring(experimentId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/${experimentId}/monitoring`);
    return response.data;
  }

  async getCostAnalysis(experimentId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/${experimentId}/cost-analysis`);
    return response.data;
  }

  async getAuditLog(experimentId: string): Promise<any[]> {
    const response = await apiClient.get(`${this.baseUrl}/${experimentId}/audit`);
    return response.data;
  }

  async addComment(experimentId: string, comment: { text: string; type: string }): Promise<void> {
  ...
  async exportData(experimentId: string, format: 'csv' | 'xlsx' | 'json'): Promise<Blob> {
    const response = await apiClient.get(`${this.baseUrl}/${experimentId}/export`, {
      params: { format },
      responseType: 'blob'
    });
    return response.data;
  ...
  async cloneExperiment(experimentId: string, name: string): Promise<ExperimentMetadata> {
    const response = await apiClient.post(`${this.baseUrl}/${experimentId}/clone`, { name });
    return response.data;
  }
}
