...
class FeedbackService {
   ...
   * 提交显式反馈
   */
  async submitExplicitFeedback(feedback: ExplicitFeedback): Promise<ApiResponse> {
    try {
   ...
   * 获取用户反馈历史
   */
  async getUserFeedbackHistory(query: FeedbackHistoryQuery): Promise<ApiResponse<FeedbackEvent[]>> {
    try {
   ...
   * 获取用户反馈分析
   */
  async getUserFeedbackAnalytics(userId: string): Promise<ApiResponse<FeedbackAnalytics>> {
    try {
   ...
   * 获取推荐项反馈分析
   */
  async getItemFeedbackAnalytics(itemId: string): Promise<ApiResponse<ItemFeedbackAnalytics>> {
    try {
   ...
   * 获取反馈质量评分
   */
  async getFeedbackQualityScore(feedbackIds: string[]): Promise<ApiResponse<FeedbackQualityScore[]>> {
    try {
      const response = await apiClient.post<ApiResponse<FeedbackQualityScore[]>>(
   ...
   * 获取实时反馈指标
   */
  async getRealTimeFeedbackMetrics(): Promise<ApiResponse<{
