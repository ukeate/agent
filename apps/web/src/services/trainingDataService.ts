...
class TrainingDataService {
  ...
  async listDataSources(activeOnly: boolean = true): Promise<DataSourceResponse[]> {
  ...
  async updateDataSource(sourceId: string, updates: Record<string, any>): Promise<{
    message: string;
  }> {
    const response = await apiClient.put(`${this.baseUrl}/sources/${sourceId}`, updates);
    return response.data;
  }
  ...
  async deleteDataSource(sourceId: string): Promise<{
    ...
    const response = await apiClient.delete(`${this.baseUrl}/sources/${sourceId}`);
    return response.data;
  }
  ...
  async getCollectionStatistics(sourceId?: string): Promise<CollectionStatistics> {
    const params = sourceId ? { source_id: sourceId } : {};
    const response = await apiClient.get(`${this.baseUrl}/statistics`, { params });
    return response.data;
  }

  async reprocessRecords(params: {
    record_ids?: string[];
    source_id?: string;
  ...
  async createAnnotationTask(taskData: AnnotationTaskCreate): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/annotation-tasks`, taskData);
    return response.data;
  }

  async listAnnotationTasks(params: {
    assignee_id?: string;
    status?: string;
    created_by?: string;
    limit?: number;
    ...
    const response = await apiClient.get(`${this.baseUrl}/annotation-tasks`, { params });
    return response.data;
  }

  async getAnnotationTaskDetails(taskId: string): Promise<AnnotationTask> {
    const response = await apiClient.get(`${this.baseUrl}/annotation-tasks/${taskId}`);
    return response.data;
  }

  async getAnnotationProgress(taskId: string): Promise<AnnotationProgress> {
    const response = await apiClient.get(`${this.baseUrl}/annotation-tasks/${taskId}/progress`);
    return response.data;
  }

  async assignAnnotationTask(taskId: string, userIds: string[]): Promise<{
    message: string;
  }> {
    const response = await apiClient.post(`${this.baseUrl}/annotation-tasks/${taskId}/assign`, userIds);
    return response.data;
  }
  ...
  async submitAnnotation(annotationData: AnnotationSubmit): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/annotations`, annotationData);
    return response.data;
  }

  async getInterAnnotatorAgreement(taskId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/annotation-tasks/${taskId}/agreement`);
    return response.data;
  }

  async getQualityControlReport(taskId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/annotation-tasks/${taskId}/quality-report`);
    return response.data;
  }
  ...
  async getUserAnnotations(params: {
    ...
    const response = await apiClient.get(`${this.baseUrl}/annotations`, { params });
    return response.data;
  }
  ...
  async listDatasetVersions(datasetName: string): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/datasets/${datasetName}/versions`);
    return response.data;
  }
  ...
  async getVersionData(versionId: string): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/versions/${versionId}`);
    return response.data;
  }
  ...
  async getVersionHistory(versionId: string): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/versions/${versionId}/history`);
    return response.data;
  }

  async compareVersions(versionId1: string, versionId2: string): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/versions/${versionId1}/compare/${versionId2}`);
    return response.data;
  }

  async exportVersion(versionId: string, format: 'jsonl' | 'json' | 'csv' = 'jsonl'): Promise<{
    version_id: string;
    export_path: string;
    format: string;
    message: string;
  }> {
    const response = await apiClient.post(`${this.baseUrl}/versions/${versionId}/export`, null, {
      params: { format }
    });
    return response.data;
  }

  async rollbackDataset(datasetName: string, targetVersionId: string): Promise<{
    dataset_name: string;
    target_version_id: string;
    new_version_id: string;
    message: string;
  }> {
    const response = await apiClient.post(`${this.baseUrl}/datasets/${datasetName}/rollback`, null, {
  ...
  async getVersionStatistics(datasetName?: string): Promise<VersionStatistics> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/version-statistics`, { params });
    return response.data;
  }

  async deleteVersion(versionId: string, removeFiles: boolean = true): Promise<{
    message: string;
  }> {
    const response = await apiClient.delete(`${this.baseUrl}/versions/${versionId}`, {
