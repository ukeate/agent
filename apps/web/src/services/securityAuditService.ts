...
class SecurityAuditService {
  ...
  async getStatistics(startTime?: string, endTime?: string): Promise<AuditStatistics> {
  ...
  async getSecurityIncidents(status?: string): Promise<SecurityIncident[]> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/incidents`, { params });
    return response.data;
  }
  ...
  async updateIncidentStatus(incidentId: string, status: string, resolution?: string): Promise<SecurityIncident> {
    const response = await apiClient.put(`${this.baseUrl}/incidents/${incidentId}/status`, {
      status,
      resolution
    });
    return response.data;
  ...
  async exportAuditLogs(format: 'json' | 'csv' | 'pdf', filters?: AuditFilterParams): Promise<Blob> {
  ...
  async getComplianceReport(standard: 'SOC2' | 'ISO27001' | 'GDPR' | 'HIPAA'): Promise<{
    standard: string;
    ...
    const response = await apiClient.get(`${this.baseUrl}/compliance/${standard}`);
    return response.data;
  }
