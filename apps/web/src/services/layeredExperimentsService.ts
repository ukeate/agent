...
export class LayeredExperimentsService {
  ...
  async updateLayer(layerId: string, layerData: Partial<ExperimentLayer>): Promise<any> {
    const response = await apiClient.put(`${this.baseUrl}/${layerId}`, layerData);
    return response.data;
  }
  ...
  async assignExperimentToLayer(layerId: string, experimentId: string): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/${layerId}/experiments`, {
      experiment_id: experimentId
    });
    return response.data;
  }

  async removeExperimentFromLayer(layerId: string, experimentId: string): Promise<any> {
    const response = await apiClient.delete(`${this.baseUrl}/${layerId}/experiments/${experimentId}`);
    return response.data;
  }

  async getLayerExperiments(layerId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/${layerId}/experiments`);
    return response.data;
  }

  async getConflicts(layerId?: string): Promise<any> {
    const params = layerId ? { layer_id: layerId } : {};
    const response = await apiClient.get(`${this.baseUrl}/conflicts`, { params });
    return response.data;
  }

  async resolveConflict(conflictId: string, resolution: string): Promise<any> {
    const response = await apiClient.post(`${this.baseUrl}/conflicts/${conflictId}/resolve`, {
      resolution
    });
    return response.data;
  }

  async validateLayerConfiguration(layerData: Partial<ExperimentLayer>): Promise<any> {
