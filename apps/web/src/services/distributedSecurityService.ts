...
class DistributedSecurityService {
  ...
  async validateToken(
    token: string
  ): Promise<{ valid: boolean; agent_id?: string; expires_at?: string }> {
    const response = await apiClient.post(`${this.baseUrl}/validate-token`, {
      token,
    })
    return response.data
  }
  ...
  async revokeToken(token: string): Promise<{ success: boolean }> {
    const response = await apiClient.post(`${this.baseUrl}/revoke-token`, {
      token,
    })
    return response.data
  }
  ...
  async getPermissions(agent_id: string): Promise<string[]> {
    const response = await apiClient.get(
      `${this.baseUrl}/agents/${agent_id}/permissions`
    )
  ...
  async lockAgent(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/agents/${agentId}/lock`,
      { reason }
    )
    return response.data
  }
  ...
  async unlockAgent(agentId: string): Promise<{ success: boolean }> {
    const response = await apiClient.post(
      `${this.baseUrl}/agents/${agentId}/unlock`
    )
    return response.data
  }
  ...
  async resetAgentCredentials(agentId: string): Promise<{
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/agents/${agentId}/reset-credentials`
    )
    return response.data
  }
  ...
  async getEvents(params?: {
    ...
    Array<SecurityEventRequest & { event_id: string; timestamp: string }>
  > {
  ...
  async getCommunicationSessions(): Promise<CommunicationSession[]> {
    const response = await apiClient.get(
      `${this.baseUrl}/communication/sessions`
    )
  ...
  async establishSecureChannel(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/communication/establish`,
      {
        participant1,
        participant2,
      }
  ...
  async terminateSession(sessionId: string): Promise<{ success: boolean }> {
    const response = await apiClient.post(
      `${this.baseUrl}/communication/sessions/${sessionId}/terminate`
    )
    return response.data
  }
