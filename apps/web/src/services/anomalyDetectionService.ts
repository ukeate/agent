...
export const anomalyDetectionService = {
  ...
  // 检查样本比例不匹配
  async checkSampleRatioMismatch(
    request: SRMCheckRequest
  ): Promise<SRMCheckResponse> {
    const response = await apiClient.post<SRMCheckResponse>(
      '/anomalies/check-srm',
      request
    )
    return response.data
  ...
  async checkDataQuality(
    ...
    const response = await apiClient.post<DataQualityResponse>(
      '/anomalies/check-data-quality',
      request
    )
    return response.data
  },
  ...
  async configureDetection(
    ...
    const response = await apiClient.post<ConfigurationResponse>(
      '/anomalies/configure',
      request
    )
    return response.data
  ...
  async getAnomalyTypes(): Promise<AnomalyTypesResponse> {
    const response =
      await apiClient.get<AnomalyTypesResponse>('/anomalies/types')
    return response.data
  },
  ...
  async getDetectionMethods(): Promise<DetectionMethodsResponse> {
    const response =
      await apiClient.get<DetectionMethodsResponse>('/anomalies/methods')
    return response.data
  },
