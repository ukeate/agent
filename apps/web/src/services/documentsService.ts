...
class DocumentsService {
  ...
  async uploadDocument(
    ...
    options: {
      ...
      autoTag?: boolean;
      chunkStrategy?: 'semantic' | 'fixed' | 'adaptive' | 'sliding_window' | 'hierarchical';
    } = {}
  ): Promise<any> {
    try {
  ...
  async analyzeDocumentRelationships(docId: string, relatedDocIds: string[] = []): Promise<any> {
    try {
      const response = await apiClient.post(`${this.baseUrl}/${docId}/analyze-relationships`, null, {
        params: {
  ...
  async generateDocumentTags(docId: string, content?: string, existingTags: string[] = []): Promise<any> {
    try {
      const params: any = { existing_tags: existingTags }
      if (content !== undefined) params.content = content
      const response = await apiClient.post(`${this.baseUrl}/${docId}/generate-tags`, null, {
        params
      });
      return response.data;
    } catch (error) {
  ...
  async getDocumentVersionHistory(docId: string, limit?: number): Promise<any> {
    try {
      const response = await apiClient.get(`${this.baseUrl}/${docId}/versions`, {
        params: limit ? { limit } : {}
      });
      return response.data;
    } catch (error) {
  ...
  async rollbackDocumentVersion(docId: string, targetVersionId: string): Promise<any> {
    try {
      const response = await apiClient.post(`${this.baseUrl}/${docId}/rollback`, null, {
        params: {
  ...
  async updateDocument(documentId: string, updates: Partial<Document>): Promise<Document> {
    const response = await apiClient.put(`${this.baseUrl}/${documentId}`, updates);
    return response.data;
  }
  ...
  async similarDocuments(documentId: string, limit: number = 10): Promise<DocumentSearchResult[]> {
    const response = await apiClient.get(`${this.baseUrl}/${documentId}/similar`, {
      params: { limit }
    });
    return response.data;
  }
  ...
  async processDocument(documentId: string, operations?: string[]): Promise<DocumentProcessingStatus> {
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/process`, {
  ...
  async chunkDocument(documentId: string, chunkSize: number = 512, overlap: number = 50): Promise<DocumentChunk[]> {
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/chunk`, {
  ...
  async analyzeDocument(documentId: string): Promise<DocumentAnalysis> {
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/analyze`);
    return response.data;
  }

  async summarizeDocument(documentId: string, maxLength?: number): Promise<{
    summary: string;
    key_points: string[];
  }> {
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/summarize`, {
  ...
  async extractEntities(documentId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/extract-entities`);
    return response.data;
  }

  // ==================== 文档导出 ====================

  async exportDocument(documentId: string, format: 'pdf' | 'docx' | 'html' | 'markdown'): Promise<Blob> {
    const response = await apiClient.get(`${this.baseUrl}/${documentId}/export`, {
      params: { format },
      responseType: 'blob'
    });
    return response.data;
  ...
  async batchProcess(documentIds: string[], operations: string[]): Promise<{
  ...
  async getDocumentVersion(documentId: string, version: number): Promise<Document> {
    const response = await apiClient.get(`${this.baseUrl}/${documentId}/versions/${version}`);
    return response.data;
  }

  async restoreDocumentVersion(documentId: string, version: number): Promise<Document> {
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/restore`, {
      version
    });
    return response.data;
  }
  ...
  async addTags(documentId: string, tags: string[]): Promise<Document> {
    const response = await apiClient.post(`${this.baseUrl}/${documentId}/tags`, {
      tags
    });
    return response.data;
  }
  ...
  async removeTags(documentId: string, tags: string[]): Promise<Document> {
    const response = await apiClient.delete(`${this.baseUrl}/${documentId}/tags`, {
      data: { tags }
    });
    return response.data;
  }
