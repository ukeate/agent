...
export class OfflineService {
  ...
  async getOperations(limit: number = 100, offset: number = 0): Promise<OfflineOperation[]> {
  ...
  async resolveConflictUseLocal(conflictId: string, localData: any): Promise<any> {
  ...
  async resolveConflictUseRemote(conflictId: string, remoteData: any): Promise<any> {
    return this.resolveConflict({
      conflict_id: conflictId,
  ...
  async resolveConflictMerge(conflictId: string, mergedData: any): Promise<any> {
    return this.resolveConflict({
      conflict_id: conflictId,
