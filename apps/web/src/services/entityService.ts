...
class EntityService {
   ...
   * 更新实体
   */
  async updateEntity(entityId: string, entityData: EntityUpdateRequest): Promise<Entity> {
    try {
      const response = await apiClient.put(`${this.baseUrl}/${entityId}`, entityData);
      return response.data;
    } catch (error) {
   ...
   * 搜索实体
   */
  async searchEntities(searchRequest: EntitySearchRequest): Promise<EntityResponse> {
    try {
   ...
   * 批量创建实体
   */
  async batchCreateEntities(batchRequest: EntityBatchRequest): Promise<EntityResponse> {
    try {
   ...
   * 验证实体数据
   */
  validateEntity(entityData: EntityCreateRequest | EntityUpdateRequest): string[] {
    const errors: string[] = [];
    
    if ('uri' in entityData && entityData.uri && !this.isValidURI(entityData.uri)) {
      errors.push('URI格式无效');
    }
    
    if ('type' in entityData && entityData.type && !entityData.type.includes(':')) {
      errors.push('实体类型必须包含命名空间前缀');
    }
    
    if ('label' in entityData && entityData.label && entityData.label.trim().length === 0) {
   ...
   * 导入实体数据
   */
  async importEntities(file: File, format: 'json' | 'rdf' | 'csv' = 'json'): Promise<{
    success: number;
    failed: number;
    errors: string[];
  }> {
