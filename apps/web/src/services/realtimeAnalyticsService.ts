...
class RealtimeAnalyticsService {
   ...
   * 配置告警规则
   */
  async setAlert(alertConfig: AlertConfig): Promise<{ message: string; alert_id: string }> {
   ...
   * 获取告警列表
   */
  async getAlerts(active_only: boolean = false): Promise<Array<AlertConfig & {
    alert_id: string;
    created_at: string;
   ...
   * 保存仪表板配置
   */
  async saveDashboard(dashboardConfig: DashboardConfig): Promise<{ message: string }> {
    const response = await apiClient.post(`${this.baseUrl}/dashboards`, dashboardConfig);
    return response.data;
  }
