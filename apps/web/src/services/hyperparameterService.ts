...
class HyperparameterService {
  ...
  async listExperiments(status?: string): Promise<Experiment[]> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/experiments`, { params });
  ...
  async createExperiment(request: CreateExperimentRequest): Promise<Experiment> {
    const response = await apiClient.post(`${this.baseUrl}/experiments`, request);
    return response.data;
  }
  ...
  async listTrials(experimentId: string, state?: string): Promise<Trial[]> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/trials`, { params });
    return response.data;
  }

  async getTrial(experimentId: string, trialId: string): Promise<Trial> {
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/trials/${trialId}`);
    return response.data;
  }

  async suggestTrial(experimentId: string): Promise<TrialSuggestion> {
    const response = await apiClient.post(`${this.baseUrl}/experiments/${experimentId}/suggest`);
    return response.data;
  }

  async reportTrialResult(experimentId: string, result: TrialResult): Promise<void> {
    await apiClient.post(`${this.baseUrl}/experiments/${experimentId}/report`, result);
  }

  async pruneTriaml(experimentId: string, trialId: string): Promise<void> {
    await apiClient.post(`${this.baseUrl}/experiments/${experimentId}/trials/${trialId}/prune`);
  }

  // 统计和分析
  async getExperimentStatistics(experimentId: string): Promise<ExperimentStatistics> {
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/statistics`);
    return response.data;
  }

  async getParameterImportance(experimentId: string): Promise<Record<string, number>> {
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/importance`);
    return response.data;
  }

  async getOptimizationHistory(experimentId: string): Promise<Array<{
    trial: number;
    value: number;
    parameters: Record<string, any>;
  }>> {
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/history`);
    return response.data;
  }

  async getParallelCoordinates(experimentId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/parallel-coordinates`);
    return response.data;
  }
  ...
  // 导出和报告
  async exportExperiment(experimentId: string, format: 'json' | 'csv' | 'pdf' = 'json'): Promise<Blob> {
    const response = await apiClient.get(`${this.baseUrl}/experiments/${experimentId}/export`, {
      params: { format },
      responseType: 'blob'
    });
    return response.data;
  ...
  async generateReport(experimentId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/experiments/${experimentId}/report`);
    return response.data;
  }

  // 批量操作
  async batchSuggest(experimentId: string, count: number): Promise<TrialSuggestion[]> {
    const response = await apiClient.post(`${this.baseUrl}/experiments/${experimentId}/batch-suggest`, {
      count
    });
    return response.data;
  }

  async batchReport(experimentId: string, results: TrialResult[]): Promise<void> {
    await apiClient.post(`${this.baseUrl}/experiments/${experimentId}/batch-report`, {
      results
    });
  }
}
