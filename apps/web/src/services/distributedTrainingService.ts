...
export interface DistributedConfig {
  strategy: 'data_parallel' | 'model_parallel' | 'parameter_server' | 'allreduce' | 'async_updates';
  num_workers: number;
  num_ps: number;
...
class DistributedTrainingService {
  ...
  async controlTrainingJob(
    ...
    const response = await apiClient.post(`${this.baseUrl}/jobs/${jobId}/control`, {
      action
    });
    return response.data;
  }
  ...
  async getDeepSpeedConfig(jobId: string): Promise<DeepSpeedConfig> {
    try {
      const response = await apiClient.get(`${this.baseUrl}/jobs/${jobId}/deepspeed`);
      return response.data;
    } catch (error) {
  ...
  async updateDeepSpeedConfig(
    ...
    const response = await apiClient.put(`${this.baseUrl}/jobs/${jobId}/deepspeed`, config);
    return response.data;
  }
  ...
  async getTrainingMetrics(
    ...
    try {
      ...
      const response = await apiClient.get(`${this.baseUrl}/jobs/${jobId}/metrics`, { params });
      return response.data;
    } catch (error) {
  ...
  async getSyncEfficiency(jobId: string): Promise<{
    ...
    try {
      const response = await apiClient.get(`${this.baseUrl}/jobs/${jobId}/sync-analysis`);
      return response.data;
    } catch (error) {
