...
class RiskAssessmentService {
  ...
  ): Promise<{ success: boolean; assessment: RiskAssessment }> {
    try {
  ...
  async getRollbackStatus(execId: string): Promise<{
    ...
    try {
      const response = await apiClient.get(
        `${this.baseUrl}/rollback/status/${execId}`
      )
      return response.data
    } catch (error) {
  ...
  // 获取风险类别列表
  async listRiskCategories(): Promise<{
    success: boolean
  ...
  // 获取回滚策略列表
  async listRollbackStrategies(): Promise<{
    success: boolean
    strategies: RollbackStrategyInfo[]
  }> {
    try {
