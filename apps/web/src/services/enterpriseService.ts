...
class EnterpriseService {
  ...
  async getAssessmentStatus(
    assessmentId: string
  ): Promise<{ status: string; progress: number; result?: ComplianceData }> {
    const response = await apiClient.get(
      `${this.baseUrl}/compliance/assess/${assessmentId}`
    )
    return response.data
  }
   ...
   * 触发安全扫描
   */
  async triggerSecurityScan(options?: {
    deep_scan?: boolean
    target_components?: string[]
  }): Promise<{ scan_id: string }> {
    const response = await apiClient.post(
      `${this.baseUrl}/security/scan`,
      options || {}
    )
    return response.data
  }
  ...
  async getSecurityScanResult(
    scanId: string
  ): Promise<{ status: string; findings: any[] }> {
    const response = await apiClient.get(
      `${this.baseUrl}/security/scan/${scanId}`
    )
    return response.data
  }
  ...
  async getPerformanceRecommendations(): Promise<{
    ...
    const response = await apiClient.get(
      `${this.baseUrl}/performance/recommendations`
    )
    return response.data
  }
  ...
  async applyPerformanceOptimization(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/performance/optimize`,
      {
        optimizations,
      }
    )
