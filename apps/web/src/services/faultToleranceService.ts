...
export interface ChaosScenario {
  name: string;
  type: 'network_delay' | 'service_failure' | 'resource_stress' | 'data_corruption';
  duration_ms: number;
  intensity: number;
...
class FaultToleranceService {
  ...
  // 故障事件
  async listFaultEvents(serviceId?: string, severity?: string, resolved?: boolean): Promise<FaultEvent[]> {
    const params: any = {};
    ...
    return items.filter((item: any) =>
      Array.isArray(item.affected_components) ? item.affected_components.includes(serviceId) : false,
    );
  }

  async getFaultEvent(eventId: string): Promise<FaultEvent> {
    const response = await apiClient.get(`${this.baseUrl}/faults`, { params: { limit: 200 } });
    const items: FaultEvent[] = response.data || [];
    const match = items.find((item: any) => item.fault_id === eventId || item.id === eventId);
    if (!match) {
  ...
  // 弹性策略管理
  async listResiliencePolicies(servicePattern?: string): Promise<ResiliencePolicy[]> {
  ...
  async createResiliencePolicy(policy: Omit<ResiliencePolicy, 'id' | 'created_at'>): Promise<ResiliencePolicy> {
    const response = await apiClient.post(`${this.baseUrl}/policies`, policy);
    return response.data;
  }

  async updateResiliencePolicy(policyId: string, updates: Partial<ResiliencePolicy>): Promise<ResiliencePolicy> {
    const response = await apiClient.put(`${this.baseUrl}/policies/${policyId}`, updates);
    return response.data;
  }
  ...
  async createFailureInjection(injection: Omit<FailureInjection, 'id'>): Promise<FailureInjection> {
    const response = await apiClient.post(`${this.baseUrl}/injections`, injection);
    return response.data;
  }

  async toggleFailureInjection(injectionId: string, enabled: boolean): Promise<void> {
    await apiClient.patch(`${this.baseUrl}/injections/${injectionId}`, { enabled });
  }
  ...
  async createRecoveryStrategy(strategy: Omit<RecoveryStrategy, 'id'>): Promise<RecoveryStrategy> {
    const response = await apiClient.post(`${this.baseUrl}/recovery-strategies`, strategy);
    return response.data;
  }
  ...
  async executeRecoveryStrategy(strategyId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/recovery-strategies/${strategyId}/execute`);
    return response.data;
  }
  ...
  async calculateAvailability(serviceId: string, timeRange: string): Promise<number> {
    const response = await apiClient.get(`${this.baseUrl}/metrics/availability`, {
      params: { service_id: serviceId, time_range: timeRange }
    });
  ...
  async createChaosExperiment(experiment: Omit<ChaosExperiment, 'id' | 'status'>): Promise<ChaosExperiment> {
    const response = await apiClient.post(`${this.baseUrl}/chaos/experiments`, experiment);
    return response.data;
  }
  ...
  async runChaosExperiment(experimentId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/chaos/experiments/${experimentId}/run`);
    return response.data;
  }

  async getChaosExperimentResults(experimentId: string): Promise<ChaosResults> {
    const response = await apiClient.get(`${this.baseUrl}/chaos/experiments/${experimentId}/results`);
    return response.data;
  }

  // 服务依赖分析
  async analyzeServiceDependencies(serviceId: string): Promise<ServiceDependency[]> {
    const response = await apiClient.get(`${this.baseUrl}/dependencies/${serviceId}`);
    return response.data;
  }
  ...
  async getFailurePropagationAnalysis(serviceId: string): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/dependencies/${serviceId}/impact-analysis`);
    return response.data;
  }
  ...
  async generateResilienceReport(timeRange: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/reports/resilience`, {
      time_range: timeRange
    });
    return response.data;
  }

  async exportFaultEvents(format: 'json' | 'csv' | 'pdf' = 'json'): Promise<Blob> {
  ...
  // 手动备份操作
  async triggerManualBackup(componentIds: string[], backupType?: string): Promise<{
  ...
  async getConsistencyStatistics(): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/consistency/statistics`);
    return response.data;
  }
  ...
  async repairConsistencyIssue(checkId: string): Promise<{
    ...
    const response = await apiClient.post(`${this.baseUrl}/consistency/${checkId}/repair`);
    return response.data;
  }

  async forceRepairConsistency(dataKey: string, authoritativeComponentId: string): Promise<{
    status: string;
    data_key: string;
    authoritative_component: string;
    timestamp: string;
  }> {
    const response = await apiClient.post(`${this.baseUrl}/consistency/force-repair`, {
      data_key: dataKey,
  ...
  // 故障注入测试
  async injectFault(faultType: string, targetService: string, duration?: number): Promise<{
    status: string;
    fault_id: string;
    component_id: string;
    fault_type: string;
    ...
    const response = await apiClient.post(`${this.baseUrl}/testing/inject-fault`, {
      component_id: targetService,
      fault_type: faultType,
      duration_seconds: duration ? Math.max(1, Math.round(duration / 1000)) : 60
    });
    return response.data;
  }
  ...
  async getRecoveryStrategies(): Promise<string[]> {
    const response = await apiClient.get(`${this.baseUrl}/enums/recovery-strategies`);
