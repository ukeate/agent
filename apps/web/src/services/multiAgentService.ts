...
export type AgentRole = 'researcher' | 'coder' | 'reviewer' | 'planner' | 'executor' | 'analyst';
export type ConversationStatus = 'pending' | 'active' | 'paused' | 'completed' | 'terminated' | 'error';
...
class MultiAgentService {
  ...
  async createConversation(request: CreateConversationRequest): Promise<ConversationResponse> {
    const response = await apiClient.post(`${this.baseUrl}/conversation`, request);
    return response.data;
  }

  async getConversationStatus(conversationId: string): Promise<ConversationStatusResponse> {
    const response = await apiClient.get(`${this.baseUrl}/conversation/${conversationId}`);
    return response.data;
  }

  async listConversations(limit?: number, offset?: number): Promise<{
  ...
  async pauseConversation(conversationId: string): Promise<{ success: boolean; message: string }> {
    const response = await apiClient.post(`${this.baseUrl}/conversation/${conversationId}/pause`);
    return response.data;
  }

  async resumeConversation(conversationId: string): Promise<{ success: boolean; message: string }> {
    const response = await apiClient.post(`${this.baseUrl}/conversation/${conversationId}/resume`);
    return response.data;
  }
  ...
  async batchTerminateConversations(
    ...
    const response = await apiClient.post(`${this.baseUrl}/conversations/batch/terminate`, {
  ...
  async cleanupOldConversations(
    ...
    const response = await apiClient.post(`${this.baseUrl}/conversations/cleanup`, {
