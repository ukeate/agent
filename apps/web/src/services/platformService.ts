...
class PlatformService {
  ...
  async registerComponent(request: RegisterComponentRequest): Promise<RegisterComponentResponse> {
    const response = await apiClient.post(`${this.baseUrl}/components/register`, {
      ...request,
  ...
  async deleteComponent(componentId: string): Promise<UnregisterComponentResponse> {
    const response = await apiClient.delete(`${this.baseUrl}/components/${componentId}`);
    return response.data;
  }
  ...
  async getWorkflowStatus(workflowId: string): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/workflows/${workflowId}/status`);
    return response.data;
  }
}
