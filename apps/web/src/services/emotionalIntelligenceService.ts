...
class EmotionalIntelligenceService {
  ...
  async getHighRiskUsers(
    threshold?: number
  ): Promise<Array<{ user_id: string; risk_score: number }>> {
  ...
  async getCrisisAlerts(active?: boolean): Promise<CrisisAssessment[]> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/crisis-alerts`, {
      params,
    })
  ...
  async acknowledgeCrisis(assessmentId: string, action: string): Promise<void> {
    await apiClient.post(`${this.baseUrl}/crisis/${assessmentId}/acknowledge`, {
      action,
    })
  }
  ...
  async createInterventionPlan(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/intervention-plan`,
      request
    )
  ...
  async getInterventionPlans(
    ...
    const response = await apiClient.get(
      `${this.baseUrl}/interventions/${userId}`,
      { params }
    )
  ...
  async updateInterventionEffectiveness(
    ...
    await apiClient.put(
      `${this.baseUrl}/intervention/${planId}/effectiveness`,
      { effectiveness }
    )
  }
  ...
  async getHealthDashboard(request: HealthRequest): Promise<HealthDashboard> {
    const response = await apiClient.get(
      `${this.baseUrl}/health-dashboard/${request.user_id}`,
      {
  ...
  async getHealthTrends(
    ...
    const response = await apiClient.get(
      `${this.baseUrl}/health-dashboard/${userId}`,
      {
  ...
  async getHealthAlerts(userId: string): Promise<
    Array<{
      alert_type: string
      severity: string
      message: string
  ...
  > {
    const response = await apiClient.get(
      `${this.baseUrl}/risk-trends/${userId}`
    )
  ...
  async getDecisionMetrics(period?: string): Promise<{
    ...
    const averageConfidence = decisions.length
      ? decisions.reduce(
          (sum: number, item: any) => sum + item.confidence_score,
          0
        ) / decisions.length
      : 0
    return {
      ...
      success_rate: decisions.length
        ? decisions.filter((item: any) => item.effectiveness_score).length /
          decisions.length
        : 0,
    }
  }
  ...
  async getRiskMetrics(): Promise<{
    ...
    return {
      ...
      risk_distribution: users.reduce(
      ...
      average_risk_score: users.length
        ? users.reduce((sum: number, item: any) => sum + item.risk_score, 0) /
          users.length
        : 0,
      trend: 'stable',
    }
  ...
  async batchAssessRisk(
    userIds: string[]
  ): Promise<Record<string, RiskAssessment>> {
    const results: Record<string, RiskAssessment> = {}
    for (const userId of userIds) {
      results[userId] = await this.assessRisk({
        user_id: userId,
        emotion_history: [],
      })
    }
  ...
  async batchHealthCheck(
    userIds: string[]
  ): Promise<Record<string, HealthDashboard>> {
   ...
   */
  async makeEmotionalDecision(
    request: DecisionRequest
  ): Promise<EmotionalDecision> {
    const response = await apiClient.post(
      `/emotional-intelligence/decide`,
      request
    )
    return response.data.decision || response.data
  }
  ...
  async performRiskAssessment(request: RiskRequest): Promise<RiskAssessment> {
    const response = await apiClient.post(
      `/emotional-intelligence/risk-assessment`,
      request
    )
    return response.data.risk_assessment || response.data
  }
  ...
  async generateInterventionPlan(
    ...
    const response = await apiClient.post(
      `/emotional-intelligence/intervention-plan`,
      request
    )
    return response.data.intervention_plan || response.data
  }
  ...
  async getEmotionalPatterns(userId: string): Promise<any> {
    const response = await apiClient.get(
      `/emotional-intelligence/emotional-patterns/${userId}`
    )
    return response.data
  }
  ...
  async assessSuicideRisk(request: any): Promise<any> {
    const response = await apiClient.post(
      `/emotional-intelligence/suicide-risk-assessment`,
      request
    )
    return response.data
  }
  ...
  async getRiskTrends(userId: string): Promise<any> {
    const response = await apiClient.get(
      `/emotional-intelligence/risk-trends/${userId}`
    )
    return response.data
  }
  ...
  async getCrisisPrediction(userId: string): Promise<any> {
    const response = await apiClient.get(
      `/emotional-intelligence/crisis-prediction/${userId}`
    )
    return response.data
  }
  ...
  async assessInterventionEffectiveness(request: any): Promise<any> {
    const response = await apiClient.post(
      `/emotional-intelligence/intervention-effectiveness`,
      request
    )
    return response.data
  }
  ...
  async getEmotionalIntelligenceSystemStatus(): Promise<any> {
    const response = await apiClient.get(
      `/emotional-intelligence/system-status`
    )
    return response.data
  }
  ...
  async performComprehensiveAnalysis(request: any): Promise<any> {
    const response = await apiClient.post(
      `/social-emotional-understanding/comprehensive-analysis`,
      request
    )
    return response.data
  }
  ...
  async getSocialEmotionalAnalytics(): Promise<any> {
    const response = await apiClient.get(
      `/social-emotional-understanding/analytics`
    )
    return response.data
  }
  ...
  async submitPrivacyConsent(consentData: any): Promise<any> {
    const response = await apiClient.post(
      `/social-emotion/privacy/consent`,
      consentData
    )
    return response.data
  }
  ...
  async analyzeEmotion(
    ...
    const response = await apiClient.post('/emotion-intelligence/analyze', {
      text,
      context,
    })
    return response.data
