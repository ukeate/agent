...
class ClusterManagementService {
  ...
  async startAgent(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/agents/${agentId}/start`
    )
    return this.unwrap(response.data)
  }
  ...
  async stopAgent(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/agents/${agentId}/stop`
    )
    return this.unwrap(response.data)
  }
  ...
  async restartAgent(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/agents/${agentId}/restart`
    )
    return this.unwrap(response.data)
  }
  ...
  async getClusterStats(): Promise<ClusterStats> {
    ...
    const updatedAtSeconds =
      typeof stats.updated_at === 'number'
        ? stats.updated_at
        : Date.now() / 1000
  ...
  async getClusterHealth(): Promise<{
    ...
    const healthScore =
      typeof health.health_score === 'number' ? health.health_score : 1
    if (failedAgents > 0) {
    ...
    if (Array.isArray(data)) {
      return data.map((group: any) => ({
        ...
        current_agents:
          ...
          (group.agent_ids ? group.agent_ids.length : 0),
        labels: group.labels,
        created_at: new Date(
          (group.created_at ?? Date.now() / 1000) * 1000
        ).toISOString(),
        updated_at: new Date(
          (group.updated_at ?? Date.now() / 1000) * 1000
        ).toISOString(),
      }))
    }
  ...
  async removeAgentFromGroup(groupId: string, agentId: string): Promise<void> {
    await apiClient.delete(
      `${this.baseUrl}/groups/${groupId}/agents/${agentId}`
    )
  }
  ...
  async getScalingPolicy(policyId: string): Promise<ScalingPolicy> {
    const response = await apiClient.get(
      `${this.baseUrl}/scaling/policies/${policyId}`
    )
    return this.unwrap(response.data)
  }
  ...
  async updateScalingPolicy(
    policyId: string,
    updates: Partial<ScalingPolicyRequest>
  ): Promise<ScalingPolicy> {
    const response = await apiClient.put(
      `${this.baseUrl}/scaling/policies/${policyId}`,
      updates
  ...
  async manualScale(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/scaling/groups/${groupId}/manual`,
      request
    )
    const data = this.unwrap<any>(response.data) || {}
    const success =
  ...
  > {
    ...
    const response = await apiClient.get(`${this.baseUrl}/scaling/history`, {
      params,
    })
    const data = this.unwrap<any>(response.data)
    if (!Array.isArray(data)) {
  ...
  async getMetrics(
    ...
    return baseSeries.map((_: any, index: number) => {
      ...
      const prevTotal =
        index > 0 ? getValue(data[keys.total], index - 1) : total
      const deltaTime = Math.max(0, total.timestamp - prevTotal.timestamp)
      const requestRate =
        deltaTime > 0
          ? Math.max(0, total.value - prevTotal.value) / deltaTime
          : 0
  ...
  async getAggregatedMetrics(
    ...
    const pick = (name: string) => {
      ...
      if (!entry) return 0
      const value =
        entry[aggregation || 'avg'] ?? entry.average ?? entry.latest ?? 0
