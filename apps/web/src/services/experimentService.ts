...
class ExperimentService {
   ...
   * 更新实验
   */
  async updateExperiment(id: string, config: Partial<ExperimentConfig>): Promise<ExperimentData> {
   ...
   * 导入实验
   */
  async importExperiments(file: File): Promise<{ imported: number; failed: number }> {
   ...
   * 获取实验事件
   */
  async getExperimentEvents(id: string, params?: { limit?: number }): Promise<any> {
    const response = await apiClient.get(`${this.baseUrl}/${id}/events`, { params });
    return response.data;
  }
   ...
   * 验证实验配置
   */
  async validateConfig(config: ExperimentConfig): Promise<{ valid: boolean; errors?: string[] }> {
   ...
   * 从模板创建实验
   */
  async createFromTemplate(templateId: string, overrides?: Partial<ExperimentConfig>): Promise<ExperimentData> {
