...
class PersonalizationService {
  ...
  private lastFeatureSnapshot: { version?: string; timestamp?: string } | null =
    null
  ...
  async getRecommendations(params?: {
    ...
    Array<{ id: string; item: string; score: number; reason: string }>
  > {
  ...
  > {
    ...
    const response = await apiClient.get(
      `${this.baseUrl}/features/realtime/${uid}`
    )
  ...
  > {
    ...
    return Object.entries(status).map(([modelId, info]: [string, any]) => {
      ...
      const isReady =
        (distribution.ready || 0) + (distribution.serving || 0) > 0
      const isLoading = (distribution.loading || 0) > 0
      const state: 'online' | 'offline' | 'updating' = isReady
        ? 'online'
        : isLoading
          ? 'updating'
          : 'offline'
  ...
  async acceptRecommendation(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/recommendations/${recommendationId}/accept`
    )
    return response.data
  }
  ...
  async trackEvent(
    event: Omit<PersonalizationEvent, 'event_id' | 'timestamp'>
  ...
  async getExperimentVariant(experimentId: string): Promise<ABTestVariant> {
    const response = await apiClient.get(
      `${this.baseUrl}/experiments/${experimentId}/variant`
    )
    return response.data
  }
  ...
  async recordExperimentEvent(
    ...
    const response = await apiClient.post(
      `${this.baseUrl}/experiments/${experimentId}/event`,
      event
    )
    return response.data
  }
  ...
  async importPersonalizationData(
    file: File
  ...
  async deletePersonalizationData(): Promise<{
    success: boolean
    message: string
  }> {
