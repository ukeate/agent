...
export class HypothesisTestingService {
   ...
   * 单样本t检验 - 适配到统一API
   */
  async oneSampleTTest(request: OneSampleTTestRequest): Promise<HypothesisTestResponse> {
    const response = await apiClient.post(`${API_BASE}/t-test/one-sample`, request);
   ...
   * 双样本t检验 - 适配到统一API
   */
  async twoSampleTTest(request: TwoSampleTTestRequest): Promise<HypothesisTestResponse> {
    const response = await apiClient.post(`${API_BASE}/t-test/two-sample`, request);
    return response.data;
  }
   ...
   * 配对t检验 - 适配到统一API
   */
  async pairedTTest(request: PairedTTestRequest): Promise<HypothesisTestResponse> {
   ...
   * 卡方拟合优度检验
   */
  async chiSquareGoodnessOfFit(request: ChiSquareGoodnessOfFitRequest): Promise<HypothesisTestResponse> {
    const response = await apiClient.post(`${API_BASE}/chi-square/goodness-of-fit`, request);
    return response.data;
  }
   ...
   * 卡方独立性检验
   */
  async chiSquareIndependence(request: ChiSquareIndependenceRequest): Promise<HypothesisTestResponse> {
    const response = await apiClient.post(`${API_BASE}/chi-square/independence`, request);
    return response.data;
  }
   ...
   * 两比例检验
   */
  async twoProportionTest(request: TwoProportionTestRequest): Promise<HypothesisTestResponse> {
    const response = await apiClient.post(`${API_BASE}/proportion-test`, request);
    return response.data;
  }
   ...
   * A/B测试比较分析
   */
  async abTestComparison(request: ABTestComparisonRequest): Promise<ABTestComparisonResponse> {
    const response = await apiClient.post(`${API_BASE}/ab-test-comparison`, request);
    return response.data;
  }
  ...
  generateContingencyTable(
    group1_category1: number, group1_category2: number,
    group2_category1: number, group2_category2: number
  ): number[][] {
    return [
