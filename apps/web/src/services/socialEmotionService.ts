...
export interface UserInteraction {
  ...
  user_id: string;
  timestamp: string;
  interaction_type: 'emotion_analysis' | 'empathy_response' | 'social_decision' | 'conflict_resolution';
...
class SocialEmotionService {
  ...
  async getUserInteractionStats(userId?: string): Promise<{
    ...
    const response = await apiClient.get(`${this.baseUrl}/interactions/stats`, { params });
    return response.data;
  }

  // System Alerts APIs
  async getSystemAlerts(unresolved_only: boolean = false): Promise<SystemAlert[]> {
  ...
  async updateConfiguration(module: string, settings: Record<string, any>): Promise<Configuration> {
    const response = await apiClient.put(`${this.baseUrl}/configurations/${module}`, settings);
    return response.data;
  }

  // Emotion Analysis APIs
  async analyzeEmotion(request: EmotionAnalysisRequest): Promise<EmotionAnalysisResponse> {
    const response = await apiClient.post(`${this.baseUrl}/analyze-emotion`, request);
    return response.data;
  }

  // Empathy Response APIs
  async generateEmpathyResponse(request: EmpathyResponseRequest): Promise<EmpathyResponseResponse> {
    const response = await apiClient.post(`${this.baseUrl}/generate-empathy`, request);
    return response.data;
  }
  ...
  async makeSocialDecision(context: {
    ...
    const response = await apiClient.post(`${this.baseUrl}/social-decision`, context);
    return response.data;
  }
  ...
  // Module Control APIs
  async startModule(moduleName: string): Promise<{ message: string; status: string }> {
    const response = await apiClient.post(`${this.baseUrl}/modules/${moduleName}/start`);
    return response.data;
  }

  async stopModule(moduleName: string): Promise<{ message: string; status: string }> {
    const response = await apiClient.post(`${this.baseUrl}/modules/${moduleName}/stop`);
    return response.data;
  }

  async restartModule(moduleName: string): Promise<{ message: string; status: string }> {
    const response = await apiClient.post(`${this.baseUrl}/modules/${moduleName}/restart`);
    return response.data;
  }
}
