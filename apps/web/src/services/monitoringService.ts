...
class MonitoringService {
      ...
      if (sys.cpu_usage !== undefined) {
        metrics.cpu_usage = {
          ...
          statistics: {
            ...
            max: 100,
            avg: sys.cpu_usage,
            median: sys.cpu_usage,
            latest: sys.cpu_usage,
          },
        }
      }
      if (sys.memory_usage !== undefined) {
        metrics.memory_usage = {
          ...
          statistics: {
            ...
            min: 0,
            max: 100,
            avg: sys.memory_usage,
            median: sys.memory_usage,
            latest: sys.memory_usage,
          },
        }
      }
    }
    ...
    if (backendData.api_metrics) {
      ...
      if (api.response_time !== undefined) {
        metrics.api_response_time = {
          ...
          points: [],
          statistics: {
            count: api.request_count || 100,
            min: 50,
            max: 500,
            avg: api.response_time,
            median: api.response_time,
            latest: api.response_time,
          },
        }
      }
    }
  ...
  async getMetric(metricName: string): Promise<Metric> {
    const response = await apiClient.get(
      `${this.baseUrl}/metrics/${metricName}`
    )
    return response.data
  }
  ...
  async getMetricHistory(
    ...
    const response = await apiClient.get(
      `${this.baseUrl}/metrics/${metricName}/history`,
      { params }
    )
    return response.data
  }
