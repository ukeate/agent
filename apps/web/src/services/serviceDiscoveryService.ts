...
export class ServiceDiscoveryService {
  ...
  async updateAgentStatus(agentId: string, status: string): Promise<any> {
    const response = await apiClient.put(`${this.baseUrl}/agents/${agentId}/status`, {
      status: { value: status }
    });
    return response.data;
  }
  ...
  async updateAgentMetrics(agentId: string, metrics: {
    ...
    const response = await apiClient.put(`${this.baseUrl}/agents/${agentId}/metrics`, metrics);
    return response.data;
  }
  ...
  async selectAgent(request: LoadBalancerRequest): Promise<LoadBalancerResponse> {
    const response = await apiClient.post(`${this.baseUrl}/load-balancer/select`, request);
    return response.data;
  }
  ...
  async getAgentsByCapability(capability: string): Promise<AgentMetadataResponse[]> {
