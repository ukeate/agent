...
describe('filesService', () => {
  ...
  it('调用 uploadFile 时正确发送请求', async () => {
    ...
    postMock.mockResolvedValue({ data: mockData } as MockResponse<
      typeof mockData
    >)
  ...
  it('调用 listFiles 时传递查询参数', async () => {
    const mockData = {
      success: true,
      data: { files: [], total: 0, limit: 10, offset: 0 },
    }
    getMock.mockResolvedValue({ data: mockData } as MockResponse<
      typeof mockData
    >)
    ...
    expect(getMock).toHaveBeenCalledWith('/files/list', {
      params,
      preserveEnvelope: true,
    })
    expect(result).toEqual(mockData)
  ...
  it('调用 getFileInfo 时访问正确路径', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/files/abc', {
      preserveEnvelope: true,
    })
    expect(result).toEqual(mockData)
  })
  ...
  it('调用 downloadFile 时设置 responseType', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/files/file-id/download', {
      responseType: 'blob',
    })
  ...
  it('调用 deleteFile 时使用 DELETE 方法', async () => {
    const mockData = { success: true }
    deleteMock.mockResolvedValue({ data: mockData } as MockResponse<
      typeof mockData
    >)

    const result = await filesService.deleteFile('file-id')

    expect(deleteMock).toHaveBeenCalledWith('/files/file-id', {
      preserveEnvelope: true,
    })
    expect(result).toEqual(mockData)
  })
  ...
  it('调用 cleanupOldFiles 时传递 days 参数', async () => {
    ...
    expect(postMock).toHaveBeenCalledWith('/files/cleanup', null, {
      params: { days: 3 },
      preserveEnvelope: true,
    })
    expect(result).toEqual(mockData)
  ...
  it('调用 getFileStats 时访问统计端点', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/files/stats/summary', {
      preserveEnvelope: true,
    })
    expect(result).toEqual(mockData)
  })
