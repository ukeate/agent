...
describe('FeedbackForm', () => {
  ...
  describe('基本渲染', () => {
    ...
    it('应该支持隐藏特定组件', () => {
      ...
      expect(screen.queryByTestId('like-dislike-component')).not.toBeInTheDocument();
  ...
  describe('状态管理', () => {
    ...
    it('应该显示反馈摘要', async () => {
      ...
      // 应该更新摘要
      await waitFor(() => {
        expect(screen.getByText(/反馈摘要: .*评分: 5\/5.*喜欢.*已收藏/)).toBeInTheDocument();
      });
    });
  });
  ...
  describe('边界情况', () => {
    ...
    it('应该正确清除验证错误', async () => {
      ...
      expect(screen.queryByText('• 评论长度不能超过5字符')).not.toBeInTheDocument();
    });
  });
