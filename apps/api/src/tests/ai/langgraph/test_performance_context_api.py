...
class TestPerformanceContextAPI:
    ...
    async def test_context_api_vs_legacy_config_performance(self):
        ...
        # 测试新Context API模式
        logger.info("开始测试新Context API模式性能...")
        ...
        logger.info(f"\\n=== 性能对比结果 ===")
        logger.info(f"传统config模式:")
        logger.info(f"  平均执行时间: {legacy_avg:.4f}秒")
        logger.info(f"  中位数时间: {legacy_median:.4f}秒")
        logger.info(f"  标准差: {legacy_std:.4f}秒")
        
        logger.info(f"\\n新Context API模式:")
        logger.info(f"  平均执行时间: {context_api_avg:.4f}秒")
        logger.info(f"  中位数时间: {context_api_median:.4f}秒")
        logger.info(f"  标准差: {context_api_std:.4f}秒")
    ...
    async def test_context_serialization_performance(self):
        ...
        logger.info(f"序列化平均时间: {serialization_avg:.6f}秒")
        logger.info(f"反序列化平均时间: {deserialization_avg:.6f}秒")
        logger.info(f"总体往返时间: {(serialization_avg + deserialization_avg):.6f}秒")
    ...
    async def test_node_caching_performance(self):
        ...
        for i in range(test_iterations):
            ...
            start_time = time.time()
            result = await expensive_computation(test_data)
            end_time = time.time()
        ...
        logger.info(f"不使用缓存平均时间: {non_cache_avg:.6f}秒")
        logger.info(f"使用缓存平均时间: {cache_avg:.6f}秒")
        logger.info(f"缓存性能提升: {performance_improvement:.1f}%")
        
        # 获取缓存统计
        cache_stats = await cache.get_stats()
        logger.info(f"缓存命中率: {cache_stats['hit_rate']:.1%}")
        logger.info(f"缓存条目数: {cache_stats['cache_entries']}")
    ...
    async def test_memory_usage_comparison(self):
        ...
        logger.info(f"传统config模式内存使用: {legacy_memory:,} bytes")
        logger.info(f"新Context API内存使用: {context_api_memory:,} bytes")
...
class TestPerformanceBenchmarks:
    ...
    async def test_full_performance_report(self):
        """生成完整的性能报告"""
        logger.info("\\n" + "="*60)
        logger.info("LANGGRAPH v0.6.5 CONTEXT API 性能基准报告")
        logger.info("="*60)
        ...
        logger.info("\\n=== 性能测试总结 ===")
        logger.info(f"Context API vs Legacy: {context_api_report['performance_improvement_percent']:.1f}% 改善")
        logger.info(f"缓存性能提升: {caching_report['performance_improvement_percent']:.1f}%")
        logger.info(f"内存使用改善: {memory_report['memory_improvement_percent']:.1f}%")
        logger.info(f"整体性能等级: {full_report['summary']['overall_performance_grade']}")
