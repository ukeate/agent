...
from src.services.qlearning_service import QLearningService
from src.services.qlearning_strategy_service import (
...
from src.ai.qlearning.agent_monitor import (
...
# Pydantic模型定义
class AgentConfig(ApiBaseModel):
    """智能体配置"""
...
class TrainingConfig(ApiBaseModel):
    """训练配置"""
...
class StrategyInferenceRequestModel(ApiBaseModel):
    """策略推理请求模型"""
    state: List[float] = Field(..., description="状态向量")
...
@router.get("/summary")
async def qlearning_summary():
    """返回当前Q-Learning会话与策略概要（无静态数据）"""
    try:
...
async def qlearning_checkpoints():
    """
    返回当前可用的训练检查点。
    """
...
async def create_agent(config: AgentConfig):
    """创建Q-Learning智能体"""
    try:
        session_id = await qlearning_service.create_agent_session(config.model_dump())
...
async def start_training(session_id: str, config: TrainingConfig, background_tasks: BackgroundTasks):
    """开始训练智能体"""
    try:
        success = await qlearning_service.start_training(session_id, config.model_dump())
        if not success:
...
class AgentEventRequest(ApiBaseModel):
    """智能体事件请求"""
...
async def log_agent_event(session_id: str, request: AgentEventRequest):
    """手动记录智能体事件"""
    try:
        from src.ai.qlearning.agent_monitor import AgentEvent, LogLevel
